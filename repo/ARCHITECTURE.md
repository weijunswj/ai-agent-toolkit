...
For sufficiently complex/STRICT work involving concurrency, async completion, deferred consequential work, causal negative controls, lifecycle coordination, privilege/context boundaries, identity/resource equivalence or comparable cross-surface implementation risk, G3 performs a mandatory pre-publication adversarial validation episode inside the same G3. After the parent reaches a green integrated in-contract validation point and before publication, it launches a depth-1 read-only adversarial challenge leaf through the selected stack's separately configured G3 adversarial-subagent route. That route is intentionally stronger than the ordinary G3 implementation route. The challenge tries to falsify materially equivalent paths, privilege/context assumptions, lifecycle/recovery, production-boundary reachability, the regression oracle, causal attribution, liveness/progress, outstanding-work accounting, resource-equivalence handling and false-green controls. The leaf never mutates, publishes, grants authority or declares completion. If it exposes a material settled in-contract implementation defect, the parent remains sole integrator, fixes the implementation inside the same G3 episode, reruns the affected integrated validation floor to green, then launches a fresh read-only adversarial challenge leaf against the updated implementation. The parent repeats this challenge/refine/revalidate/re-challenge cycle until the current green implementation survives a fresh challenge with no unresolved material in-contract finding, or ordinary G3 convergence/non-convergence law requires return to Web. Sequential challenge leaves are permitted; there is no one-leaf-per-G3 ceiling, no nested delegation and no new G3 RUN/Lock merely because a prior challenge found an implementation defect. For simple/low-risk G3 this leaf is optional unless the accepted contract/Web requires it. It is not another gate: missing product/compatibility semantics return to G2, and changed root/trust/architecture returns to G1.
