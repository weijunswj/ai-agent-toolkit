...
Canonical Toolkit governance identity is remote-first: the authoritative source is canonical `weijunswj/ai-agent-toolkit` `main` at an exact bound commit plus exact Controller/registry blobs. A local Toolkit checkout is cache/transport only and never authority; its working tree, local HEAD, current branch and local refs cannot redefine governance. Exact local Git objects may be reused only after their canonical remote identities are already bound.

Root model/route selection is an out-of-band User/Web/controller/harness act. Concrete provider/model/reasoning choices are selected through an explicitly named stack registry binding and are configuration, not architecture law. There is no authoritative default stack. Stack selection and physical harness selection are orthogonal. User/Web/controller/launcher selects the named root route and records it in trusted orchestration state before launch or adoption; the resolved route is displayed before launch, and an interactive desktop launch after human route selection establishes the binding. A root semantic worker never verifies or attests its own model identity; it never inspects, verifies or attests its own provider/model/reasoning identity, and missing runtime model metadata is not a HOLD. A route handoff is a pre-launch/adoption orchestration decision when the selected route cannot be established; if the required route cannot be selected/configured, the worker is not launched and User/Web may choose another registered stack/harness or explicitly change the route. Silent fallback remains prohibited. One logical lane may hand off between qualified harnesses by moving to another separately human/launcher-bound session without changing semantic gate identity, RUN/Lock, ownership, correction accounting or candidate identity. Service treatment/speed is non-authoritative observed metadata. A route or harness change that preserves these stage semantics and authority boundaries does not require an architecture redesign.
