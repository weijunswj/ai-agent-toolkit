...
Every invocation resolves and deeply freezes one immutable `toolkit.local-bridge.invocation-authority.v1` value from its entrypoint, explicit request, initial raw state, exact destinations, and source identity. It is the only executable permission source. After every lock acquisition or reacquisition, the Bridge verifies the exact ownership token, rereads state, repeats discovery, compares authority-relevant bindings, and creates a short-lived narrowed phase context. That context carries no independent authority, expires on release or lock loss, and cannot be reused. The historical `invocation-action-scope-v1.schema.json` remains evidence only and is neither accepted nor emitted as executable write authority.

`--write` is execution consent, not mutation authority. It performs no write unless the immutable invocation authority contains the exact preference, target, repository, native, report, failure-status, delegation, or staging action and destination. Runtime state and discovery may narrow or skip authorised work but never add an action or destination. `--preference-only` may change only explicitly named global fields under the bridge lock. It does not discover or normalize target state, generate adapters, replace the hub, sync destinations, refresh native caches, repair hooks, update repositories, clean reports, or create maintenance reports.

Runtime audit fields such as `authorised_actions`, `eligible_actions`, `blocked_actions`, and `planned_writes` are descriptive projections only. They are derived from the immutable authority and current observations and are never consumed as permission. Lock ownership, temp files, staging generations, and backups remain bounded implementation mechanics and never grant an action.

Managed effects use private action-specific executors. The executor derives or copies the exact operand, revalidates the action, owned lock, relevant durable state, destination/source continuity, and operand identity, immediately performs that fixed operation, verifies its postcondition, and returns detached frozen evidence. Every loop iteration, retry, fallback, cleanup, restoration, report operation, and failure-status persistence crosses a new admission. There is no generic callback executor, reusable permit, arbitrary command/path executor, or persistent capability ledger.

Repository verification originates the frozen `toolkit.local-bridge.verified-source-receipt.v1` before returning control. The receipt binds repository/ref/remote, commit and tree, clean-worktree result, package version, platform, Node/Git executable identities, Bridge entry digest, validation result, and the complete static local-require source-closure inventory. Exact Git-object bytes are retained only in a private in-memory capsule. Continuity is checked at repository verification, post-verification, refresh relock, envelope construction, pre-launch, and child start; later observations may reject but never replace the admitted identity.

Repo-update delegation uses one fixed `toolkit.local-bridge.delegated-invocation-authority.v1` JSON payload inside a receipt-backed child capsule. Admitted source bytes are staged by executor-owned per-entry operations and read back against the receipt. A fixed `node -e` bootstrap hashes the same buffers it compiles, classifies POSIX-slash, Windows-backslash, and absolute local JavaScript/JSON requests before loader dispatch, confines every such request to the admitted manifest, and confirms a child-start handshake binding receipt ID, commit, tree, entry digest, and manifest digest. Package specifiers are not reinterpreted as local requests. The child accepts no extra fields or report/repository/native permission, establishes its own hub mutation phase, rereads current state and discovery, and rejects any mismatch before writing. `--parent-scope-digest` and write-mode `--scope-target-sync` are retired.

Codex cache setup is a separate narrowly delegated native child. Its canonical `--delegated-invocation-authority` capsule binds the verifier-owned receipt, original setup-source and cache fingerprints, repository, Codex home, Node executable, package version, environment, and exact effect ceiling. Before any native effect, the child freshly recomputes the actual repository/native source closure against the verifier-owned fingerprint and exclusively creates its own independently revocable mutation-phase record under the bound Codex home. Every Codex CLI command or asynchronous continuation, child termination, per-directory creation, SessionStart temporary write/rename/cleanup, TOML parser launch, postcondition, and phase release repeats the source-closure proof and proves the exact phase record, bound durable identities, and exact operands immediately before dispatch. SessionStart publication additionally binds every existing destination ancestor plus the source and destination objects; ancestor replacement or redirection rejects write, rename, and cleanup before dispatch. Phase replacement or source drift fails before the next effect, and revoked phase evidence is never cleaned up as though authority remained valid. Delegated continuity failures are not swallowed into marketplace retry or reinstall fallback. Downstream refresh may reject the original receipt but cannot recapture a replacement digest as authority. Windows n8n hook reconciliation supplies a pure repair plan; the Bridge applies each planned file itself. Staging helpers likewise provide inspection/plans to Bridge-owned effects. Standalone compatibility writers remain outside the managed Bridge route.

These controls assume a cooperative user-space host. They detect ordinary process, path, state, source, and file-identity drift; they do not claim protection from a hostile kernel, administrator, or a process able to rewrite the executing runtime itself.
...
7. Resolves `FETCH_HEAD` to the admitted immutable commit, then updates only with `git merge --ff-only <exact-admitted-commit>`; later `FETCH_HEAD` movement cannot substitute the executed operand.
...
```powershell
node <repo_path>/repo/scripts/toolkit-local-bridge.cjs --write --sync-source repo --hub <same-hub> --skip-repo-auto-update --suppress-update-report --delegated-invocation-authority --audit
```

The parent sends the fixed delegated invocation-authority payload on stdin. Flags and durable enabled-target state cannot add child actions, and delegated children do not independently clean, create, or open update reports.
...
7. Install the correct repo-local instructions through the normal user-approved `repository-agent-rules` flow. Open another fresh task and verify the missing-rule warning no longer appears.
