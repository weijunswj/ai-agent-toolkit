...
- It acquires an exclusive cross-process lock derived from the exact resolved selected cache before final reclassification. Separate Toolkit hubs targeting the same cache contend on the same lock, while different cache roots have distinct lock identities. Dead owners recover immediately; a target-only bounded live-owner lease prevents an orphan whose numeric PID was later reused from blocking that cache forever. Recovery still uses the exact lock-generation token, rename, recovery-marker, and tombstone protocol, and never signals the recorded process. A loser re-evaluates a winner's healthy result or fails without writing.
...
- The complete authority is revalidated after parsing, after lock acquisition, before current/obsolete adjudication, before every target/stage/backup rename or deletion, after obsolete restoration, before staging and displacement, before backup-cleanup authorization, before every resumable cleanup operation, before each evidence retirement, and before completion. Legitimate phases advance one exclusive append-only transition at a time while all prior bytes remain exact; gaps, overwrites, unexpected phase-shaped files, and newly appeared optional markers fail closed. Cleanup receives the prior authority, moves each exact authorized object to a unique quarantine name before deletion, verifies the moved object and bytes, and never treats a fresh pathname snapshot as historical ownership. A replacement introduced before or at cleanup remains inspectable and the transaction is not reported complete.
...
- Normal pre-verification failures roll back exactly. Installed verification precedes an append-only backup-cleanup authorization bound to the exact original backup and verified winner directory identities. The complete identity-bound healthy winner is revalidated before backup retirement, before every resumable backup deletion, after retirement, and again at the success-return boundary. Drift at any of those boundaries is `final-winner-drift`, never `repaired`, and preserves the exact transaction authority. If exact backup deletion is interrupted or partially blocked on Windows, the winner is not rolled back: the next run resumes bounded deletion inside that recorded ordinary directory, rejects links, redirects, special entries or identity changes, and then removes only the exact owned stage, backup, transaction, phase, and lock evidence. A full selected-tree digest binds every preserved byte to the approval and transaction, while the exact adapter fingerprints remain the compatibility decision.
