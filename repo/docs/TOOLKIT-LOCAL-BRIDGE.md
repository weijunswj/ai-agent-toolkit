...
Every Claude invocation path uses `repo/scripts/claude-process-launch.cjs`: plugin list/install/update, `--help` and `--version` probes, and direct workers share identical executable validation and argument boundaries. Direct launch refuses known-missing bare or explicit executables before admission. Bare `claude` retains Windows shim behavior; `.js`/`.cjs`/`.mjs` and explicit `.exe` paths run without a shell; explicit `.cmd`/`.bat` paths use `cmd.exe /d /s /v:off /c` with command and argument metacharacters escaped. Empty, quoted, control-character, relative Windows, otherwise ambiguous, or known-missing executable inputs are rejected. Children deny both `Agent` and legacy `Task`; prompts remain stdin-only.
