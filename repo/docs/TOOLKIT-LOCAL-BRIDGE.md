...
Repo-update delegation uses one fixed `toolkit.local-bridge.delegated-invocation-authority.v1` JSON payload inside a receipt-backed child capsule. Admitted source bytes are staged by executor-owned per-entry operations and read back against the receipt. A fixed `node -e` bootstrap hashes the same buffers it compiles, restricts relative executable loads to the manifest, and confirms a child-start handshake binding receipt ID, commit, tree, entry digest, and manifest digest. The child accepts no extra fields or report/repository/native permission, establishes its own hub mutation phase, rereads current state and discovery, and rejects any mismatch before writing. `--parent-scope-digest` and write-mode `--scope-target-sync` are retired.

Codex cache setup is a separate narrowly delegated native child. Its canonical `--delegated-invocation-authority` capsule binds the verifier-owned receipt, original setup-source and cache fingerprints, repository, Codex home, Node executable, package version, environment, and exact effect ceiling. Before any native effect, the child exclusively creates its own independently revocable mutation-phase record under the bound Codex home. Every Codex CLI command or asynchronous continuation, per-directory creation, SessionStart temporary write/rename/cleanup, and TOML parser launch freshly proves that exact phase record, bound durable identities, and exact operands immediately before dispatch. Phase replacement or source drift fails before the next effect, and revoked phase evidence is never cleaned up as though authority remained valid. Downstream refresh may reject the original receipt but cannot recapture a replacement digest as authority. Windows n8n hook reconciliation supplies a pure repair plan; the Bridge applies each planned file itself. Staging helpers likewise provide inspection/plans to Bridge-owned effects. Standalone compatibility writers remain outside the managed Bridge route.
