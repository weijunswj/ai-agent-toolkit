...
Every Claude invocation path uses `repo/scripts/claude-process-launch.cjs`: plugin list/install/update, `--help` and `--version` probes, and direct workers share identical executable validation and argument boundaries. Bare `claude` retains Windows shim behavior; `.js`/`.cjs`/`.mjs` and explicit `.exe` paths run without a shell; explicit `.cmd`/`.bat` paths use `cmd.exe /d /s /v:off /c` with command and argument metacharacters escaped. Empty, quoted, control-character, relative Windows, or otherwise ambiguous executable inputs are rejected. Prompts remain stdin-only.
