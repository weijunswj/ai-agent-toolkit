...
Claude Code can package agents with `effort: medium` and omit the child `Agent` tool, but plugin agent definitions have the lowest precedence, environment and per-invocation model controls can override them, and `SubagentStart` fires after spawning. Fast mode is a session setting with no verified child-only override. Toolkit therefore does not expose Claude topology choices that would claim global root-only, direct-only, nested, adaptive-admission, or strict non-fast enforcement. A future profile may be added only after every admitted spawn path can be intercepted before launch and effective child effort plus non-fast mode can be verified. Codex and Claude limitations are separate; neither host's state proves the other's behavior.
...
```powershell
node ".../repo/scripts/toolkit-codex-session-start.cjs" --hook --sync-enabled --write --sync-source codex-plugin
```
