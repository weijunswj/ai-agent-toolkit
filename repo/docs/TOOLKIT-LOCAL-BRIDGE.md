...
The wizard groups consequential decisions under `Automatic updates`, `Computer performance`, and `Other coding apps`. Each block shows what it controls, the current user-visible outcome, the recommended user-visible outcome, and choices in that order. Raw paths, runtime names, TOML keys, ownership, migration, backup, and restore details stay out of the primary bank. OpenCode and Antigravity rows are omitted unless detected or already enabled; Claude choices appear only on the Claude setup path. Report creation and retention remain under automatic maintenance, but report auto-open is no longer a question: action-required reports open automatically and successful reports stay closed.

The canonical Codex question is `How many helper agents may Codex use?` and recommends one helper at most, with no helpers, keep current, and advanced as the other ordinary choices. Existing unsafe capacity carries a visible memory-risk label. Runtime-specific keys and root-inclusive capacity arithmetic appear only in the technical proposal. When user-owned effective controls require replacement, setup immediately prints the exact runtime, target file, before/after behavior, affected keys, structural block, backup generation metadata, and shell-safe PowerShell/POSIX restore commands, then requests one `apply` confirmation. Approval completes the transaction in the same setup flow; refusal or unsupported detection says the selected outcome remains unapplied and prevents a completed verdict. Verification after managed setup uses the managed checkout verifier. Codex agents verify and refresh only the Codex native Toolkit plugin cache installed by the supported plugin flow, and hook trust guidance identifies the executing SessionStart bridge in that installed cache, while the managed checkout is only its refresh source. Claude Code verifies `.claude-plugin/plugin.json` and `.claude-plugin/hooks/hooks.json`, then follows the equivalent isolated Claude cache path. Routine setup uses `repo/tests/toolkit-local-bridge-hook-light.test.cjs`; the full bridge suite remains reserved for bridge changes, PR review, and release validation.
