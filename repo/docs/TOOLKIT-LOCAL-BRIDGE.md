...
Claude Code 2.1.198 exposes a usable Toolkit-controlled external-session boundary: `--print`, `--output-format`, `--effort`, `--disallowedTools`, and `--permission-mode` are available for a fresh child invocation; `CLAUDE_CODE_DISABLE_FAST_MODE=1` hard-disables fast mode for that child process. Under the `claude-toolkit-direct` profile, the plugin `PreToolUse` hook denies native `Agent` or legacy `Task` launches because those would bypass admission, while `repo/scripts/toolkit-agent-control.cjs` starts only direct external sessions with medium effort by default, fast mode disabled, background tasks disabled, and `Agent` disallowed. The root's own effort and mode are untouched. The bounded child prompt is transported over stdin, never the Claude argv; the private job specification, stdout, stderr, profile, state, and reservation metadata are created or atomically replaced as regular private files with explicit restrictive permissions.
...
Setup exposes separate canonical Claude topology and capacity rows only when the required CLI flags verify. The supported managed topology is direct-only; no Toolkit-controlled nesting is advertised. Capacity choices are automatic, root-only, keep-current, and a manual maximum. Topology and capacity are resolved as one compatible outcome before plan, approval, or write. Root-only and broader-native resolve to root-only capacity; only verified direct mode can use automatic or manual admission. Keep-current is accepted only when the current profile remains compatible and enforceable. The manual maximum and a hidden emergency ceiling are backstops, never launch permission.
...
The automatic controller reads physical memory availability plus commit/pagefile headroom (Win32 operating-system counters on Windows or `/proc/meminfo` on Linux). Unknown, malformed, unsupported-platform, or critical-pressure state returns `refuse-root-only`. It retains the larger of 4 GiB or 25% of physical memory and the larger of 6 GiB or 20% of commit capacity for the OS and user applications, then subtracts the requested worker estimate (2 GiB by default, bounded from 1 to 16 GiB). Temporary pressure, a full manual backstop, or the emergency ceiling returns a bounded observable `queue`; unsafe state or a full queue returns `refuse-root-only`.

One atomic directory lock protects stale recovery, queue insertion, and reservation creation across concurrent parents. Reservations record owner PID, topology, depth, effort, estimate, creation, and expiry. Expired reservations are reclaimed only when their owner is also dead; live ownership is never deleted merely because a clock expired. A detached Toolkit supervisor becomes the reservation owner, launches the child, records output paths, and releases on child exit or launch failure. Abandoned dead ownership expires safely. The controller never kills, suspends, enumerates, or reports unrelated processes.
