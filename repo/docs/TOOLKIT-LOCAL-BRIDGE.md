...
Claude Code has the same three behavior choices in the setup question bank: `keep`, `instructions`, and recommended `install`. Installation freshness requires the reported identity, version, enabled state, exact source path, exact installed-cache path, and byte-for-byte current manifest, hooks, controller, process-launch helper, native-agent hook, n8n admission hook, external-system router, and n8n domain router. Strict enforcement is a separate result requiring host-reported `trusted: true` plus active hook execution. Activation-proof schema 5 binds the exact cache identity and SHA-256 hashes for all of those executable surfaces; every bound installed path must be a non-symlink regular file, and older proof schemas fail closed as stale. If the host does not expose a supported active indication, setup reports strict enforcement unavailable rather than inferring it from bytes or enabled state. Setup uses supported marketplace/install/update commands and `--scope user`; it never writes trust state or treats restart guidance as activation proof. Codex must not install or update Claude Code, and Claude Code must not install or update Codex.

Claude Toolkit-direct also installs a fail-closed n8n domain adapter. `UserPromptSubmit`/prompt expansion establish the task-local operation ledger, `PostToolUse` accepts only successful exact official Skill evidence, `PreToolUse` denies governed workflow mutations until every required capability is verified, and `TaskCompleted`/`Stop` deny premature completion. The logical classifier and ledger are shared, but this native hook enforcement is proven only for Claude Code. Codex consumes the same policy and completion contract through instructions and skill routing; Toolkit does not claim a Codex mutation-denial hook unless that host capability is separately proven.
