...
- While holding that target lock, it stages only the exact selected cache through the existing owned-generation mechanism, revalidates immediately before replacement, verifies the installed result, rolls back exactly on failure, removes owned stage/backup residue, and finally releases and cleans the lock. A full selected-tree digest binds every preserved byte to the approval and transaction, while the exact adapter fingerprints remain the compatibility decision. Tree identity uses bounded chunked hashing plus explicit per-file, total-byte, file-count, directory-count, and depth limits; exceeded limits, symbolic links, junctions, and other redirected or special entries fail closed before copying.
