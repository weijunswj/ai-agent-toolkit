...
Repo-update delegation uses one fixed `toolkit.local-bridge.delegated-invocation-authority.v1` JSON payload over the direct child process's stdin. It contains only the parent invocation identity, approved target actions, exact hub and target destinations, verified repository result, and child source identity. The child accepts no extra fields or report/repository/native permission, proves its own source and repository-result identity, acquires the hub lock, rereads current state and discovery, and rejects any mismatch before writing. `--parent-scope-digest` and write-mode `--scope-target-sync` are retired.
