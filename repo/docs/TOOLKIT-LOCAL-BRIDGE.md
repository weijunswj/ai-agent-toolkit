...
- One iterative, deterministically ordered bounded tree inspection derives both the full selected-tree identity and the exact critical `hooks/` file set. It enforces 4,096 files, 1,024 directories, 256 MiB total bytes, 64 MiB per file, and depth 32; rejects symbolic links, junctions, redirected directories, special entries, and changed-during-read files; and fails `unsupported-layout` before lock acquisition or staging.
- While holding that target lock, it stages only the exact selected cache through the existing owned-generation mechanism and revalidates immediately before replacement. Before displacing the canonical target, it durably registers the exact normalized target identity, selected version and adapter/repair profile, authoritative compatibility-contract and source-lock byte identities, source-watch identity, approval and staged tree evidence, deterministic stage and backup paths, filesystem directory identities, generation/token, owner metadata, and append-only transition phases. A later SessionStart reruns parity and adjudicates any one exact interrupted owned transaction under the same target lock before ordinary current-cache selection: it preserves a verified canonical winner, completes installation from the exact recorded stage, or restores only the exact identity-bound original backup. Malformed, ambiguous, conflicting, aliased, redirected, or mismatched evidence fails closed; unrelated caches and backup-like decoys are never scanned or changed.
