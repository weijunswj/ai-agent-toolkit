...
1. When no lock exists, acquisition first respects an active recovery claim: the main lock is absent for the entire window in which a recovery has displaced it, and the recovery marker exists for that entire window, so a live or fresh marker makes hook runs skip and manual runs fail before any create is attempted. No replacement writer can enter behind the recoverer. Only then is exclusive creation (`wx`) of the lock the atomic claim; losing that race skips (hook) or fails (manual).
...
5. The recoverable lock is displaced by rename (not deleted in place) and the displaced file's owner is verified not alive before it is discarded. A live owner's file is restored untouched to its exact path when that path is free; if restoration is impossible (the path re-occupied or a rename failure), the protocol fails closed: the displaced live-owner lock is preserved on disk as evidence, this contender does not write a replacement, and the skip/error message names the preserved path.
...
7. Preserved displaced evidence is a persistent fail-closed acquisition barrier, not just a one-run refusal. Every acquisition -- including the no-main-lock path -- inspects `update.lock.displaced.*` evidence before creating anything, so a later contender does not acquire merely because the recovery marker was released, the intruding main lock went away, or the evidence is old. While the recorded owner is alive the evidence blocks and is never deleted, protecting a long-running displaced owner indefinitely; an owner whose liveness is indeterminate (for example a permission-denied probe) also fails closed; evidence with unusable owner data uses the age fallback. Hook runs skip and manual runs fail with the preserved evidence path and owner status; the messages never tell the user to delete evidence that may belong to a live owner.
8. Once the displaced owner is provably dead, the evidence is retired through the same atomic identity-safe tombstone protocol as marker reclaim (exclusive tombstone on the inspected bytes, verify-under-claim, yield untouched if the generation changed), so exactly one contender retires each generation and the barrier cannot become a permanent wedge after the owner dies. Artifact cleanup ages out only spent reclaim/retirement tombstones; it never deletes displaced evidence by age.
