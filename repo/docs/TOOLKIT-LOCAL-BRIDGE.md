...
Claude Code has the same three behavior choices in the setup question bank: `keep` (no action), `instructions` (verify metadata only and report manual native refresh instructions), and `install` (recommended default), which runs `node repo/scripts/setup-claude-toolkit-plugin.cjs` through the same verify-then-write-then-verify sequence as Codex, using the supported `claude plugin marketplace add <repo>` and `claude plugin install ai-agent-toolkit@ai-agent-toolkit-local --scope project` commands and `claude plugin list --json` for verification. Unlike Codex, this script does not fingerprint a version-pinned plugin cache directory, because Claude Code's local-marketplace install path has no documented equivalent to Codex's copied plugin cache; verification relies solely on `claude plugin list --json` reporting the plugin installed and enabled. `--scope project` writes to the repo-tracked `.claude/settings.json`, so a fresh install creates or modifies a file that must be committed for the change to persist for collaborators. Codex must not install or update Claude Code, and Claude Code must not install or update Codex.
