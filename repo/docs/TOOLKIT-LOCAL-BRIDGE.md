...
Toolkit does not parse TOML structure with regular expressions. It validates original bytes with Python 3.11+ standard-library `tomllib`, asks the official Codex app-server `config/batchWrite` editor to produce a proposed file inside an isolated temporary `CODEX_HOME`, and validates the proposed bytes again before commitment. Exact integer values are required; float, string, boolean, malformed, duplicate, or structurally conflicting values are rejected. Explicit `limit` prints the exact target path and requested values, then defers commitment until managed checkout, plugin, validation, preference, and target operations succeed. Existing regular files receive a Toolkit-owned exact-byte backup with mode and integrity metadata; missing files receive restore metadata. Symlinks and special files fail closed. The final audit runs inside the bounded transaction, so failure restores the exact prior bytes or removes a newly created file. Setup reports the backup path and exact restore command. Official configuration docs explicitly confirm user config layers for Codex CLI and IDE extension; they do not separately state that the ChatGPT desktop app reads the same TOML, so desktop effectiveness remains unverified.
