...
Codex agents verify and refresh only the Codex native Toolkit plugin cache; Claude Code remains responsible for its own native cache.
...
5. Verify no active lock, recovery marker, displaced evidence, ownership sidecar, or new staging residue remains in the fixture. Exact logically retired lock artifacts may remain under their bounded quarantine names and are inert.
...
3. Reclaiming a marker left by a dead or stale recovery is itself atomic and identity-safe. The reclaimer must first exclusively create a tombstone named after a hash of the exact marker bytes it inspected; contenders that inspected the same marker generation compute the same tombstone path, so exactly one wins. The winner re-reads the marker under its tombstone and proceeds only when the bytes are still the inspected generation; any change means another process cycled the marker and the reclaimer yields without touching it. Exact spent tombstones and marker generations are moved into bounded logical-retirement quarantine and retained rather than deleted. A marker owned by a live process is never reclaimable.
...
8. Once the displaced owner is provably dead, the evidence is retired under recovery-marker ownership through the same exact-generation protocol as marker reclaim. The source is re-proved, atomically renamed to a unique bounded quarantine name without overwrite, its source-path absence is proved, and the moved bytes, semantic record, filesystem identity and parent authority are revalidated. No pathname deletion follows. `ENOENT` during pre-retirement verification means that generation genuinely vanished; every other read failure preserves the evidence and blocks. Retained quarantine is inert, excluded from cache-candidate identity, restart-adjudicable, and subject to a 64-object/512-MiB target-parent ceiling. Ceiling exhaustion fails closed with offline cleanup required.

Release removes a lock from the active namespace only when the current lock file still carries that run's ownership token: the exact file is atomically moved to retained logical retirement, while a lock replaced by another process is never moved or deleted. Marker release is equally token-guarded. Locks leave the active namespace in normal success and caught-failure paths; recovery from abrupt process termination relies on the dead-owner rule above, not on cleanup code running after a forced kill.
...
Exit code `23` is an intentional stop only when the managed child has emitted exactly one complete bank on stdout and the delegating parent has acknowledged that it forwarded the bank visibly. The parent launches the managed child before awaiting non-TTY stdin EOF and streams input as exact bytes under a fixed limit; the child emits and completes the bank before reading unresolved input. The child binds the bank to bounded marker/question counts, exact byte length, and SHA-256 on the private control pipe. The parent waits for that exact stdout payload under the overall timeout regardless of control/stdout delivery order, validates length, digest, and markers, forwards it once, and only then acknowledges visibility. It never retries a missing bank or forwards partial or wrong-stream output, and it reports no output, partial/misordered/duplicate markers, wrong stream, length/digest mismatch, bounded-input/output failure, invalid identity, child failure, timeout, or signal termination with privacy-safe diagnostics. In chat, the complete compact bank must be ordinary visible text in the same response as any recommended-default shortcut; hidden tool output and summaries do not count. Agents must not rerun with `--yes-recommended` unless the user explicitly requested the displayed recommended choices in the current turn. If the managed setup script exists but pauses or hits a real safety blocker, do not bypass it with the active repo fallback.
...
Toolkit supports distinct exact adapters for official `n8n-skills@n8n-io` versions `1.0.1` and `1.0.2`. The current `1.0.2` adapter is pinned to `n8n-io/skills` commit `eb18fc3ab3e2820c748c2d84386fb5496efc1516`. Upstream now installs the repository root and removed the former mirrored `plugins/n8n-skills` bundle; the exact manifest, hook, and entry meta-skill blobs used by the adapter remain byte-identical to reviewed commit `2c26822deb522ea2862d864b0c808b767a13aa9a`. The official package still invokes `.sh` hooks directly and lacks equivalent Windows Node fallback support. Therefore `1.0.2` remains `repair-required`; it is not treated as healthy merely because it is newer.
...
- Before preview or mutation, the selected Codex version, discovered cache-directory version, package-manifest version, and exact adapter version must all agree. Any mismatch is `selected-version-mismatch` and performs no staging, backup, marker, lock, or target write.
- The source-owned `compatibility-contract.json` is checked for exact parity with runtime package identity, current and supported versions, upstream commits, repair profiles, declared compatibility paths, current adapter choice, and report-only source-watch policy. The trusted workspace validator parses the target runtime's uniquely marked JSON metadata as inert data and never loads target-workspace JavaScript. Contract/runtime drift is a fail-closed validation and runtime barrier, not a second compatibility source.
- It skips the Toolkit native plugin cache so Toolkit updates remain source-owned by the managed checkout.
- On non-Windows platforms it returns `not-supported` before plugin-list inspection, cache discovery, repair-journal binding, or any filesystem mutation.
...
- It acquires an exclusive cross-process lock derived from the exact resolved selected cache before final reclassification. Separate Toolkit hubs targeting the same cache contend on the same lock, while different cache roots have distinct lock identities. A confirmed-live owner is never recovered by age, and an indeterminate owner remains a hard fail-closed barrier; only a proven-dead owner may enter exact automatic recovery. Numeric PID reuse can therefore retain target-lock contention until that PID is no longer alive. The supported recovery procedure is to stop all Toolkit and Codex maintenance processes, wait until the recorded PID is absent, and rerun maintenance so the existing exact dead-owner recovery adjudicates the lock. If an unrelated process continues to hold the reused PID, restart the host. Never delete a lock artifact manually, edit its token, or force recovery. Recovery uses the exact lock-generation token, rename, recovery-marker, and logical-retirement protocol and never signals the recorded process. A loser re-evaluates a winner's healthy result or fails without writing.
- One iterative, deterministically ordered bounded tree inspection derives both the full selected-tree identity and the exact critical `hooks/` file set. It enforces 4,096 files, 1,024 directories, 256 MiB total bytes, 64 MiB per file, and depth 32; rejects symbolic links, junctions, redirected directories and special entries; and fails `unsupported-layout` before lock acquisition or staging. Each regular file uses nanosecond stat identity where supported, two bounded hashes through the same open descriptor, and final descriptor/path/realpath rechecks. A synchronized same-size overwrite is therefore detected even when inode, size, and coarse timestamps remain unchanged.
- While holding that target lock, it stages only the exact selected cache through the existing owned-generation mechanism and revalidates immediately before replacement. Before copy, a target-untouched pre-transaction binds the exact normalized target, selected version and adapter/repair profile, authoritative compatibility-contract and source-lock bytes, source-watch identity, approval tree, planned repaired fingerprints, deterministic stage and backup paths, stage/target directory identities, generation/token, and owner. Append-only copied, transforming, and transformed evidence distinguishes incomplete staging from a registered replacement. A later SessionStart reruns parity and adjudicates any one exact dead generation under the same target lock before ordinary current-cache selection. Recovery mutation requires positive `installed === true` and `enabled === true` evidence from a matching Codex entry, or the same bounded config/cache selector used by ordinary reconciliation when the CLI omits the plugin. Config fallback requires explicit enablement plus exactly one ordinary retained candidate from the canonical direct-child inventory. The selected path, version, directory identity, package-parent identity, and inventory digest remain bound through rechecks immediately before recovery mutation, after obsolete restoration, before staging, and before target displacement. The exact validated transaction-owned stage, backup, evidence, and lock entries are the only exclusions; similarly named decoys, redirects at recorded paths, changed ownership/generation, and another transaction's artifacts remain ambiguous. Zero or multiple candidates, unsafe siblings, or inventory changes fail closed with all target, stage, backup, and transaction evidence unchanged. It removes only the exact owned target-untouched stage when the canonical original is unchanged, preserves a verified canonical winner, completes installation from the exact recorded stage, or restores only the exact identity-bound original backup after replacement registration. When the exact config-selected candidate or positive Codex identity selects another version, the obsolete transaction restores its exact historical original and retires only its owned evidence; config fallback then revalidates the previously bound current candidate and still-enabled config before current selection continues. Live, indeterminate, changed, malformed, ambiguous, conflicting, aliased, redirected, or foreign evidence fails closed; unrelated caches and stage/backup-like decoys are never scanned or changed.
- Recovery evidence is one immutable authority rather than a set of expected names. Descriptor-based inspection binds every present generation record, ready/completed/failed marker, pre-transaction record, replacement record, phase file, and nested stage-owner marker to its normalized path, evidence kind, required/optional status, generation/token, owner/schema/state, non-following type, real path, parent identity, device/inode identity, mode, link count, size, birth/modification/change times, exact byte SHA-256, and parsed semantic digest. Every absent optional path is bound as expected absence. The canonical cache inventory digest includes ordinary candidates, exact owned stage/backup roots, every present or absent evidence entry, and applicable lock evidence; an entry ignored for candidate counting still contributes the reason and identity that authorize that exclusion.
- A returned final-winner failure leaves an exact `failed` terminal marker inside that immutable evidence authority. A later call may classify only that validated failed generation as terminally owned even when the hosting Node PID remains alive; recovery still must acquire the exact target lock before adjudication. A live generation without that terminal marker, or any indeterminate or changed terminal evidence, remains fail closed.
- Schema-2 recovery authority is stored outside every mutable cache payload at `<codex-home>/.ai-agent-toolkit-n8n-repair/v2/targets/<sha256-normalized-target>/`. Each transition is one immutable framed segment: a 256-byte header, at most 512 KiB of canonical JSON, and a 96-byte commit trailer bind sequence, generation, ownership-token digest, target ID, previous record, payload, and complete-frame digests. A torn segment has no semantic authority. One primary and eight pre-authorized rescue slots are available; a successful rescue is a `TAIL_SEAL` that binds every incomplete predecessor's exact name, size, and raw SHA-256 while carrying the intended continuation. Accepted records are never truncated, rewritten, renamed, or reinterpreted. Mutation requires Node 22 or 24 and supported public exclusive-open, bounded write, file `fsync`, close, exact-path reopen, byte, identity, and topology checks. On POSIX, every admitted journal level revalidates the exact ordinary child and parent, `fsync`s the parent even when the child pre-existed, and proves both identities and topology unchanged afterward; this makes a restart re-establish a parent entry whose creating process stopped before its original directory flush. Read-only inspection performs no flush or creation. Windows retains only the file-flush guarantee supplied by Node and the operating system, not directory-`fsync`, controller-cache, or hardware persistence.
- Exact v1 evidence is migrated append-only into that schema-2 journal before target mutation. The v1 files remain authoritative until every present v1 phase is mirrored by exact byte, semantic, and filesystem-identity digests. A torn migration resumes through `TAIL_SEAL`; migration never deletes or rewrites v1 evidence. Phase order is mechanical: prepared, copied, transforming, transformed, registered, displacement intent/completion, installation intent, installed, verification intent/completion, cleanup authorization, business commitment, retirement intent, and logical retirement. Recovery that finds the exact installed winner with durable phase 30 must append authoritative phase 40 before verification, backup cleanup, retirement, or completion.
- `L20_LOGICALLY_RETIRED` is terminal transaction truth. Physical cleanup is a later exact-authority operation: a blocked cleanup remains visibly pending and cannot revoke the committed winner or original. Before removing anything, the journal appends `C10_CLEANUP_PENDING` with the exact L20 residue digest and any owned-stage identity. A restart may accept only monotonic absence after C10; every remaining file or stage must still match its prior identity and bytes. Deletion of a required phase, the owned stage, or the final v1 generation anchor is therefore restart-resumable. When the anchor is already gone, the target-scoped stable journal can complete the proof after ordinary exact target selection. `C20_CLEANUP_COMPLETE` is appended only when every bound v1 and stage residue is absent. Changed or substituted residue fails closed and is preserved.
- Alternating immutable `checkpoint-a`/`checkpoint-b` generations retain at most two active checkpoint authorities and chain a cumulative terminal root before any superseded checkpoint is retired. Each checkpoint is admitted only when its target ID, exact `checkpoint-(a|b)-<16-digit epoch>.jseg` filename, slot parity, frame family, frame/payload generation, ownership-token digest, previous digest, terminal state, and residue authority agree. Retired checkpoint names bind the exact source filename and activating checkpoint digest; foreign-target or mismatched checkpoint bytes are never ignored, relocated, or used for retention. The successor binds the exact superseded checkpoint and its terminal transaction segment manifest. A stop after checkpoint publication, checkpoint quarantine, transaction quarantine, any individual segment deletion, or removal of the empty `segments` directory resumes from that exact prefix. The final empty quarantine root is removable only while its exact name, identity, emptiness, absent source transaction, and unchanged successor checkpoint still agree; POSIX flushes the empty root before its removal and the transactions parent afterward. Changed, redirected, foreign, or unexpectedly populated residue is preserved and fails closed. Only C20-complete terminal transactions replaced by a later generation are compacted. Active/recovery authority, a current transaction's still-growing earlier checkpoint, and the one required rollback authority are never compacted. Locked limits are 128 active segments and 64 MiB per active transaction; 512 entries/512 MiB soft and 2,048 entries/1 GiB hard physical residue per target; 2,200 entries/1 GiB total target journal; 16 terminal entries/4 MiB and 2 MiB per checkpoint. Bounded usage inventory counts every child before classification and revalidates every directory identity, final name set, and file identity. Hard limits block new staging, not healthy read-only startup or required safety recovery. Age, count, and filename alone never authorize retirement.
- The complete authority is revalidated after parsing, after lock acquisition, before current/obsolete adjudication, before every target/stage/backup rename or deletion, after obsolete restoration, before staging and displacement, before backup-retirement authorization, before every resumable retirement operation, before each evidence retirement, and before completion. Legitimate phases advance one exclusive append-only transition at a time while all prior bytes remain exact; gaps, overwrites, unexpected phase-shaped files, and newly appeared optional markers fail closed. Main locks, recovery markers/claims/tombstones, displaced-lock evidence, displaced-evidence tombstones, existing lock quarantine and manifest-authorized backup roots use retained logical retirement: the exact source is atomically renamed without overwrite, source absence and the moved identity/bytes/topology are re-proved, and no pathname deletion follows. Retained objects are inert, excluded from cache selection and candidate digests, restart-adjudicable, and bounded to 64 objects and 512 MiB per target parent. A stable parent-derived capacity mutex serializes only the shared-parent capacity proof and exact rename, so distinct target locks cannot concurrently over-admit the same ceiling; its own retained generations live in a separately bounded coordination namespace outside cache selection. Ceiling exhaustion returns `offline-cleanup-required` and preserves all generations.
- Immediately before each destructive pathname operation, and again before every retry after a Windows sharing violation, the bridge revalidates the lock, package and stage parent topology, non-following directory identity, complete bounded target/stage/backup bytes, journal chain, v1 evidence authority, and destination absence. The final proof step is adjacent to the `rename` or removal with no callback or unrelated work between them; deterministic tests replace the rename operation itself when injecting a sharing violation. Any byte, identity, parent, redirect, or destination change is `destructive-boundary-drift` and performs no stale-authority displacement.
- Normal pre-verification failures roll back exactly. Before staging, a deterministic pre-displacement admission binds the exact approved original-tree digest, manifest serialization bytes, UTF-8 relative-path bytes, counts, identities, hashes, topology, retained-retirement name overhead and locked ceiling into the pre-transaction and journal. Limit-plus-one rejection occurs before stage or transaction registration and leaves the target exact. Installed verification precedes append-only backup-retirement authorization bound to that admission, the exact original backup and verified winner directory identities. Phase 70 performs exactly three complete healthy-winner proofs independent of residue count: before retirement, immediately before the whole-root rename, and after retirement. The exact manifest-authorized backup root is atomically renamed to its transaction-bound retained name and is never recursively removed in this repair. Drift at any boundary is `final-winner-drift`, never `repaired`, and preserves the exact transaction, backup retirement and evidence authority. A retained backup is inert and excluded from cache candidates and stable candidate digests. Offline cleanup is outside SessionStart and is permitted only under separate authorization after every Toolkit and Codex maintenance process is stopped and exact inactive authority is independently established. A full selected-tree digest binds every preserved byte to the approval and transaction, while the exact adapter fingerprints remain the compatibility decision.
- The current repair writes BOM-tolerant Node JSON fallbacks for Windows PowerShell 5.1 redirected stdin. Exact prior repaired fingerprints remain explicitly healthy and byte-idempotent; Toolkit never rewrites those recognized legacy repaired bytes merely to adopt the newer wrapper/parser shape.
- It fails closed on disabled, unsupported-version, unsupported-layout, partial-repair, malformed, ambiguous-target, identity-unverified, repair-failed, and verification-failed outcomes and leaves every unrelated cache file unchanged. Each exact adapter owns its pristine and repaired fingerprints; versions are never inferred by ordering.
- `healthy` and `repaired` attest only the declared Windows hook compatibility contract. They do not attest unrelated official skill-pack content, which remains outside this bounded repair and is preserved byte-for-byte. A full selected-tree digest binds those unrelated bytes to the approval and rollback transaction without broadening the compatibility adapter into a skill-pack installer or validator.
- Healthy SessionStart performs one complete bounded classification and hands its immutable result to a pure decision renderer. It does not perform the former second compatibility-tree walk. Lock acquisition, any mutation, or any recovery requirement invalidates that pre-lock result and requires fresh authority.
