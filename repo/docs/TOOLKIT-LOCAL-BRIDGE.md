...
- Update-report state: `last_update_report_path`, `update_report_enabled`, `update_report_open_enabled`, `update_report_retention_days`, max retained report count, and last cleanup status.
...
Release removes the lock only when the current lock file still carries that run's ownership token: a lock replaced by another process is never deleted, even by the process that previously owned that path. Marker release is equally token-guarded. Locks are released in normal success and caught-failure paths; recovery from abrupt process termination relies on the dead-owner rule above, not on cleanup code running after a forced kill. Older bridge versions refuse to overwrite newer hub state unless `--force-downgrade` is supplied for explicit manual recovery. In hook mode this skip is printed to stdout with the exact host-native refresh remediation. For a stale Claude Code plugin cache, the message steers users to `setup toolkit --host claude-code` or `setup toolkit` from Claude Code first, then restart Claude Code; if a raw native update is needed it includes `claude plugin update ai-agent-toolkit@ai-agent-toolkit-local --scope user` and tells users to rerun setup if the native update still reports stale. For a stale Codex plugin cache, the message points to `setup toolkit` in Codex.
...
Exit code `23` or a required-answer question-bank message is an intentional stop for user input. Agents must present the choices to the user, including in chat for non-interactive shells, and must not rerun with `--yes-recommended` unless the user explicitly requested recommended/default choices in the current turn. If the managed setup script exists but pauses or hits a real safety blocker, do not bypass it with the active repo fallback.
...
The question bank collects the managed checkout path, repo-backed auto-update, current host native cache behavior, meaningful update report writes, report auto-open, report/log retention days, Codex delegation control when Codex is the host, and OpenCode or Antigravity 2 target decisions up front. It must show current state and a recommended default for each question. Delegation control recommends `keep` pending native UAT; empty interactive input and `--yes-recommended` do not enable the limit. Only explicit `limit` proceeds to preview and transactional write of `agents.max_threads = 1` and `agents.max_depth = 1`, preserving one general direct-specialist slot and preventing recursive spawning. Codex Security ordinary/deep compatibility remains unverified pending native UAT. Keep current, refresh/sync, disable, and skip are distinct target answers; skip must not mean disable. Do not reintroduce later preference pauses. Verification after managed setup must use the managed checkout verifier, not an active stale verifier. Codex agents verify and refresh only the Codex native Toolkit plugin cache from the managed checkout. When Codex plugin cache auto-refresh is enabled on Windows, the trusted Toolkit Codex hook may also scan installed non-Toolkit Codex plugin caches under `CODEX_HOME/plugins/cache/**/**/<version>` and repair unsafe `.sh` hook launchers with the Toolkit Windows wrapper. Claude Code verifies `.claude-plugin/plugin.json` and `.claude-plugin/hooks/hooks.json`, then relies on Claude Code's own native plugin install/trust flow when the host reports the package is missing, stale, disabled, or untrusted. Final setup reports should state active worktree path/commit if inspected, managed checkout path/commit, exact setup script path executed, delegation enforcement and backup/restore status, and whether the question bank appeared. If an installed stale `toolkit-setup` skill says to run the full `repo/tests/toolkit-local-bridge.test.cjs` suite during routine setup, override it with root `AGENTS.md` and this document. Routine setup uses `repo/tests/toolkit-local-bridge-hook-light.test.cjs`; the full bridge suite is for bridge changes, PR review, or release validation.

Current official Codex configuration supports `[agents].max_threads` and `[agents].max_depth`. The root session is depth `0`; the thread cap applies to subagent threads, so the opt-in Toolkit values preserve one general direct-specialist slot and depth `1` prevents that specialist from spawning descendants. They are intended to support an explicitly invoked specialist. Codex Security ordinary/deep compatibility remains unverified pending native UAT; if official Security workflows require more capacity, a later reviewed change will handle that result explicitly rather than silently widening the limit.

Toolkit does not parse TOML structure with regular expressions. It snapshots original bytes, topology, mode, and available identity metadata with Python 3.11+ standard-library `tomllib` validation, asks the official Codex app-server `config/batchWrite` editor to produce a proposal inside an isolated temporary `CODEX_HOME`, then adds exact Toolkit markers around only `max_threads = 1` and `max_depth = 1` inside the real `[agents]` table. The final marked proposal is structurally parsed again before commitment. Exact integer values are required; float, string, boolean, malformed, duplicate, or structurally conflicting values are rejected. Explicit `limit` prints the exact target path, requested values, and backup directory, then defers commitment until managed checkout, plugin, validation, preference, target, and final bridge-audit operations succeed. The verified config commitment is the final fallible setup operation. A final local snapshot comparison aborts if a concurrent edit occurred. The replacement file receives its preserved mode before atomic rename, and any failure signalled after replacement is treated as committed so rollback cannot be skipped. Existing regular files receive a Toolkit-owned exact-byte backup with mode, identity, and integrity metadata; missing files receive restore metadata. Restore accepts only generation-local, non-symlinked metadata and backup paths for the expected current Codex config, validates schema, topology, modes, sizes, and hashes before mutation, and rechecks the full current snapshot immediately before deleting a config that Toolkit created from an originally missing file. Symlinks and special files fail closed. A failure inside the post-write transaction restores the exact prior bytes or removes a newly created file. Setup reports the backup path and exact restore command. Official configuration docs explicitly confirm user config layers for Codex CLI and IDE extension; they do not separately state that the ChatGPT desktop app reads the same TOML, so desktop effectiveness remains unverified.
