...
Fresh locks cause a safe skip. Stale locks are removed before retry. Older bridge versions refuse to overwrite newer hub state unless `--force-downgrade` is supplied for explicit manual recovery. In hook mode this skip is printed to stdout with the exact host-native refresh remediation, for example `claude plugin update ai-agent-toolkit@ai-agent-toolkit-local` (then restart Claude Code) for a stale Claude Code plugin cache, or `setup toolkit` in Codex for a stale Codex plugin cache.
