...
- While holding that target lock, it stages only the exact selected cache through the existing owned-generation mechanism and revalidates immediately before replacement. Before copy, a target-untouched pre-transaction binds the exact normalized target, selected version and adapter/repair profile, authoritative compatibility-contract and source-lock bytes, source-watch identity, approval tree, planned repaired fingerprints, deterministic stage and backup paths, stage/target directory identities, generation/token, and owner. Append-only copied, transforming, and transformed evidence distinguishes incomplete staging from a registered replacement. A later SessionStart reruns parity and adjudicates any one exact dead generation under the same target lock before ordinary current-cache selection: it removes only the exact owned target-untouched stage when the canonical original is unchanged, or it preserves a verified canonical winner, completes installation from the exact recorded stage, or restores only the exact identity-bound original backup after replacement registration. Live, indeterminate, changed, malformed, ambiguous, conflicting, aliased, redirected, or foreign evidence fails closed; unrelated caches and stage/backup-like decoys are never scanned or changed.
- Normal failures roll back exactly, installed verification precedes backup deletion, and successful completion removes only the exact owned stage, backup, transaction, phase, and lock evidence. A full selected-tree digest binds every preserved byte to the approval and transaction, while the exact adapter fingerprints remain the compatibility decision.
