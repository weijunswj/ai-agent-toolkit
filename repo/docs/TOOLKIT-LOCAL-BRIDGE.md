...
Codex cache setup is a separate narrowly delegated native child. Its canonical `--delegated-invocation-authority` capsule binds the repository, Codex home, setup source, Node executable, package version, environment, and exact effect ceiling. The child independently re-admits fixed Codex CLI operations, SessionStart publications, and TOML parser launches. Windows n8n hook reconciliation supplies a pure repair plan; the Bridge applies each planned file itself. Staging helpers likewise provide inspection/plans to Bridge-owned effects. Standalone compatibility writers remain outside the managed Bridge route.
