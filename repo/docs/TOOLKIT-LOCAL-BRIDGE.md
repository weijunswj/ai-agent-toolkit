...
- It stages only the exact selected cache through the existing owned-generation mechanism. A full selected-tree digest binds every preserved byte to the approval and immediate pre-replacement revalidation, while the exact adapter fingerprints remain the compatibility decision. Symbolic links, junctions, and other redirected or special tree entries fail closed before copying. Installed verification proves both the repaired compatibility fingerprints and byte identity for every non-transformed file before deleting the backup; failure restores the exact original directory.
