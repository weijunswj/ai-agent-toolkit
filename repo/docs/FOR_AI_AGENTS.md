...
The recommended Codex outcome is root work plus at most one justified helper. Root-only remains available; unsafe existing capacity has a visible risk label; unknown or unsupported runtime detection cannot mutate configuration. User-owned effective controls that differ from the selected safe outcome require an exact technical proposal and one `apply` confirmation before the supported transactional editor runs; structurally complete controls already at that outcome are a byte-preserving no-op. Recursion is reported as hard-enforced only for a runtime with a supported hard control; current V2 guidance is policy-only, while concurrent excess work is queued or rejected by configured capacity subject to post-merge native UAT. Codex Security never raises normal capacity automatically, and unsupported Deep Security capacity requires an explicit alternative or elevate-run-restore-restart procedure.
