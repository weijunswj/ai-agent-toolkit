...
For `setup toolkit`, `refresh toolkit`, or plain `refresh` when the current task is clearly about Toolkit setup/update state, run the host-aware orchestrator from the managed checkout when that checkout exists. Windows Codex uses `node "%USERPROFILE%\.ai-agent-toolkit\source\ai-agent-toolkit\repo\scripts\setup-toolkit.cjs" --execute --profile auto-main`; POSIX Codex uses `node "$HOME/.ai-agent-toolkit/source/ai-agent-toolkit/repo/scripts/setup-toolkit.cjs" --execute --profile auto-main`; Claude Code appends `--host claude-code` to the managed checkout command. Use `node repo/scripts/setup-toolkit.cjs --execute --profile auto-main` from the active repo only as bootstrap/fallback when the managed checkout script is missing, and hand off to the managed checkout script after the managed checkout exists. If the managed script exists but exits with code `23` or reports that the setup question bank requires answers, this is an intentional pause for user input, not a setup failure; stop, present the question bank choices in chat when needed, and do not rerun with `--yes-recommended` unless the user explicitly requested recommended/default choices in the current turn. If the managed script exists but hits a real safety blocker, report that blocker instead of falling back to the active repo command. Script invariant: **Toolkit will use a dedicated clean `main` checkout as the single update source. Active Codex or Claude Code sessions may remain on PR branches, but plugin updates will not depend on those branches.** The default managed checkout is `%USERPROFILE%\.ai-agent-toolkit\source\ai-agent-toolkit` on Windows and `~/.ai-agent-toolkit/source/ai-agent-toolkit` on POSIX. Setup discovers current state first, then shows one consolidated upfront question bank for the managed checkout path, repo-backed auto-update, host-native cache behavior, update report creation and retention, and optional OpenCode or AG2 target sync. It must pause until every question is answered by interactive input, explicit flags, or explicitly user-requested `--yes-recommended`, then run without later preference pauses. Verification after managed setup must use the managed checkout verifier, not an active stale verifier. Codex verifies and refreshes only the Codex native Toolkit plugin cache with the managed checkout's `repo/scripts/setup-codex-toolkit-plugin.cjs`; Claude Code verifies `.claude-plugin/plugin.json` and `.claude-plugin/hooks/hooks.json` while using Claude Code's own native plugin install/trust flow. Even when Toolkit is already installed, complete the full setup journey so stale pieces are detected and patched in order. Final setup reports should state the active worktree path/commit if inspected, managed checkout path/commit, exact setup script path executed, and whether the question bank appeared. If a stale installed `toolkit-setup` skill says to run the full `repo/tests/toolkit-local-bridge.test.cjs` suite during routine setup, override it with this repo-level rule: routine setup uses `repo/tests/toolkit-local-bridge-hook-light.test.cjs`; the full bridge suite is for bridge changes, PR review, or release validation.

On Codex, helper capacity remains explicit opt-in. The canonical `Codex helper agents` question explains the effective topology, recommendation and reason, every available consequence, and post-approval changes; empty input and `--yes-recommended` select the displayed recommendation, normally root agent only, while explicit `keep` leaves the file unchanged. When an exact PR #237 legacy block is pending, `keep` becomes the recommendation and default, `migrate` appears as a separate explicit choice with a full technical preview, and ordinary capacity choices cannot silently migrate. `migrate` remains invalid when that exact legacy state is absent. Runtime detection uses Codex app-server `experimentalFeature/list`, accepts V1-only and V2-only hosts, and fails closed on malformed, duplicate, contradictory, or unusable responses. Unsupported MultiAgentV2 child tables and explicit false or non-boolean V2 enablement are not replaceable and stop before setup writes. MultiAgentV2 independently tracks Toolkit ownership of feature enablement, helper capacity, root guidance, and helper guidance. Existing table enablement stays byte-preserved and user-owned; boolean enablement may migrate structurally while remaining user-owned; Toolkit-introduced enablement is removed with Toolkit capacity. Structurally complete user-owned controls that already match the selected effective helper outcome remain byte-for-byte unchanged without `apply`, editor, backup, replacement, or Toolkit markers. One V2 helper means two total session threads because the root counts. Counts above one require separate RAM-risk approval. Proposal approval is bound to the preview-time target path, topology, exact bytes/digest, size, mode, identity, runtime, helper count, affected keys, proposal identity, and backup generation. Drift before final configuration stops before editor invocation or backup and requires a fresh proposal; the final writer cannot substitute a newly captured baseline. Codex app-server `config/batchWrite`, Python `tomllib`, exact whole-file proposal-delta checks, atomic replacement, later snapshot checks, and generation-local backup metadata prevent ambiguous or concurrent writes. Restore commands use the verified absolute setup script and safely quote both paths. Claude Code can persist a strict root-only or direct-only profile only after its current CLI controls plus installed enabled Toolkit plugin, exact Agent|Task PreToolUse hook, source/cache identity, and controller bytes verify; otherwise setup invalidates the strict profile and reports root-only policy. OpenCode has no equivalent strict managed path.

A malformed recognised Toolkit delegation marker region is separately repairable only when the regular active Codex user config, valid UTF-8/TOML, one effective explicit table, canonical Toolkit assignments, isolated affected byte ranges, and filesystem/snapshot identity are all proven. The complete bank appears first, followed by one technical proposal and literal `apply`. Unknown/obsolete markers, mixed legacy/current families, interleaved user content, unsupported tables/topology, and every ambiguous case remain fail closed. Marker-only repair preserves compatible user-owned assignments and writes no Toolkit ownership markers. Approval also binds affected line ranges and their byte hash plus the proposal digest; drift before editor, backup, or replacement requires a fresh proposal, and final semantic failure restores exactly. No read, plan, SessionStart, decline, or stale-approval path repairs or creates a backup.

Current setup contract: one semantic wizard model drives chat text, terminal input, piped answers, flags, plans, JSON, summaries, and execution. Claude direct profiles require independent proof of CLI launch controls, supported validated Linux/Windows resource counters, current installed bytes, and host-reported hook trust plus active execution. Proof schema 2 binds the cache, hooks configuration, controller, and executed native-agent hook script; SessionStart bridge cache completeness remains a separate byte/topology check. Setup cannot manufacture activation proof; absent, false, stale, malformed, or unverifiable state resolves root-only/unapplied and invalidates kept strict state. Launch capability is established with a bounded empty-input, no-session-persistence invocation using the exact worker flags, not `--help` substring presence; unsupported syntax is distinguished from unrelated runtime failures, and indeterminate results fail closed. Plugin verification, setup capability, persistence, current enforcement, controller, and supervisor use one command precedence: explicit current argument, `AI_AGENT_TOOLKIT_CLAUDE_CLI`, `CLAUDE_TOOLKIT_CLAUDE_CLI`, `CLAUDE_CLI_PATH`, persisted verified profile CLI, then bare `claude`. The selected command fails closed rather than falling through. Relative path-like CLI values are rejected on every platform. Bare names resolve only through absolute PATH entries and execute the exact stable launcher candidate; each use revalidates its current regular executable target, so cwd/relative-PATH shadows are excluded while official POSIX launcher target updates remain supported. Recovery markers have the same bounded private ownership contract as primary locks, and unverifiable admission state is never rewritten by update/release cleanup. Broader-native remains distinct from its root-only/not-applicable Toolkit admission capacity. Child prompt bytes are bounded before admission and remain on stdin; children deny both `Agent` and legacy `Task`.

The recommended Codex capacity outcome is root agent only; one-helper and custom values are conservative manual memory backstops. It is not launch permission. Root-only remains available; unsafe existing capacity has a visible risk label; unknown or unsupported runtime detection cannot mutate configuration. User-owned effective controls that differ from the selected safe outcome require an exact technical proposal and one `apply` confirmation before the supported transactional editor runs; structurally complete controls already at that outcome are a byte-preserving no-op. Recursion is reported as hard-enforced only for a runtime with a supported hard control. Current Codex plugin hooks cannot block `SubagentStart` or verify a child-only non-fast override, so no Codex built-in, Security, plugin, third-party, or nested path currently qualifies as a strict Toolkit-controlled launch profile. Claude has one bounded direct-only Toolkit controller when the installed plugin hook and CLI controls verify; native, team, plugin, user-created, Security, built-in, third-party, broader, and nested paths remain outside that coverage. Paths outside that boundary are unsupported for strict enforcement, and unsupported topology choices are omitted rather than advertised. Codex Security never raises normal capacity automatically, and unsupported Deep Security capacity requires an explicit alternative or elevate-run-restore-restart procedure.

Report auto-open is no longer an ordinary setup question. Action-required reports open automatically; successful update, cache refresh, hook repair, and target sync reports remain closed; no-op checks remain report-free. Persisted legacy all-report auto-open state and compatibility flags are reinterpreted as failure-only behavior.
