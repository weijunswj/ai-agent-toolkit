...
On Codex, helper capacity remains explicit opt-in. One canonical question asks "How many helper agents may Codex use at the same time?" and drives displayed rows, prompts, piped input, flags, recommendations, plans, and execution; the recommendation, empty input, and `--yes-recommended` all select `keep`. Runtime detection uses Codex app-server `experimentalFeature/list`: MultiAgentV2 configuration uses a `[features.multi_agent_v2]` table with `enabled = true` and `max_concurrent_threads_per_session`; the official boolean enablement form may migrate to that table. MultiAgentV1 uses `[agents].max_threads` and `[agents].max_depth`. RAM-safe V2 mode allows one helper with two total session threads because the root counts toward the total. Custom counts above one require a separate RAM-risk approval. Codex app-server `config/batchWrite` prepares the proposal in an isolated temporary `CODEX_HOME`; Python `tomllib` structural validation, exact Toolkit markers, exact proposal-delta checks, atomic replacement, final snapshot checks, and generation-local backup metadata prevent ambiguous or concurrent writes. The exact PR #237 Toolkit-managed V1 block may migrate to V2; user-owned and malformed blocks remain untouched. V2 helper guidance prohibits recursive helper spawning by policy, but no native hard block has been verified. No documented scan-scoped Codex Security capacity activation exists, so setup never raises normal global capacity automatically; alternatives are lower-capacity ordinary or sequential review, Deep Scan on another machine, or an explicitly approved temporary global increase with backup, restart, restoration, and another restart. Claude Code and OpenCode receive portable single-agent policy only and report host-level enforcement as unsupported.
