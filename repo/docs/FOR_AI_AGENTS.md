...
This repo is organized for AI-agent reuse. Prefer the direct canonical `skills/**` and `repo/**` surfaces over assumptions.
...
The Toolkit package version uses `semver`. Do not use Git tags, package tags, GitHub release tags, upstream versions, or per-file versions as substitutes for the package version.
...
- `repo-methodology/`: source-preserving skill surfaces, audit strategy, and anti-drift methodology.
...
| `github-program-reconciler` | Mandatory managed-programme lifecycle inspection/preflight plus deterministic preview-bound Parent/Child/PR reconciliation, native relationships, truthful exact-head review evidence, and Deferred Findings; implicit discovery grants no mutation or finality. |
...
- When adding, removing, renaming, or materially changing a skill under `skills/**`, update `repo/contracts/agent-rules/toolkit-skill-routing.md`.
- When changing skill names, `SKILL.md` frontmatter, or descriptions, update README skill tables when applicable, the routing source, and portable rule templates.
...
The canonical controller admits every Toolkit-managed child only after atomic live memory reservation and verified host responsiveness. Existing reservations reduce headroom; failed launch and completion release reservations; bounded identity-safe stale recovery handles abnormal termination; CPU cannot override insufficient RAM. Ordinary workers additionally require a bounded declaration of separable concurrent work, concrete expected wall-clock speedup, the root retaining and immediately continuing the longest/critical-path task, and a shorter/easier child task. Claude has one enforceable direct path. Codex and OpenCode adapters must invoke the same controller or remain root-only, and no Markdown-only path is called hard enforcement. Worker/checker mappings are Codex and OpenCode GPT-5.6 Sol Medium, Claude worker Fable 5 and checker Opus 4.8; children never use Fast mode or nest. After meaningful validated code changes, exactly one read-only bounded pre-PR checker is required unless the deterministic trivial classifier returns `SKIPPED_TRIVIAL`. The checker is the independent-verification exception to worker speedup fields and uses the supported stdin factory/launch plus result-retrieval workflow. Results are `PASS`, `FINDINGS`, `ADMISSION_DENIED`, or `SKIPPED_TRIVIAL`; denied admission requires a reported root self-review and is not an independent pass.
