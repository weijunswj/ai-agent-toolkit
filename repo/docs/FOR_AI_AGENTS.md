...
The recommended Codex capacity outcome is at most one helper as a conservative memory backstop. It is not launch permission. Root-only remains available; unsafe existing capacity has a visible risk label; unknown or unsupported runtime detection cannot mutate configuration. User-owned effective controls that differ from the selected safe outcome require an exact technical proposal and one `apply` confirmation before the supported transactional editor runs; structurally complete controls already at that outcome are a byte-preserving no-op. Recursion is reported as hard-enforced only for a runtime with a supported hard control. Current Codex plugin hooks cannot block `SubagentStart` or verify a child-only non-fast override, so no Codex built-in, Security, plugin, third-party, or nested path currently qualifies as a strict Toolkit-controlled launch profile. Claude plugin agents can declare effort and tool restrictions, but lower-precedence definitions and session/environment overrides prevent a global strict enforcement claim. Unsupported topology choices are omitted rather than advertised. Codex Security never raises normal capacity automatically, and unsupported Deep Security capacity requires an explicit alternative or elevate-run-restore-restart procedure.
