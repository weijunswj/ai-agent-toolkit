...
The canonical controller admits every Toolkit-managed child only after atomic live memory reservation and verified host responsiveness. Existing reservations reduce headroom; failed launch and completion release reservations; bounded identity-safe stale recovery handles abnormal termination; CPU cannot override insufficient RAM. Claude has one enforceable direct path. Codex and OpenCode adapters must invoke the same controller or remain root-only, and no Markdown-only path is called hard enforcement. Worker/checker mappings are Codex and OpenCode GPT-5.6 Sol Medium, Claude worker Fable 5 and checker Opus 4.8; children never use Fast mode or nest. After meaningful validated code changes, exactly one read-only bounded pre-PR checker is required unless the deterministic trivial classifier returns `SKIPPED_TRIVIAL`. Results are `PASS`, `FINDINGS`, `ADMISSION_DENIED`, or `SKIPPED_TRIVIAL`; denied admission requires a reported root self-review and is not an independent pass.
