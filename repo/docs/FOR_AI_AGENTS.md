...
On Codex, delegation control is explicit opt-in pending native UAT. The recommendation is `keep`; empty interactive input and `--yes-recommended` leave `CODEX_HOME/config.toml` unchanged. Only an explicit `limit` selection previews and prepares `agents.max_threads = 1` and `agents.max_depth = 1`. Setup uses Codex app-server `config/batchWrite` against an isolated temporary `CODEX_HOME`, validates original and proposed bytes with Python `tomllib`, then backs up and transactionally commits the exact validated result after prior setup work succeeds. Unsupported topology, invalid or ambiguous TOML, conflicting values, or unavailable parser/editor support fail closed without writing. The values preserve one general direct-specialist slot by documented thread semantics and prevent recursive spawning; Codex Security ordinary/deep compatibility remains unverified pending native UAT. Claude Code and OpenCode receive portable single-agent policy only and report host-level enforcement as unsupported.
