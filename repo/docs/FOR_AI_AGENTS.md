...
On Codex, the setup question bank offers a recommended delegation-control choice. Empty interactive input selects the displayed recommendation. The supported limit is `agents.max_threads = 1` and `agents.max_depth = 1` in a Toolkit-marked block under `[agents]` in `CODEX_HOME/config.toml`; setup preserves unrelated content, does not overwrite conflicting user-owned limits, and reports the resulting configured, skipped, or conflicting state. This leaves one direct slot for an explicitly invoked specialist, including a security specialist or Codex Security workflow, and prevents recursive spawning. Claude Code and OpenCode receive portable single-agent policy only and report host-level enforcement as unsupported.
