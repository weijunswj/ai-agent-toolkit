...
Current setup contract: one semantic wizard model drives chat text, terminal input, piped answers, flags, plans, JSON, summaries, and execution. Chat hosts must surface the complete compact bank in the same visible response as any recommended-default shortcut; hidden tool output and summaries do not count, and a missing render is retried before setup fails closed. Unexpected extra non-empty piped answers are rejected for every host before setup mutation; whitespace-only trailing input is ignored. The primary sections are `Automatic updates`, `Computer performance`, and `Other coding apps`, with irrelevant app rows omitted. Ordinary text excludes raw paths, runtime names, TOML keys, ownership, and restore mechanics; pending exact legacy migration is the only case that visibly adds a `migrate` choice.
