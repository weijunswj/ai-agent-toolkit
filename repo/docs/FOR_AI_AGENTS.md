...
On Codex, delegation control remains explicit opt-in until native UAT is recorded. One canonical question specification drives displayed rows, prompts, piped input, flags, recommendations, plans, and execution; the recommendation, empty input, and `--yes-recommended` all select `keep`. Only explicit interactive `limit` or `--codex-delegation-control limit` may preview, back up, and write `agents.max_threads = 1` and `agents.max_depth = 1` in `CODEX_HOME/config.toml`. Codex app-server `config/batchWrite` prepares the proposed TOML in an isolated temporary `CODEX_HOME`; structural inspection uses standard-library Python `tomllib`, then exact Toolkit markers are added around only those two values in the real `[agents]` table and parsed again before commitment. Plugin, validation, preference, target, and final bridge-audit operations must succeed before config commitment, which is the final fallible setup operation. A final local snapshot comparison prevents stale proposals from overwriting concurrent edits. Restore metadata is generation-local and validates paths, topology, modes, and hashes before mutation; an originally missing config is fully rechecked immediately before restore deletes the Toolkit-created replacement. Unsupported topology or ambiguous TOML fails closed. Documented thread semantics preserve one general direct-specialist slot and depth one prevents recursive spawning. The limit is intended to support an explicitly invoked specialist. Codex Security ordinary/deep compatibility remains unverified pending native UAT; any future capacity change must be explicit rather than silently widening the limit. Claude Code and OpenCode receive portable single-agent policy only and report host-level enforcement as unsupported.
