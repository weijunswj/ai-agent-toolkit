...
On Codex, helper capacity remains explicit opt-in. The canonical question asks "How many helper agents may Codex use?"; empty input and `--yes-recommended` select the displayed recommendation, normally one helper at most, while explicit `keep` leaves the file unchanged. Exact PR #237 legacy migration remains a separate explicit choice with a full technical preview. Runtime detection uses Codex app-server `experimentalFeature/list`, accepts V1-only and V2-only hosts, and fails closed on malformed, duplicate, contradictory, or unusable responses. MultiAgentV2 independently tracks Toolkit ownership of feature enablement, helper capacity, root guidance, and helper guidance. Existing table enablement stays byte-preserved and user-owned; boolean enablement may migrate structurally while remaining user-owned; Toolkit-introduced enablement is removed with Toolkit capacity. One V2 helper means two total session threads because the root counts. Counts above one require separate RAM-risk approval. Proposal approval is bound to the preview-time target path, topology, exact bytes/digest, size, mode, identity, runtime, helper count, affected keys, proposal identity, and backup generation. Drift before final configuration stops before editor invocation or backup and requires a fresh proposal; the final writer cannot substitute a newly captured baseline. Codex app-server `config/batchWrite`, Python `tomllib`, exact whole-file proposal-delta checks, atomic replacement, later snapshot checks, and generation-local backup metadata prevent ambiguous or concurrent writes. Restore commands use the verified absolute setup script and safely quote both paths. Claude Code and OpenCode receive portable single-agent policy only and report host-level enforcement as unsupported.

Current setup contract: one semantic wizard model drives chat text, terminal input, piped answers, flags, plans, JSON, summaries, and execution. Chat hosts must surface the complete compact bank in the same visible response as any recommended-default shortcut; hidden tool output and summaries do not count, and a missing render is retried before setup fails closed. The primary sections are `Automatic updates`, `Computer performance`, and `Other coding apps`, with irrelevant app rows omitted. Ordinary text excludes raw paths, runtime names, TOML keys, ownership, migration, and restore mechanics.

The recommended Codex outcome is root work plus at most one justified helper. Root-only remains available; unsafe existing capacity has a visible risk label; unknown or unsupported runtime detection cannot mutate configuration. User-owned effective controls require an exact technical proposal and one `apply` confirmation before the supported transactional editor runs. Recursion is reported as hard-enforced only for a runtime with a supported hard control; current V2 guidance is policy-only, while concurrent excess work is queued or rejected by configured capacity subject to post-merge native UAT. Codex Security never raises normal capacity automatically, and unsupported Deep Security capacity requires an explicit alternative or elevate-run-restore-restart procedure.
