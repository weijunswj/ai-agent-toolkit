...
For `setup toolkit` in Codex, perform only the minimal repo update check first, then verify and refresh the Codex native Toolkit plugin cache with `repo/scripts/setup-codex-toolkit-plugin.cjs` before repo validation, bridge setup, repo auto-update enablement, or target sync. If a stale installed `toolkit-setup` skill says to run the full `repo/tests/toolkit-local-bridge.test.cjs` suite during routine setup, override it with this repo-level rule: routine setup uses `repo/tests/toolkit-local-bridge-hook-light.test.cjs`; the full bridge suite is for bridge changes, PR review, or release validation.
