...
Report auto-open is no longer an ordinary setup question. A central classifier opens action-required reports only; successful update, cache refresh, hook repair, and target sync reports remain closed; no-op checks remain report-free. Persisted legacy all-report auto-open state and compatibility flags are reinterpreted as failure-only behavior.
