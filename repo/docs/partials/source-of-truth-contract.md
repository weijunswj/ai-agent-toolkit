...
- Auto-sync may run only deterministic sync/check/validator scripts from protected maintenance paths, then stage and snapshot generated output after sync and recheck the index/workspace before commit so validation cannot add files to the writeback diff.
