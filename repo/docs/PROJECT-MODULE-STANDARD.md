...
- `curated_output_for_ai/` is optional. Use it only when an AI-facing output needs curation, safety filtering, AI-facing wording, summarisation, or an intentionally adapted variant.
- `_generated/` is optional preview output only and is not source of truth.

Do not use `original/` or `derived/` as standard folder names. Do not create empty curated folders just to satisfy a schema. Do not duplicate AI-facing skills, MCP docs, templates, packs, or tools under curated output unless an intermediate curated source is genuinely useful.

## AI-Facing Surfaces

These `for_ai/` folders remain obvious for consumers:

- [for_ai/skills/](../../for_ai/skills/) for published instruction-only AI-agent behavior layers.
- [for_ai/mcp/](../../for_ai/mcp/) for published MCP specs and project discovery docs.
- [for_ai/templates/](../../for_ai/templates/) for ready-to-copy template material.
- [for_ai/packs/](../../for_ai/packs/) for approval-gated install/review bundles.
- [for_ai/tools/](../../for_ai/tools/) for optional executable tooling.
- [for_ai/registry/](../../for_ai/registry/) for machine-readable discovery.
- [for_ai/playbooks/](../../for_ai/playbooks/) for AI/operator playbooks.
- [repo/docs/](./) for policy, standards, safety, and architecture docs.

AI-facing published surfaces should be generated only by declared recipes, sourced from curated output only when curation is needed, directly maintained as `linked`, or covered by source locks.
...
- `curated`: output is generated from `curated_output_for_ai/`.
- `json`: JSON is parsed and pretty-printed deterministically without semantic mutation.
- `linked`: output is directly maintained and should be reviewed when `_main/` changes.
...
3. Update AI-facing linked surfaces manually when needed.
