...
AI-facing published surfaces should be generated by declared recipes whenever practical. Use curated output for reviewed AI-facing source material, direct `_main/` recipes for preserved source copies, and `linked` only for rare root surfaces that are toolkit-local or otherwise cannot reasonably be generated.
...
4. Update `SOURCE-LOCK.json` with exact, adapted, or excluded entries when preserved or source-locked files change.
5. Update rare linked surfaces manually only when their manifest note justifies direct maintenance.
6. Run:
