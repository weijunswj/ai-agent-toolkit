...
The structured result reports `.gitignore`, `.git/info/exclude`, both, or not-covered for each folder. Public API results, nested previews, CLI output, and routine errors use only those stable file labels, canonical or legacy backup-folder labels, and a non-reversible repository identity digest. The exact repository root and resolved Git metadata paths remain private operational bindings used only for repository identity, target selection, digest-bound approval, stale-approval detection, and the requested mutation. They are never returned or printed. A broader pattern counts only when Git demonstrates that it covers the exact folder probe; the reported evidence identifies it as broader coverage. Toolkit never adds a broader pattern.
...
Each target uses same-directory rename transactions. Immediately before displacing each target, restore re-resolves its safe repository path and real parent topology and revalidates its expected existence or missing state, regular-file type, size, and checksum. If a prepared target drifts, restore leaves that target's new entry and bytes untouched, reverses every earlier target mutation exactly, removes private preparation residue, retains the completed backup, and returns non-success. For a multi-file restore, the current replacement is held in a private rollback sibling until all targets succeed. An injected partial failure reverses already-applied targets to their exact pre-restore bytes and fails visibly. Checksum mismatch or uncertain topology fails closed. Restore never touches unrelated files and never removes the completed backup.
