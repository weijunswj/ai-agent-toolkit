...
10. Answer the consolidated setup question bank before setup writes preferences or bridge targets. Delegation control recommends `keep` pending native UAT, so Enter and `--yes-recommended` leave Codex config unchanged. Only explicit `limit` previews the exact `agents.max_threads = 1` and `agents.max_depth = 1` values and path, uses the official Codex app-server editor in an isolated temporary home, validates TOML structurally, creates a restorable backup, and commits transactionally. Claude Code and OpenCode receive portable policy only. Use explicit flags or `--yes-recommended` for non-interactive runs only when you explicitly want the displayed defaults. Final setup reports should state the active worktree path/commit if inspected, managed checkout path/commit, exact setup script path executed, delegation enforcement and backup/restore status, and whether the question bank appeared.
