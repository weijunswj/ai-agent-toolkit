...
3. Setup is handled by the root agent alone. It shows the managed checkout path and all preferences up front; it must not spawn subagents to inspect instructions, docs, state, choices, or validation output. If it exits with code `23` or says the question bank requires answers, the agent must stop and ask you for those choices; do not rerun with `--yes-recommended` unless the user explicitly asks for recommended/default choices.
...
10. Answer the consolidated setup question bank before setup writes preferences or bridge targets. One canonical choice model drives rows, prompts, piped input, flags, recommendations, plans, execution, and summaries. For "How many helper agents may Codex use at the same time?", `keep`, empty input, and `--yes-recommended` leave the config unchanged; exact PR #237 legacy migration is a separate explicit choice. RAM-safe mode allows one helper; under MultiAgentV2 this writes two total session threads because the root counts toward the total. Custom counts above one require separate RAM-risk approval. Setup detects V2-only and V1-only effective hosts through app-server `experimentalFeature/list`; malformed, duplicate, contradictory, or unusable responses remain unknown. MultiAgentV2 tracks Toolkit ownership of enablement, capacity, root guidance, and helper guidance independently. Existing table enablement stays byte-preserved and user-owned; boolean enablement may migrate structurally but stays user-owned and survives removal. Toolkit-introduced enablement is removed with Toolkit capacity. App-server `config/batchWrite`, `tomllib`, exact marker ownership, proposal-delta checks, atomic commitment, concurrent-snapshot checks, and exact backup/restore metadata protect the config. Pre-write previews include target, runtime, before/after semantics, proposed blocks, planned backup metadata, and exact restore commands bound to the verified absolute setup script. V2 recursive-helper prevention is policy-only. Ordinary work remains root-only by default with at most one justified helper; explicitly invoked defined multi-worker workflows require a stated worker plan and explicit higher-capacity approval. Codex Security never raises normal global capacity automatically, and setup does not imply that official Deep Scan works under insufficient capacity. Claude Code and OpenCode receive portable policy only.
