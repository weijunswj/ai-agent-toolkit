...
3. Setup is handled by the root agent alone and must not spawn subagents for instructions, state, choices, or validation. In chat, the complete compact bank must be visible in the same response as any recommended-default shortcut; hidden command output or a summary does not count. Managed delegation uses one execution with an explicit child/parent visibility acknowledgement; missing, partial, duplicate, misordered, or wrong-stream bank output blocks approval and writes without retry. Do not rerun with `--yes-recommended` unless the user explicitly accepts the displayed recommendations.
...
- [External System Router](../../skills/external-system-router/) contains the cross-provider risk tiers, task envelope, operation-specific interface audit, universal graphical disclosure, local registry/ledger/receipt schemas, and deterministic drift helper. It is not a generic executor.
...
Generated template outputs are intentional. `node repo/scripts/sync-agent-instruction-shims.cjs --write` regenerates project-pure source-side baseline templates and root instruction shims from `_projects/development/ai-coding-agent-rules/_main/`; `node repo/scripts/sync-toolkit-projects.cjs --write` publishes inert skill copies, including `external-system-router` and `n8n-agent-rules`.

Portable baseline templates do not include n8n by default. Reconcile repository evidence and current intent first. When n8n workflow ownership or current n8n intent is proven, recommend an owner-approved compact `AI-AGENT-TOOLKIT:N8N` domain block that routes to `n8n-agent-rules`; preserve all unmarked content. Recommend removal when only historical n8n text remains, but never remove a marker silently. Official n8n Skills and live n8n MCP are separate capabilities, and installing Skills never installs or implies MCP.

External-system work uses one ignored user-local registry keyed by `provider + target alias + environment`, with credential references rather than values. Default to one appropriately scoped credential per target; do not guess among multiple targets from tabs, history, recent credentials, environment-variable order, or generic tokens. Generic Toolkit setup installs no provider integration. Explicit provider intent starts progressive reconciliation, and every required capability must end configured/verified, deferred with a blocker, or proven unnecessary before completion.
