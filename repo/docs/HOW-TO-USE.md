...
10. Answer the consolidated setup question bank before setup writes preferences or bridge targets. One canonical question order drives rows, prompts, piped input, flags, recommendations, plans, and execution. For Codex delegation control, the recommendation, empty input, and `--yes-recommended` all select `keep` until native UAT is recorded. Only explicit interactive `limit` or `--codex-delegation-control limit` may preview, exactly back up, and write `agents.max_threads = 1` and `agents.max_depth = 1`; unsupported or conflicting TOML remains untouched. Codex app-server `config/batchWrite` prepares the isolated proposal, then `tomllib` validates it, exact Toolkit markers are added around only the two values in the real `[agents]` table, and the final marked bytes are parsed again before atomic commitment. A final snapshot comparison protects concurrent edits. Restore metadata validates its generation-local paths, topology, modes, and hashes before mutation. The values preserve one general direct-specialist slot by documented thread semantics and prevent recursive spawning; Codex Security ordinary/deep compatibility remains unverified pending native UAT. Claude Code and OpenCode receive portable policy only. Use explicit flags or `--yes-recommended` for non-interactive runs only when you explicitly want recommended/default choices. Final setup reports should state the active worktree path/commit if inspected, managed checkout path/commit, exact setup script path executed, delegation enforcement status, and whether the question bank appeared.
