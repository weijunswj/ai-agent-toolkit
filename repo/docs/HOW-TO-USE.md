...
10. Answer the consolidated setup question bank before setup writes preferences or bridge targets. One canonical question order drives rows, prompts, piped input, flags, recommendations, plans, and execution. For "How many helper agents may Codex use at the same time?", the recommendation, empty input, and `--yes-recommended` all select `keep`. RAM-safe mode allows one helper; under MultiAgentV2 this writes two total session threads because the root counts toward the total. Custom counts above one require separate RAM-risk approval. Setup detects the effective V2, V1, disabled, or unknown runtime through app-server `experimentalFeature/list`, then uses the matching V2 or V1 key family. The official V2 boolean enablement form may migrate to a configured table containing `enabled = true`. App-server `config/batchWrite`, `tomllib`, exact marker ownership, proposal-delta checks, atomic commitment, concurrent-snapshot checks, and exact backup/restore metadata protect the config. Only the exact Toolkit PR #237 V1 block may migrate automatically; user-owned or malformed blocks remain untouched. V2 recursive-helper prevention is policy-only unless Codex later provides a verified native hard block. Normal global capacity is never raised automatically for Codex Security because no documented scan-scoped activation mechanism was found. Claude Code and OpenCode receive portable policy only. Final setup reports the effective runtime, normal helper capacity and total-thread semantics, routine policy, recursive-enforcement truth, technical setting, config path and scope, whether setup changed it, backup/restore commands, and the unsupported Security exception with safer alternatives.
