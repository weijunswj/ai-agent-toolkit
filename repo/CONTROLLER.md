...
- **Post-Web-directed G4 reclosure default:** after a bounded `WEB_DIRECTED_CONTINUATION` candidate receives a material `G4_AMEND`, the normal next step before considering another same-root Web-directed G3 grant is a fresh targeted G2 reclosure over the exact material G4 counterexamples. G2 converts those counterexamples into executable required/forbidden behaviour, deterministic regressions plus positive controls, consequential/production-boundary evidence, effect or zero-effect expectations, the validation floor, and any still-missing exact mutation/candidate boundary. A changed root/trust/architecture model requires G1 re-entry. An explicit `G2_CONTRACT_COVERAGE_MISS` requires targeted G2 re-entry under the selected stack's normal `G2` route. G2 reclosure grants no mutation authority, budget reset, new lineage or automatic follow-on.
- **Narrow G2-reuse exception:** Web may reuse the current accepted G2 without a fresh G2 call only when every exact material G4 counterexample is demonstrably already bound to an executable G2 invariant, regression plus positive-control obligation, production-boundary evidence requirement and validation criterion, with no missing semantic or contract decision, and Web records that mapping. For bypass/observability/provenance findings, reuse additionally requires `MECHANISM_COMPLETENESS_ALREADY_BOUND=YES`: accepted G2 already proved its mechanism covers the counterexample family. Semantic mapping or a `G3_IMPLEMENTATION_MISS` label alone is insufficient; `MECHANISM_COMPLETENESS_UNPROVEN` forces targeted G2.
- **Loop executor anti-bounce is not child lifecycle:** bounded diagnosis/recovery applies inside the existing child, but the Loop does not automatically spend a separate `G1_RECONVERGENCE` call merely to avoid returning to Web. After one focused diagnosis/recovery, a repeated same/root-related HOLD returns to Owner/Web with the bounded evidence. Web may optionally invoke one read-only `G1_RECONVERGENCE` synthesis when that diagnostic is materially useful. Re-convergence may identify the smallest G2/G1/Owner boundary or confirm an already-authorised continuation, but it cannot grant authority, reset budgets or change child lifecycle.
- Semantic delegation authority is stage-based, not model-based.
- `G0` and `G3` are the only subagent-capable stages/roles.
- `G_FRAME` is an optional read-only, leaf-only, non-gating problem-framing role used when the problem is materially ambiguous, diagnostic, security/authority-sensitive, or needs competing hypotheses and discriminating evidence questions before investigation. It grants no G1 architecture authority.
- `G0` is bounded read-only evidence acquisition/investigation. It consumes an accepted `G_FRAME` packet or sufficiently specific framing already supplied by User/Web/current evidence, may perform a cheap framing sanity check, and may fan out bounded depth-1 read-only discovery subagents only when the split is genuinely separable and useful.
- **Known-good vs production differential:** when a qualified path works but production fails and the causal question is not already sufficiently framed, `G_FRAME` records the material differences, known-good positive control, real production entry point and discriminating evidence. `G0` then prefers one bounded differential experiment that varies/minimises those differences. When the supplied framing already contains those facts, G0 may proceed directly without a ceremonial G_FRAME invocation. Serial one-delta probes are fallback-only when a prior boundary blocks deeper observation. Incidental environment/check/transport failure does not replace the causal question when another authorised deterministic carrier can answer it.
...
- All other semantic stages/roles are leaf-only, including G_FRAME, G1, G2, G4, G1_RECONVERGENCE, Final Audit, Browser/computer-use, and every spawned subagent.
...
- Governance refers to symbolic execution stages/roles, not concrete model families: `G_FRAME`, `G0`, `G1`, `G2`, `G3`, `G4`, `G1_RECONVERGENCE`, `FINAL_AUDIT`, and `BROWSER`. `G_FRAME` is an optional pre-G0 framing role; `G0` is the non-gating evidence-acquisition/investigation stage. Repository Loop Manager is retired from current route selection; historical Loop receipts/evidence remain compatibility-readable.
...
- `G1_RECONVERGENCE` is a distinct read-only role/context but uses exactly the selected stack's `G1` provider/model/reasoning route; it is not an independent model-strength tier.
- G2 always resolves through the selected named stack's single `G2` route. There is no automatic higher-reasoning G2 retry category. If G2 cannot close after the required evidence/root semantics are sufficient, return to Web for causal adjudication rather than automatically spending another model tier. Web may explicitly select another registered stack for a later run when justified; that changes routing only and grants no mutation, budget, lineage or gate authority.
...
- Authoritative root semantic prompts remain portable and stage/task/authority oriented and do not carry concrete provider/model/reasoning verification obligations. Harness-specific root launch state and G0/G3 subagent-launch configuration may contain the resolved concrete route because they execute orchestration; prompt wording never grants route or model authority.
...
- ASSURED/STRICT material work uses `G0 -> G1 -> G2 -> G3 -> G4` where those stages are required. `G_FRAME` may precede G0 when the investigation is not already sufficiently framed; it is non-gating and must not be invoked ceremonially when User/Web/current evidence already supplies the needed causal framing.
- `G_FRAME` = optional bounded problem framing. For materially ambiguous, diagnostic, security- or authority-sensitive work it identifies known facts, contradictions, material unknowns, competing hypotheses, discriminating evidence questions and a stopping condition for sufficient evidence. It is leaf-only and read-only and grants no G1 architecture authority.
- `G0` = bounded evidence acquisition/investigation. It consumes an accepted G_FRAME packet or sufficiently specific framing already supplied by User/Web/current evidence, may use depth-1 read-only semantic subagents only when the questions are genuinely separable and useful, and publishes durable self-sufficient evidence packets. Leaves gather observations/evidence rather than independently redesigning the solution.
...
- **G2 in-gate convergence:** once admitted, one same-root G2 episode internally performs bounded read-only `challenge -> refine -> challenge` over its proposed mechanism/completeness model, state machine, protocol/schema, recovery behavior, validation/evidence contract and mutation/candidate ceiling. A defect found in G2's own draft contract is ordinary in-gate refinement, not by itself `G2_AMEND` or authority for a fresh same-root G2 RUN/Lock. Keep refining inside the episode while governing root/architecture/trust/Owner decisions are unchanged, required evidence is sufficient/available and the authorised G2 problem boundary is unchanged.
- G2 terminal outcomes are `G2_PASS`, genuine evidence/environment/capability `G2_HOLD`, `G2_REENTRY_REQUIRED` when root/trust/Owner/upstream semantics must change, or `G2_NONCONVERGED` when bounded internal challenge/refinement cannot close the same-root contract despite sufficient evidence/root authority. `G2_NONCONVERGED` returns to Web with the surviving contradiction/hypotheses/evidence; do not manufacture another materially equivalent G2 merely by issuing a new RUN/Lock. Another G2 requires materially changed evidence/authority/upstream decision or explicit Web/Owner justification.
- **Mandatory requirement coverage closure:** before G3 freezes scope, G2 maps every current mandatory acceptance criterion, inherited blocker/defect family and still-required contract obligation exactly once to `IMPLEMENT_IN_THIS_CANDIDATE`, `ALREADY_SATISFIED_WITH_EXACT_EVIDENCE`, `UNCHANGED_REQUIRED_CONSUMER`, or `OUT_OF_SCOPE_WITH_EXPLICIT_CONTINUING_OWNER`. Every implemented row binds requirement -> invariant -> consequential boundary -> affected consumers/surfaces -> deterministic negative regression -> positive control -> G3 executable proof -> G4 assurance surface. Missing, duplicate/conflicting, unevidenced or proof-less current rows are `G2_CONTRACT_COVERAGE_MISS`; G2 cannot PASS or freeze G3 scope.
- **Public-surface / predecessor-compatibility closure:** when an invariant claims one screened/public boundary, no bypass or equivalent surface exclusivity, enumerate exported/callable aliases and materially reachable alternate routes; every public/reachable route must use the accepted enforcement, while internal-only routes are explicit. For backward/readback/serialization compatibility, positive evidence uses immutable bytes/artifacts produced by an exact accepted predecessor producer and consumed unchanged by the candidate; candidate-regenerated equivalents are insufficient.
- **Canonical equivalence / rejection noninterference:** semantically equivalent accepted representations canonicalise before consequential identity/digest/hash/signature comparison. When rejection requires zero user-controlled execution/side effect, eventual rejection is insufficient: bind the material pre-rejection hooks (for example getters, Proxy traps, coercion, iterators, serialization hooks or callbacks) and require zero prohibited executions at the real boundary, with ordinary supported values as positive controls.
...
- **Validation resource non-interference:** when validation cases share a materially bounded/mutable production-like resource, G2 binds the shared resource/equivalence identity, relevant capacity/window/state semantics, which cases consume or mutate it, and each later oracle's prerequisite state. G3 prevents test-created false RED using the smallest faithful strategy: isolation first, then deterministic reset, then explicit shared-state ordering/ownership, and bounded pacing/window separation only when the real resource is inherently time-windowed and cannot be safely isolated/reset. Do not spoof identities, disable limits, bypass production controls or scatter sleeps. Added validation controls must not make an otherwise-valid later oracle fail merely by consuming its prerequisite shared resource unless that interference is itself the accepted behavior under test.
...
- **Mechanism completeness / observability:** for universal/arbitrary/unknown claims, G2 must name the enforcement mechanism and prove it complete under actual runtime observability. Finite hooks/detectors/brands/APIs/events do not prove universal coverage unless exhaustiveness is established. If required post-exposure state is not generically observable, enforce at a complete trusted boundary (invalidate provenance, copy/normalise/re-establish trust) or return G1/Owner; post-hoc detection cannot substitute.
- **Mechanism-completeness proof model:** for universal/arbitrary/no-bypass/complete-provenance claims, regression breadth cannot establish exhaustiveness by itself. G2 PASS additionally requires a bounded `MECHANISM_COMPLETENESS_PROOF_MODEL` covering the complete trusted-boundary inventory, material state machine, protocol schema, enforcement mapping and falsifiable assumptions. Every material executable ingress/egress/export/receipt/binding/actor/state transition is classified as enforced, proven unreachable, or explicitly outside the invariant with valid ownership. G3 maps the model to production-boundary evidence; G4 attacks the model through unlisted paths, multiplicity/replay/reordering, malformed evidence, actor/identity substitution and bypass transitions. If the finite boundary cannot be proven exhaustive, return `MECHANISM_COMPLETENESS_UNPROVEN`. This is conditional on exhaustive claims, not ordinary bounded finite behaviour.
...
- **Commit-required validation sequencing:** when an accepted validator materially requires immutable commit identity or a clean committed working tree, G3 may create its one authorised immutable local candidate commit before that validator only after all meaningful non-commit-dependent checks are green and candidate contents/mutation scope are frozen. Bind the exact commit/tree/parent, then prohibit source amendment, amend/rebase/reconstruction or replacement candidate inside that episode; run the identity-dependent and remaining required floor on that exact commit. Publication remains prohibited until the complete required floor is green. A local candidate commit is construction/custody, not publication, `G3_PASS`, G4 admission, Ready, merge or finality. Candidate RED returns through normal authorised correction/re-entry; environment/transport/evidence HOLD preserves the exact commit rather than rebuilding it. Use this only when the accepted repository validation contract actually requires committed identity.
...
- The ordinary coding path is `G1 -> G2 -> G3 in-gate convergence -> G4 -> Web finality`. G0, targeted re-entry, G1_RECONVERGENCE, HOLD recovery and Web-directed continuation are exception mechanics. Repeated exception use without material blocker/root reduction returns to the responsible root/Owner boundary rather than becoming the default workflow.
- Material G4 findings should carry one primary diagnostic classification when supported: `G1_ROOT_MODEL_MISS`, `G2_CONTRACT_COVERAGE_MISS`, `MECHANISM_COMPLETENESS_UNPROVEN`, `G3_IMPLEMENTATION_MISS`, or `G4_NOVEL_EDGE_CASE`. For detector/interceptor/hook/brand/parser/ledger mechanisms, G4 challenges a materially plausible equivalent violating path that avoids the candidate observer. `MECHANISM_COMPLETENESS_UNPROVEN` is targeted G2 re-entry, not a new gate. Classification is learning evidence, not a score or authority grant.
- **Failure attribution:** every blocking G4/validation finding also records `OWNER=PRODUCT|CONTRACT|TOOLKIT|HARNESS|ENVIRONMENT|UNKNOWN` and `PRODUCT_SEMANTICS_PROVEN_BAD=YES|NO`. `G3_IMPLEMENTATION_MISS` requires `OWNER=PRODUCT` plus evidence that candidate semantics themselves violate an accepted invariant through a valid evidence path. Contract/completeness/proof-model defects are CONTRACT/G2; Toolkit, harness and environment defects do not consume product/G3 correction budget merely because they block evidence. UNKNOWN remains blocking when necessary and triggers bounded diagnosis rather than blind candidate mutation.
- **Equivalent evidence for a broken verifier:** when a canonical verifier/evidence path is defective but is not itself an unresolved required product/security/finality deliverable, Web may accept bounded alternative evidence only if it proves the same unchanged invariant at the same consequential boundary or a proven faithful equivalent, with the same positive/negative/effect obligations, exact provenance and normal independent review. Equivalent evidence may establish product correctness while the verifier defect remains separately owned; it never closes/discards that defect. If product correctness remains unproven, stay blocked as validation uncertainty with `PRODUCT_SEMANTICS_PROVEN_BAD=NO`.
- `G1_RECONVERGENCE` is a read-only leaf G1-class root-model reconsideration/synthesis role, not a gate and not an automatic Loop escalation. After bounded focused diagnosis/recovery, the Loop returns material ambiguity/non-convergence to Web. Web may invoke Re-convergence when a read-only synthesis is worth the additional model call. Its result is one of `CONTINUE_CURRENT_CONTRACT`, `G2_REENTRY_REQUIRED`, `G1_REENTRY_REQUIRED`, `OWNER_DECISION_REQUIRED`, or `NONCONVERGED`; the result grants no authority by itself.
