...
- Every Toolkit-managed launch packet also binds the canonical governance source: repository `weijunswj/ai-agent-toolkit`, exact remote canonical-`main` commit, Controller blob, and stack-registry blob. Consume an already-bound CURRENT/Web receipt when available; do not rediscover those identities from a local Toolkit checkout.
