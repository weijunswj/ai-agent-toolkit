...
- If a registered route is valid but the current harness cannot launch it, return `HARNESS_HANDOFF_REQUIRED` with the portable continuation identity; preserve the lane and let User/Web either switch harness or explicitly select another registered stack. Missing stack/route or genuinely unavailable provider/model remains `ROUTE_UNAVAILABLE`. Neither condition authorises silent fallback or consumes repair budget.
...
- Worker/harness self-report, self-introspection, or post-launch attestation of provider/model/reasoning is non-binding and is never required for gate admission. The authoritative route evidence is the trusted launch binding: selected stack ID, exact stack-registry revision/digest, semantic stage, and resolved provider/model/reasoning recorded before launch. Absence of runtime model/reasoning metadata is not a HOLD and must not cause `HARNESS_HANDOFF_REQUIRED`. Where a host independently exposes trusted runtime route metadata, it may be used only as additional contradiction evidence; a material contradiction fails closed, while missing metadata does not. Silent route/model fallback remains prohibited.
