...
- **Known-good vs production differential evidence:** when a qualified/tested path works but the real production path fails, G0-A first inventories the material differences between those paths and freezes the known-good upper-bound positive control, real production entry point, competing difference families and discriminating evidence. G0-B then prefers one bounded differential experiment that starts from the known-good configuration and systematically varies/minimises those differences while exercising the real production entry point. Serial one-delta evidence episodes are allowed only when an earlier boundary genuinely prevents deeper observation in the same safe experiment. An unrelated environment/check/transport failure does not redefine the causal question when another deterministic authorised evidence carrier can still exercise it. Existing fail-closed, authority, gate and evidence standards remain unchanged.
...
- **Resource-equivalence identity check:** when a logical identity both maps to a consequential external resource and governs locks, uniqueness, deduplication, tombstones/retirement, cleanup ownership, rollback authority or equivalent lifecycle state, G1/G2 must determine whether two different accepted logical identities can address the same underlying resource under the material filesystem/object-key/URL-origin/database/case-folding/normalisation rules of that resource. If yes, the accepted contract defines one coherent resource-equivalence identity or explicit alias semantics before G3. Do not widen this into speculative Unicode/path normalisation work where the resource does not require it.
...
- **Causal negative-control oracle:** when a regression claims operation/actor X causes material outcome Y, test instrumentation may expose, delay, synchronise or observe X but must not independently manufacture Y. Bind the exact actor/operation under test and observe the actual required consequential outcome. Missing observation fails. Constructed/fallback evidence, cleanup actions and fault-injection performed by another actor cannot substitute for the causal observation the oracle claims to prove.
- **Async/liveness adversarial validation:** when implementation introduces or materially changes a drain, wait, flush, join, poll, quiesce, retry queue, completion tracker or equivalent async-completion boundary, G3 validation includes a deterministic waiter-first control where completion occurs later and requires event-loop, I/O, timer or callback progress; the waiter must permit that progress and complete. A favourable already-complete-before-waiter control is insufficient by itself. If required outstanding work is non-zero but no pending completion/progress signal exists, fail loudly rather than spin to timeout; prefer barriers/events/deferred completion over arbitrary sleeps.
- **Deferred consequential-work accounting:** queued, debounced or timer-scheduled required work is outstanding from scheduling until its real consequential completion. Cancelling/clearing a timer cannot make required work disappear. A flush either allows the scheduled normal path to execute or takes ownership of that same work, dispatches it through the real normal path and awaits the required response/body/readback or equivalent completion semantics.
- **Conditional G3 adversarial pre-publication validation:** for sufficiently complex/STRICT G3 work involving concurrency, async completion, deferred work, causal negative controls, lifecycle coordination or logical-identity/resource mapping, perform an adversarial validation episode inside G3 before publication. When genuinely separable and materially useful, prefer an optional depth-1 read-only validation leaf to try to falsify regression oracles, causal attribution, liveness/progress assumptions, outstanding-work accounting, resource-equivalence handling and false-green controls; otherwise run the same challenge serially in the parent. The leaf never mutates or declares gate completion, and the parent remains sole owner/integrator and independently revalidates the integrated result. RED against settled behaviour stays inside G3; a missing product/compatibility semantic returns to G2; a changed root/trust/architecture model returns to G1.
...
- Before tearing down a disposable worktree, sandbox, worker/session or temporary evidence surface after material construction/validation, preserve every later-required exact candidate and non-repository evidence artifact in a durable/retrievable or deterministically reproducible form bound to repository/RUN/Lock/gate/candidate identity, and verify the intended later consumer can recover it. Temporary path, chat memory, digest-only evidence or disappearing uncommitted state is insufficient. Publication authority is not implied; environment/transport failure preserves the immutable candidate rather than forcing reconstruction.
...
- A still-required active lineage admitted under older governance is reconciled prospectively onto compatible stricter current Controller mechanics at the next safe terminal/reconciliation boundary, never by silently rewriting an in-flight worker contract. Preserve Delivery Child/root/continuation identity, all consumed budgets/attempts, historical findings/evidence/candidates and original gate outcomes. Newer governance alone does not reopen accepted architecture/contract or reset history; a material semantic conflict routes explicitly to the responsible G1/G2/Owner boundary.
