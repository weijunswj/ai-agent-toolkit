...
- **Mandatory requirement coverage closure:** before G3 freezes scope, G2 maps every current mandatory acceptance criterion, inherited blocker/defect family and still-required contract obligation exactly once to `IMPLEMENT_IN_THIS_CANDIDATE`, `ALREADY_SATISFIED_WITH_EXACT_EVIDENCE`, `UNCHANGED_REQUIRED_CONSUMER`, or `OUT_OF_SCOPE_WITH_EXPLICIT_CONTINUING_OWNER`. Every implemented row binds requirement -> invariant -> consequential boundary -> affected consumers/surfaces -> negative regression -> positive control -> G3 executable proof -> G4 assurance surface. Missing, duplicate/conflicting, unevidenced or proof-less current rows are `G2_CONTRACT_COVERAGE_MISS`; G2 cannot PASS or freeze G3 scope.
...
- **Mechanism completeness / observability:** for universal/arbitrary/unknown claims and invariants, G2 must name the enforcement mechanism and prove it complete under actual runtime observability, including the language/runtime model and material paths that can bypass a finite observer. Finite hooks/detectors/brands/APIs/events do not prove universal coverage unless exhaustiveness is established. If required post-exposure state is not generically observable, enforce at a complete trusted boundary (for example invalidate provenance at exposure and require re-establishment), or return to G1/Owner; post-hoc detection cannot substitute.
...
- **Commit-required validation sequencing:** when an accepted validator materially requires immutable commit identity or a clean committed working tree, G3 may create its one authorised immutable local candidate commit before that validator only after all meaningful non-commit-dependent checks are green and candidate contents/mutation scope are frozen. Bind the exact commit/tree/parent, then prohibit source amendment, amend/rebase/reconstruction or replacement candidate inside that episode; run the identity-dependent and remaining required floor on that exact commit. Publication remains prohibited until the complete required floor is green. A local candidate commit is construction/custody, not publication, `G3_PASS`, G4 admission, Ready, merge or finality. Candidate RED returns through normal correction/re-entry; environment/transport/evidence HOLD preserves the exact commit rather than rebuilding it. Use this only when the accepted repository validation contract actually requires committed identity.
...
- `G1_RECONVERGENCE` is a read-only, leaf-only role, not a gate, for G1-class root-model reconsideration/synthesis that Web may invoke after bounded focused recovery fails to converge at the same/root-related boundary. The Loop does not invoke it automatically merely to avoid a Web return. Its result is `CONTINUE_CURRENT_CONTRACT`, `G2_REENTRY_REQUIRED`, `G1_REENTRY_REQUIRED`, `OWNER_DECISION_REQUIRED`, or `NONCONVERGED`; it grants no authority.
