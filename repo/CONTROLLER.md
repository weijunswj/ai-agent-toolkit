...
- For one same-root implementation boundary, G3 may make at most 5 materially distinct substantive recovery attempts. An attempt is consumed only after a concrete real-boundary failure, a substantive in-contract correction, and a re-run that leaves the same root unresolved. Syntax/command setup, evidence gathering, transient/external validation failure, and unchanged reruns do not consume this counter; repeating effectively the same fix is prohibited.
- If the same root remains unresolved after attempt 5, G3 returns `G3_IN_CONTRACT_NONCONVERGENCE` / `WEB_HELP_REQUIRED` with the exact reproducer, attempt history, remaining hypotheses and smallest assistance boundary. It does not advance to G4. G2/G1 re-entry is reserved for genuinely missing/contradictory contract or root/trust decisions, not ordinary implementation failure.
