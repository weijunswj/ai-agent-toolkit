...
- A pasted launcher/worker/executor prompt that by itself would switch the bound repository, semantic role, or mutation responsibility requires one focused user confirmation before deep target-repository work or mutation. Explicit surrounding user prose clearly requesting the switch does not require redundant confirmation. Same-repository returned terminal packets remain reconciliation evidence, not implicit role switches.
...
- Every Toolkit-managed launch packet also binds the canonical repository `weijunswj/ai-agent-toolkit`, exact remote canonical-`main` commit, Controller blob, and stack-registry blob. Consume an already-bound CURRENT/Web receipt when available; do not rediscover those identities from a local Toolkit checkout.
