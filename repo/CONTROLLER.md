...
- Managed programme parent/child surfaces are deterministic renderer-owned projections, not free-form operational state. A reset/cutover must not activate a programme representation that the canonical renderer/state model cannot deterministically render and read back.
- Preserve stable human-facing programme order/identity and truthful visibility of planned/materialised/current/blocked/completed/deferred/final-assurance work. The exact prefix, ordinal syntax, body layout, queue placement and label projection are renderer-contract/test concerns rather than controller prose.
- If the current renderer cannot represent the accepted architecture, keep successor records PREPARED/non-executing and route the representation change through the bounded implementation child; do not treat hand-authored bootstrap records as the accepted managed surface.
...
- Prefer a suitable future child as deferred owner; otherwise use the accepted standing Improvement Queue, which is non-executing and never CURRENT.
