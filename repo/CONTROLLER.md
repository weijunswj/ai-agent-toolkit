...
- Only `G0-B` and `G3` may resolve semantic subagent routes. All other stages/roles are leaf-only. A subagent uses the route that the human/launcher bound for that child, or inherits the already-bound parent route where the harness supports inheritance; the subagent itself does not verify the route.
