...
- Governance refers to symbolic execution stages/roles, not concrete model families: `G_FRAME`, `G0`, `G1`, `G2`, `G3`, `G4`, `LOOP`, `G1_RECONVERGENCE`, `FINAL_AUDIT`, and `BROWSER`. `G_FRAME` is an optional pre-G0 framing role; `G0` is the non-gating evidence-acquisition/investigation stage.
