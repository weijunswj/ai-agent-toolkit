...
const { comparableWorkflow } = require(path.join(repoRoot, 'skills', 'n8n-workflow-transport', 'templates', 'helper-scripts', 'import-export-sync', 'should-import-n8n-workflow.cjs'));
const { selectBindingsWithMeta, restoreLiveWebhookIds } = require(path.join(repoRoot, 'skills', 'n8n-workflow-transport', 'templates', 'helper-scripts', 'import-export-sync', 'prepare-n8n-live-import.cjs'));

const scriptDir = path.join(repoRoot, 'skills', 'n8n-workflow-transport', 'templates', 'helper-scripts', 'import-export-sync');
const sourceScriptDir = scriptDir;
const {
  parseArgs,
  resolveN8nDockerTarget,
} = require(path.join(sourceScriptDir, 'resolve-n8n-docker-target.cjs'));
const sanitizerDir = path.join(repoRoot, 'skills', 'n8n-workflow-transport', 'templates', 'helper-scripts', 'sanitizer');
const sourceSanitizerDir = sanitizerDir;
const ragFixtureDir = path.join(repoRoot, 'repo', 'tests', 'fixtures', 'n8n-workflows');
...
test('RAG workflow fixtures stay generic and inactive', () => {
  for (const fileName of [
    'customer-support-agent.workflow.template.json',
    'rag-ingestion.workflow.template.json',
  ]) {
    const sourcePath = path.join(ragFixtureDir, fileName);
  ...
  const customerSupportText = readText(path.join(ragFixtureDir, 'customer-support-agent.workflow.template.json'));
...
test('RAG workflow Code nodes contain executable JavaScript instead of placeholder-only bodies', () => {
  ...
  for (const [fileName, nodeName] of cases) {
    const workflow = JSON.parse(readText(path.join(ragFixtureDir, fileName)));
