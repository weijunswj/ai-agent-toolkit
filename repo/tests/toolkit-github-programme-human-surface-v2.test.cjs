...
const humanSurfaceSchema = JSON.parse(fs.readFileSync(HUMAN_SURFACE_SCHEMA_PATH, 'utf8'));
const fixture = proofFixture.historical_reference;
const LEGACY_STAGE_A = runtime.FINALISATION_RENDERED_TARGETS.stage_a;
...
function descriptor(overrides = {}) {
  return {
    ...
    summary: 'Preserve frozen-v5 child readComplete compatibility while correcting sequential history evidence reuse for Root-008.',
...
function validateProofFixture() {
  ...
  assert.equal(proofFixture.root, HISTORICAL_PROOF_ROOT);
  assert.equal(proofFixture.lock, HISTORICAL_PROOF_LOCK);
...
function humanSurfaceContractInstance() {
  const read = completeRead('');
  const inputDescriptor = descriptor();
  return {
    schema: 'toolkit.github-program.human-surface.v2',
    root: ROOT,
    lock: LOCK,
    version: 'human-v2',
    ...
    stages: [
      'INPUT', 'COMPLETE_READ', 'CLASSIFY', 'PARSE', 'AUTHORITY', 'CANONICAL', 'RELATIONSHIP',
    ...
    inputs: {
      ...
      history: {
        ...
        lock: LOCK,
        repository: REPOSITORY,
        source: { body_sha256: read.body_sha256, canonical_sha256: '0'.repeat(64) },
        authority: {
          kind: 'USER_WEB_CONTROLLER',
          reference: 'https://example.com/web-controller',
          body_sha256: '0'.repeat(64),
        },
        additions: { pr_history: [], evidence_refs: [], transitions: [] },
        decision_sha256: '0'.repeat(64),
      },
      provider_observation: [],
      migration: { parent_read: read, child_read: read, history_decision: null, provider_observations: null },
    },
    result: {
      success_required_fields: ['ok', 'code', 'operation', 'safe_for_provider_write', 'provider_mutation_authorised'],
      failure_required_fields: ['ok', 'code', 'operation', 'stage', 'safe_for_provider_write', 'provider_mutation_authorised'],
      provider_write: false,
      provider_mutation_authorised: false,
      render_artifact: {
        operation: 'extendHistory',
        parent_fields: ['kind', 'repository', 'number', 'action', 'body', 'read_record', 'digests'],
        child_fields: ['kind', 'repository', 'number', 'action', 'body', 'read_record', 'digests'],
        read_record_fields: ['body', 'complete', 'byte_length', 'body_sha256', 'revision'],
        digest_fields: ['complete_body_sha256', 'public_prose_sha256', 'managed_block_sha256', 'carrier_sha256', 'projection_sha256', 'canonical_sha256', 'descriptor_sha256', 'candidate_sha256', 'bound_authority_sha256'],
        action_sources: { parent: 'h2ProgrammeAction', child: 'h2ChildAction' },
        full_body_readback: true,
      },
    },
    legacy: {
      accepted_parent: 'frozen-v5-only',
      accepted_child: 'frozen-v5-only',
      whole_body_classifier: true,
      new_rendering: 'human-v2-only',
    },
    serialization: {
      canonical_json: 'existing-toolkit-sort-keys-finite-safe-integers',
      base64url: 'unpadded-canonical-json',
      typed_nodes: true,
    ...
    lifecycle: {
      ...
      programme_actions: ['CONTINUE_CURRENT_CHILD', 'AWAIT_PROGRAMME_FINALITY', 'PROGRAMME_COMPLETE'],
      precedence: ['CONTRADICTION', 'COMPLETED', 'BLOCKING_HOLD', 'WAIT_DEPENDENCIES', 'AWAIT_CHILD_AUTHORITY', 'CONTINUE_ACTIVE_GATE', 'AMEND_REQUIRED', 'REPLACEMENT_REQUIRED', 'AWAIT_EPOCH_AUTHORITY', 'BEGIN_OR_CONTINUE_EPOCH', 'AWAIT_CHILD_FINALITY'],
      zero_current_is_not_complete: true,
      terminal_truth: 'source-backed',
    },
    migration: {
      max_write_plans: 1,
      legacy_legacy: 'WRITE_PARENT',
      human_legacy: 'WRITE_CHILD',
      human_human: 'RECONCILED',
    ...
    executable_proof: {
      fixture: 'repo/tests/fixtures/github-program-human-surface/root-005-executable-proof-matrix-v1.json',
      receipt_schema: 'toolkit.github.program.executable-proof-receipt.v1',
      families: ['REJECTED_RED_PROOF', 'REPLACEMENT_GREEN_PROOF', 'F1_F8_GREEN'],
      cases_per_family: 8,
      aggregate_rule: 'executed production receipts only',
      metadata_only_aggregate: false,
...
test('human-surface-v2 schema declares every root requirement and passes strict semantic qualification', () => {
  assert.equal(humanSurfaceSchema.additionalProperties, false);
  assert.ok(humanSurfaceSchema.required.every((property) => Object.prototype.hasOwnProperty.call(humanSurfaceSchema.properties, property)));
  assert.ok(Object.prototype.hasOwnProperty.call(humanSurfaceSchema.properties, 'phase_projection'));
  assert.deepEqual(Object.keys(humanSurfaceSchema.properties).sort(), humanSurfaceSchema.required.slice().sort());

  const validate = new Ajv2020({ allErrors: true, strict: true }).compile(humanSurfaceSchema);
  const valid = humanSurfaceContractInstance();
  assert.equal(validate(valid), true, JSON.stringify(validate.errors));

  const missingPhaseProjection = clone(valid);
  delete missingPhaseProjection.phase_projection;
  assert.equal(validate(missingPhaseProjection), false);
...
test('accepted frozen-v5 child readComplete uses bound parent state and preserves source binding', () => {
  const legacyParent = publicRead(LEGACY_STAGE_B.parent, { kind: 'parent', repository: REPOSITORY, issue: 240 });
  assert.equal(legacyParent.ok, true, JSON.stringify(legacyParent));

  const legacyChild = publicRead(LEGACY_STAGE_B.child, {
    kind: 'child', repository: REPOSITORY, issue: 359, parent_issue: 240, parent_read: legacyParent.read,
  });
  assert.equal(legacyChild.ok, true, JSON.stringify(legacyChild));
  assert.equal(legacyChild.format, 'legacy-v5');
  assert.equal(legacyChild.repository, REPOSITORY);
  assert.equal(legacyChild.issue, 359);
  assert.equal(legacyChild.canonical_sha256, legacyParent.canonical_sha256);
  assert.deepEqual(legacyChild.canonical_state, legacyParent.canonical_state);
  assert.equal(legacyChild.read.body, LEGACY_STAGE_B.child);

  const mismatchedParent = completeRead(LEGACY_STAGE_A.parent);
  const mismatchedChild = publicRead(LEGACY_STAGE_B.child, {
    kind: 'child', repository: REPOSITORY, issue: 359, parent_issue: 240, parent_read: mismatchedParent,
  });
  expectFailure(mismatchedChild, 'CHILD_SOURCE_MISMATCH');
});
...
test('extendHistory reuses accepted history evidence across sequential PR and transition records', () => {
  const initial = historyContext();
  const evidenceId = 'history-evidence';
  const firstDescriptor = descriptor({ evidence_refs: [evidenceId] });
  const firstAuthority = boundAuthority(firstDescriptor, 400);
  const firstAddition = historyAddition(firstDescriptor, firstAuthority, evidenceId);
  const firstDecision = historyDecision(initial.parent, firstDescriptor, firstAuthority, firstAddition);
  const first = surface.extendHistory({
    parent_read: initial.parentRead,
    decision: firstDecision,
    provider_observations: null,
  });
  assert.equal(first.ok, true, JSON.stringify(first));
  assertHistoryArtifacts(first);
  assert.deepEqual(first.target_state.human_surface_v2_history.evidence_refs.map((item) => item.id), [evidenceId]);

  const firstParent = publicRead(first.parent.body, { kind: 'parent', repository: REPOSITORY, issue: 240 });
  const secondDescriptor = descriptor({
    summary: 'Record a later PR history entry that reuses accepted evidence.',
    evidence_refs: [evidenceId],
  });
  const secondAuthority = boundAuthority(secondDescriptor, 401, {
    source: {
      kind: 'USER_WEB_CONTROLLER',
      reference: 'https://github.com/weijunswj/ai-agent-toolkit/pull/401',
      body_sha256: '3333333333333333333333333333333333333333333333333333333333333333',
    },
  });
  const secondEntry = historyAddition(secondDescriptor, secondAuthority, 'not-added').pr_history[0];
  const secondDecision = historyDecision(firstParent, secondDescriptor, secondAuthority, {
    pr_history: [secondEntry],
    evidence_refs: [],
    transitions: [],
  });
  const second = surface.extendHistory({
    parent_read: first.parent.read_record,
    decision: secondDecision,
    provider_observations: null,
  });
  assert.equal(second.ok, true, JSON.stringify(second));
  assertHistoryArtifacts(second);
  assert.deepEqual(second.target_state.human_surface_v2_history.pr_history.map((item) => item.bound_authority.pr_number), [400, 401]);
  assert.deepEqual(second.target_state.human_surface_v2_history.evidence_refs.map((item) => item.id), [evidenceId]);

  const secondParent = publicRead(second.parent.body, { kind: 'parent', repository: REPOSITORY, issue: 240 });
  const reusedTransition = {
    id: 'history-transition-reuse',
    child_issue: 359,
    epoch_id: 'E4',
    gate: 'G3',
    disposition: 'ADDED',
  ...
  const thirdDecision = historyDecision(secondParent, secondDescriptor, secondAuthority, {
    pr_history: [],
    evidence_refs: [],
    transitions: [reusedTransition],
  });
  const third = surface.extendHistory({
    parent_read: second.parent.read_record,
    decision: thirdDecision,
    provider_observations: null,
  });
  assert.equal(third.ok, true, JSON.stringify(third));
  assertHistoryArtifacts(third);
  assert.deepEqual(third.target_state.human_surface_v2_history.transitions, [reusedTransition]);

  const duplicateDecision = historyDecision(firstParent, secondDescriptor, secondAuthority, {
    pr_history: [],
    evidence_refs: [clone(firstAddition.evidence_refs[0])],
    transitions: [],
  });
  expectFailure(surface.extendHistory({
    parent_read: first.parent.read_record,
  ...
  const unknownDescriptor = descriptor({
    summary: 'Reject an unknown evidence reference in a later PR history entry.',
    evidence_refs: ['unknown-history-evidence'],
  });
  const unknownAuthority = boundAuthority(unknownDescriptor, 402, {
    source: {
      kind: 'USER_WEB_CONTROLLER',
      reference: 'https://github.com/weijunswj/ai-agent-toolkit/pull/402',
      body_sha256: '3333333333333333333333333333333333333333333333333333333333333333',
    },
  });
  const unknownEntry = historyAddition(unknownDescriptor, unknownAuthority, 'not-added').pr_history[0];
  const unknownPrDecision = historyDecision(firstParent, unknownDescriptor, unknownAuthority, {
    pr_history: [unknownEntry],
    evidence_refs: [],
    transitions: [],
  });
  expectFailure(surface.extendHistory({
    parent_read: first.parent.read_record,
    decision: unknownPrDecision,
    provider_observations: null,
  }), 'HISTORY_EVIDENCE_MISSING');

  const unknownTransitionDecision = historyDecision(firstParent, secondDescriptor, secondAuthority, {
    pr_history: [],
    evidence_refs: [],
    transitions: [{ ...reusedTransition, id: 'history-transition-unknown', evidence_ref: 'unknown-history-evidence' }],
  });
  expectFailure(surface.extendHistory({
    parent_read: first.parent.read_record,
    decision: unknownTransitionDecision,
    provider_observations: null,
  }), 'HISTORY_REFERENT_MISSING');
});
