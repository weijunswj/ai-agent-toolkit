...
test('portable surface contract requires the full Parent Child and active PR minimum', () => {
  ...
  assert.equal(contract.$schema, 'toolkit.github-program.surface.v4');
  assert.equal(contract.canonical_state.semantic_authority_count, 1);
  for (const field of ['goal', 'progress', 'child_graph']) assert.ok(contract.legacy_v1_portable_minimum.parent.includes(field), field);
  for (const field of ['scope', 'achieved', 'remaining', 'holds', 'eli5']) assert.ok(contract.legacy_v1_portable_minimum.child.includes(field), field);
  for (const field of ['purpose', 'out_of_scope', 'safety_constraints', 'validation_status', 'eli5']) assert.ok(contract.legacy_v1_portable_minimum.pr.includes(field), field);
  assert.equal(contract.rendering.Parent_contains_canonical_state, true);
  assert.equal(contract.migration.unrelated_bytes_preserved, true);
});
