...
function authorityPacket(overrides = {}) {
  ...
  return {
    schema: programme.AUTHORITY_PACKET_CURRENT_SCHEMA,
    repository,
    parent_issue: 240,
    child_issue: 359,
...
test('optional CURRENT authority packet survives v5 canonical, render, parse and child projection paths', () => {
  const source = parsedSources();
  ...
  const parsedChild = programme.parseChildV5Body(rendered.child, { repository, parent_issue: 240, canonical_digest: valid.canonical_digest });
  assert.equal(parent.ok, true, parent.code);
  assert.equal(parsedChild.ok, true, parsedChild.code);
  ...
  assert.deepEqual(projection.projection.authority_packet_current, packet);
  assert.deepEqual(Object.keys(projection.projection).includes('predecessors'), false);

  const unknown = clone(state);
  unknown.children.find((item) => item.issue === 359).authority_packet_current.body = 'not allowed';
  assert.equal(programme.validateCanonicalStateV5(unknown).ok, false);
  const recursive = clone(state);
  recursive.children.find((item) => item.issue === 359).authority_packet_current.predecessors[0].predecessors = [];
