...
function createMinimalSetupRepo(root) {
  ...
  writeFile(path.join(root, 'repo', 'scripts', 'setup-codex-toolkit-plugin.cjs'), [
    ...
    "process.stdout.write(JSON.stringify({ ok: true, version: '2.3.7', cache_root: path.join(process.cwd(), 'fake-codex-cache'), hook_trust_message: 'review Codex hook trust if prompted' }));",
    'process.exit(0);',
    ''
  ].join('\n'));
  writeFile(path.join(root, 'repo', 'scripts', 'setup-claude-toolkit-plugin.cjs'), [
    ...
    "process.stdout.write(JSON.stringify({ ok: true, version: '2.3.7', scope: 'user' }));",
    'process.exit(0);',
    ''
  ].join('\n'));
...
function createFakeManagedSetupScript(root, version = '2.3.7', options = {}) {
...
test('setup execute persists all selected preferences in one run and prints final summary', () => {
  ...
  assert.match(result.stdout, /Codex expected Toolkit version: 2\.3\.7/);
  assert.match(result.stdout, /Codex installed Toolkit version: 2\.3\.7/);
...
test('active setup command delegates to managed checkout script when it exists', () => {
  const root = tmpRoot();
  const { managedPath, scriptPath } = createFakeManagedSetupScript(root, '2.3.7');
  ...
  assert.match(result.stdout, /managed setup script version 2\.3\.7/);
  assert.match(result.stdout, /# setup toolkit question bank/);
});

test('active setup command does not block on stdin before delegating to the managed checkout', async () => {
  const root = tmpRoot();
  createFakeManagedSetupScript(root, '2.3.7', { emitQuestionBank: false, exitCode: 0 });
...
test('managed question-bank pause is not bypassed with active fallback', () => {
  const root = tmpRoot();
  createFakeManagedSetupScript(root, '2.3.7');
  const result = run(['--execute', '--profile', 'auto-main'], {
    env: isolatedHomeEnv(root)
  });
...
test('claude-code setup execute with instructions behavior verifies Claude plugin metadata and skips Codex helper', () => {
  ...
  assert.match(result.stdout, /Claude expected Toolkit version: 2\.3\.7/);
  assert.match(result.stdout, /Claude manifest Toolkit version: 2\.3\.7/);
