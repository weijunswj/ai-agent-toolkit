...
test('resource evidence never falls through from explicit gaps and keeps the test seam explicit', () => {
  const fixture = {
    fixture_id: control.REPOSITORY_TEST_RESOURCE_FIXTURE_ID,
    physical_total: 16 * control.GIB,
    physical_available: 8 * control.GIB,
    commit_total: 32 * control.GIB,
    commit_available: 16 * control.GIB,
    source: control.REPOSITORY_TEST_RESOURCE_SOURCE,
    host_responsive: true,
  };
  const fixtureEnv = {
    [control.REPOSITORY_TEST_RESOURCE_CONTEXT_ENV]: control.REPOSITORY_TEST_RESOURCE_CONTEXT_VALUE,
    [control.REPOSITORY_TEST_RESOURCE_STATE_ENV]: JSON.stringify(fixture),
  };

  assert.equal(control.inspectResources({ resourceState: undefined, test_seam: false }), null);
  assert.equal(control.inspectResources({ resourceState: null, test_seam: false }), null);
  assert.equal(control.inspectResourceCapability({ resourceState: undefined, test_seam: false }).supported, false);
  assert.equal(control.inspectResourceCapability({ resourceState: null, test_seam: false }).supported, false);

  const production = control.inspectResourceCapability({ resourceState: resources({ source: 'proc-meminfo' }) });
  assert.equal(production.supported, true);
  assert.equal(production.source, 'proc-meminfo');

  assert.equal(control.inspectResourceCapability({ resourceState: fixture }).supported, false);
  assert.equal(control.inspectResourceCapability({ resourceState: fixture, test_seam: true }).supported, true);
  const seam = control.inspectResourceCapability({ env: fixtureEnv, test_seam: true });
  assert.equal(seam.supported, true);
  assert.equal(seam.resources.fixture_id, control.REPOSITORY_TEST_RESOURCE_FIXTURE_ID);
  for (const options of [{ env: fixtureEnv }, { env: fixtureEnv, test_seam: false }]) {
    const ordinary = control.inspectResourceCapability(options);
    assert.notEqual(ordinary.resources?.fixture_id, control.REPOSITORY_TEST_RESOURCE_FIXTURE_ID);
  }

  const work = root();
  const direct = configured(control.TOPOLOGIES.CLAUDE_DIRECT, control.CAPACITY_MODES.AUTO, {
    resource_counter_supported: undefined,
    resource_counter_source: undefined,
  });
  assert.throws(() => control.configureProfile('claude-code', direct, { root: work, test_seam: false }), /resource counters/i);
  assert.throws(() => control.configureProfile('claude-code', configured(control.TOPOLOGIES.CLAUDE_DIRECT, control.CAPACITY_MODES.AUTO, {
    resource_state: undefined,
  }), { root: root() }), /resource counters/i);
  assert.throws(() => control.configureProfile('claude-code', configured(control.TOPOLOGIES.CLAUDE_DIRECT, control.CAPACITY_MODES.AUTO, {
    resource_state: fixture,
  }), { root: root() }), /resource counters/i);
  assert.equal(control.admissionDecision(spec(), verifiedOptions({ root: root(), resourceState: undefined, test_seam: false })).result, control.RESULTS.REFUSE);
  assert.equal(control.resourceAdmissionDecision(spec(), profile(), undefined, { root: root() }).result, control.RESULTS.REFUSE);
});
