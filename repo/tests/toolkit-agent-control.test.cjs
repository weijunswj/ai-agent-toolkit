...
function activationProof(overrides = {}) {
  return { schema: 4, source: 'claude-plugin-list', plugin_version: control.CONTROL_VERSION,
    cache_identity: 'a'.repeat(64), hook_sha256: 'b'.repeat(64), controller_sha256: 'c'.repeat(64), process_launch_sha256: 'e'.repeat(64), agent_hook_sha256: 'd'.repeat(64), n8n_admission_hook_sha256: 'f'.repeat(64), n8n_domain_router_sha256: '1'.repeat(64), ...overrides };
}
let defaultVerifier;
...
test('legacy strict profiles fail closed while a correct schema-4 proof persists and revalidates', () => {
  ...
  assert.equal(persisted.activation_proof.schema, 4);
