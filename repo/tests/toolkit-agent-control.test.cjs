...
function profile(overrides = {}) {
  return { schema: control.SCHEMA, host: 'claude-code', topology: control.TOPOLOGIES.CLAUDE_DIRECT, capacity_mode: control.CAPACITY_MODES.AUTO, manual_maximum: 0, ...overrides };
}
...
test('atomic admission prevents two concurrent parents from consuming one manual slot', async () => {
  const work = root();
  control.configureProfile('claude-code', { topology: control.TOPOLOGIES.CLAUDE_DIRECT, capacity_mode: control.CAPACITY_MODES.MANUAL, manual_maximum: 1 }, { root: work });
...
test('Claude hook denies native Agent under root/direct profiles and allows broader native behavior', () => {
  for (const topology of [control.TOPOLOGIES.ROOT_ONLY, control.TOPOLOGIES.CLAUDE_DIRECT]) {
    const work = root();
    control.configureProfile('claude-code', { topology, capacity_mode: control.CAPACITY_MODES.ROOT_ONLY }, { root: work });
...
test('Codex and Claude profile state is isolated', () => {
  const work = root();
  control.configureProfile('claude-code', { topology: control.TOPOLOGIES.CLAUDE_DIRECT, capacity_mode: control.CAPACITY_MODES.AUTO }, { root: work });
