...
function writeInstalledCache(codexHome, options = {}) {
  const version = options.version || '2.3.0';
...
function installCache(repoRoot) {
  const root = path.join(codexHome, 'plugins', 'cache', 'ai-agent-toolkit-local', 'ai-agent-toolkit', '2.3.0');
...
test('Codex Toolkit plugin setup verifier accepts active 2.3.0 install with SessionStart cache', () => {
...
test('Codex Toolkit isolated CODEX_HOME smoke command is documented', () => {
  ...
  assert.match(bridgeDoc, /plugins\/cache\/ai-agent-toolkit-local\/ai-agent-toolkit\/2\.3\.0/);
