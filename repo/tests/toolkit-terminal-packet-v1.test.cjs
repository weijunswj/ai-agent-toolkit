...
test('terminal packet candidate mode is all-or-none and gate-bound', () => {
  const none = { pr: null, head: null, tree: null, base: { ref: null, sha: null } };
  assert.equal(makePacket({ gate: 'G2', candidate: none }).ok, true);
  assert.equal(makePacket({ gate: 'G2', candidate }).code, 'TERMINAL_PACKET_CANDIDATE_MODE_MISMATCH');
  assert.equal(makePacket({ gate: 'G3', candidate: none }).code, 'TERMINAL_PACKET_CANDIDATE_MODE_MISMATCH');
  assert.equal(makePacket({ gate: 'G3', candidate }).ok, true);
  assert.equal(makePacket({ gate: 'G3_REPAIR_1', candidate }).ok, true);
  assert.equal(makePacket({ gate: 'G3', candidate: { ...candidate, head: null } }).code, 'TERMINAL_PACKET_INCOMPLETE');
});
