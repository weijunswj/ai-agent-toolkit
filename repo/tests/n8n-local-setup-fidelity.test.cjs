...
test('local launcher and menu keep the console open until Exit', () => {
  ...
  assert.match(functionBody(menu, 'Repair-N8nConfigEncryptionKey'), /\/home\/node\/\.n8n\/config[\s\S]*process\.env\.N8N_ENCRYPTION_KEY[\s\S]*config\.encryptionKey = key[\s\S]*--entrypoint', 'node'/);
