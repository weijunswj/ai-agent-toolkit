...
function launchInput(overrides = {}) {
  return {
    ...
    fence_valid: true,
    structural_admission: { ...programme.PROGRAMME_421_CURRENT_STRUCTURAL_ADMISSION },
...
test('CURRENT launch safety blocks unknown, conflicting, active, and completed worker state', () => {
  for (const overrides of [
    { worker_liveness: 'unknown' },
    { worker_liveness: 'inactive', liveness: 'active' },
    { run: { id: 'run-c1', state: 'IN_FLIGHT' } },
  ]) {
    const checked = programme.validateCurrentLaunchSafety(launchInput(overrides));
    assert.equal(checked.ok, false);
    assert.equal(checked.reason_code, 'WORKER_LIVENESS_UNVERIFIED');
    assert.equal(checked.launch_allowed, false);
  }

  const completed = programme.validateCurrentLaunchSafety(launchInput({ run: { id: 'run-c1', state: 'COMPLETED' } }));
  assert.equal(completed.ok, false);
  assert.equal(completed.reason_code, 'TERMINAL_STATE_NOT_EXECUTABLE');
  assert.equal(completed.launch_allowed, false);

  const staleBranchObservation = programme.validateCurrentLaunchSafety(launchInput({
    worker_liveness: 'inactive',
    worker_liveness_evidence: 'branch-pr',
  }));
  assert.equal(staleBranchObservation.ok, false);
  assert.equal(staleBranchObservation.reason_code, 'WORKER_LIVENESS_UNVERIFIED');
});
...
test('public CURRENT and graph boundaries reject accessors without invoking getters', () => {
  ...
  const currentArray = [];
  Object.defineProperty(currentArray, '0', {
    enumerable: true,
    get() { getterRuns += 1; return 'unsafe'; },
  });
  const arrayChecked = programme.validateCurrentLaunchSafety(launchInput({ opaque: currentArray }));
  assert.equal(arrayChecked.ok, false);
  assert.equal(arrayChecked.reason_code, 'CURRENT_ADMISSION_INVALID');
  assert.equal(getterRuns, 0);

  const revoked = Proxy.revocable(launchInput(), {});
  revoked.revoke();
  assert.doesNotThrow(() => programme.validateCurrentLaunchSafety(revoked.proxy));
  assert.equal(programme.validateCurrentLaunchSafety(revoked.proxy).ok, false);
});
