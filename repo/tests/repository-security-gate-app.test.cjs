...
test('required-check App authority adversarial fixture matrix', async (t) => {
  ...
  await t.test('scheduled recovery resumes an immutable publication set after completed-run processing faults mid-publication', async () => {
    const state = new app.MemoryRunState();
    const github = fakeGithub();
    const startedAt = Date.parse('2026-07-27T00:00:00.000Z');
    const headSha = '1'.repeat(40);
    const authoritySha = '3'.repeat(40);
    const correlationId = 'correlation-partial-publication';
    const attempt = await state.beginAttempt(`1/2/${headSha}`, correlationId, {
      repository_id: 1,
      installation_id: 2,
      repository: 'synthetic/toolkit',
      ...
      authority_sha: authoritySha,
      nonce: 'abcdefghijklmnopqrstuv',
      delivery_id: 'delivery-partial-publication',
      envelope_digest: `sha256:${'4'.repeat(64)}`,
      default_branch: 'main',
      expires_at: new Date(startedAt + 10 * 60 * 1000).toISOString()
    }, startedAt);
    ...
    const contexts = Object.fromEntries(
      Object.keys(app.CHECK_CONTEXTS).map((contextId, index) => [contextId, {
        conclusion: 'failure',
        summary: 'Synthetic immutable terminal outcome.',
        terminalDigest: `sha256:${String(index + 5).repeat(64)}`
      }])
    );
    const result = await app.recoverExpiredDispatches({
      APP_ID: '1',
      ...
      processCompletedRun: async () => {
        await state.sealPublicationSet(correlationId, {
          attempt_generation: attempt.generation,
          contexts
        });
        const current = state.data.correlations.get(correlationId);
        await state.transitionCorrelation(correlationId, 'dispatch_unknown', {
          ...current,
          state: 'publishing',
          workflow_run_id: 9002
        });
        const firstContextId = Object.keys(app.CHECK_CONTEXTS)[0];
        const firstContext = contexts[firstContextId];
        await app.publishRequiredCheck({
          github,
          state,
          integrationId: 77,
          ...
          status: 'completed',
          conclusion: firstContext.conclusion,
          summary: firstContext.summary,
          terminalDigest: firstContext.terminalDigest,
        ...
        await state.markPublicationContext(
          correlationId,
          firstContextId,
          firstContext.terminalDigest
        );
        throw new Error('TK023_SYNTHETIC_PARTIAL_PUBLICATION');
      }
    });
    assert.deepEqual(result, { ok: true, reconciled: 1, terminalized: 0 });
    assert.equal(state.data.correlations.get(correlationId).state, 'completed');
    const publication = await state.getPublicationSet(correlationId);
    assert.equal(publication.state, 'published');
    assert.deepEqual(publication.contexts, contexts);
    assert.ok(Object.values(publication.progress).every((progress) => progress === 'published'));
    assert.equal(state.data.checks.size, 3);
    assert.ok([...state.data.checks.values()].every((check) =>
      check.state === 'completed' && check.conclusion === 'failure'
