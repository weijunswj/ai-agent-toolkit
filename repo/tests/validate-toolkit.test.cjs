...
test('Toolkit plugin packaged version surfaces stay aligned', () => {
  ...
  fs.writeFileSync(
    bridgePath,
    readTextFile(bridgePath).replace("const BRIDGE_VERSION = '2.9.24';", "const BRIDGE_VERSION = '2.2.1';"),
    'utf8'
  );

  const result = runValidate(cwd);
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /BRIDGE_VERSION must match Toolkit Local Bridge project version 2\.9\.24/i);
});
