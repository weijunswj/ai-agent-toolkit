...
function isolatedHomeEnv(root) {
  ...
  return {
    ...
    ...(process.platform === 'win32' ? {
      ...
      PSModulePath: process.env.PSModulePath || process.env.PSMODULEPATH || '',
    } : {}),
    USERPROFILE: root,
    HOME: root,
...
function createMinimalSetupRepo(root, options = {}) {
  ...
  writeFile(path.join(root, '.claude-plugin', 'plugin.json'), JSON.stringify({
    name: 'ai-agent-toolkit', version: '2.10.10', skills: './skills', hooks: './.claude-plugin/hooks/hooks.json'
  }, null, 2));
  writeFile(path.join(root, '.codex-plugin', 'plugin.json'), JSON.stringify({
    name: 'ai-agent-toolkit', version: '2.10.10', hooks: './.codex-plugin/hooks/hooks.json'
  }, null, 2));
