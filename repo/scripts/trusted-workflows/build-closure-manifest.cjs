...
function createClosureParser(trustedRoot, repoRoot) {
  if (!trustedRoot || typeof trustedRoot !== 'string') die('TW_CLOSURE_ARGUMENTS', 'trustedRoot required');
  if (!repoRoot || typeof repoRoot !== 'string') repoRoot = trustedRoot;

  function relative(absolute) {
    return path.relative(repoRoot, absolute).replace(/\\/g, '/');
  }

  function assertCanonicalLF(file) {
    const bytes = fs.readFileSync(file);
    if (bytes.indexOf(0x0d) !== -1) die('TW_CLOSURE_NON_CANONICAL_LF', relative(file));
  }

  function hash(file) {
    assertCanonicalLF(file);
    const bytes = fs.readFileSync(file);
  ...
  function resolveLiteral(fromFile, specifier) {
    ...
    const rel = path.relative(trustedRoot, absolute);
  ...
  function parseDependencies(file) {
    ...
    walk(ast, (node) => {
      ...
      if (node.type === 'CallExpression' && isDirectRequireCallee(node) &&
      ...
      if (isDirectRequireCallee(node)) {
        if (!isValidStaticRequireCall(node)) die('TW_CLOSURE_COMPUTED_REQUIRE', relative(file));
  ...
  return { parseDependencies, build, collectFiles, hash, resolveLiteral, assertCanonicalLF };
}

function main() {
  const mode = process.argv[2];
  if (!['--write', '--check'].includes(mode) || process.argv.length !== 3) die('TW_CLOSURE_ARGUMENTS');
  const parser = createClosureParser(TRUSTED_ROOT, REPO_ROOT);
  const expected = JSON.stringify(parser.build(), null, 2) + '\n';
  if (mode === '--write') {
...
const productionParser = createClosureParser(TRUSTED_ROOT, REPO_ROOT);
module.exports = {
  build: productionParser.build,
  parseDependencies: productionParser.parseDependencies,
  createClosureParser
};
