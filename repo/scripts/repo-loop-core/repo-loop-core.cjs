...
function credentialFreeRemoteUrl(value) {
  ...
  try {
    ...
    const sensitiveQuery = [...parsed.searchParams.keys()].some((key) => /^(?:access[_-]?token|auth|credential|password|pat|secret|token)$/i.test(key));
    if (parsed.username || parsed.password || sensitiveQuery) fail('CREDENTIAL_BEARING_REMOTE_URL');
  } catch (error) {
