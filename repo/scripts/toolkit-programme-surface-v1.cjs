...
const PROGRAMME_421_CURRENT_STRUCTURAL_ADMISSION = Object.freeze({
  schema: 'toolkit.controller.structural-admission.v1',
  version: 1,
  repository: 'weijunswj/ai-agent-toolkit',
  parent_issue: 421,
  child_issue: 435,
  outcome_id: 'C1',
  source: 'programme-421-proof-v1',
  source_digest: PROGRAMME_421_PROOF_FIELD_MAP_DIGEST,
});
...
function workerAliases(value, aliases) {
  if (!isRecord(value)) return { ok: true, present: [], value: undefined };
  const present = aliases.filter((key) => Object.prototype.hasOwnProperty.call(value, key));
  if (present.length > 1) return { ok: false, present, value: undefined };
  return { ok: true, present, value: present.length === 1 ? value[present[0]] : undefined };
}

function workerInputParts(input = {}) {
  const source = isRecord(input) ? input : {};
  const inFlightAlias = workerAliases(source, ['in_flight', 'inFlight']);
  const nested = inFlightAlias.ok && inFlightAlias.present.length === 1
    ? inFlightAlias.value === true ? { state: 'RUNNING' } : isRecord(inFlightAlias.value) ? inFlightAlias.value : {}
    : {};
  const run = isRecord(source.run) ? source.run : {};
  const packetAlias = workerAliases(source, ['terminal_packet', 'terminalPacket', 'packet']);
  const nestedLivenessAliases = ['worker_liveness', 'workerLiveness', 'liveness'];
  const nestedLiveness = workerAliases(nested, nestedLivenessAliases);
  const sourceLiveness = workerAliases(source, nestedLivenessAliases);
  const nestedRun = workerAliases(run, ['worker_identity', 'workerIdentity', 'worker_id']);
  const workerFields = (aliases) => {
    const sources = [workerAliases(source, aliases), workerAliases(nested, aliases), workerAliases(run, aliases)];
    const selected = sources.filter((item) => item.present.length > 0);
    if (sources.some((item) => !item.ok) || selected.length > 1) return { ok: false, value: undefined };
    return { ok: true, value: selected.length === 1 ? selected[0].value : undefined };
  };
  const livenessField = workerFields(nestedLivenessAliases);
  const refreshedField = workerFields(['worker_liveness_refreshed', 'workerLivenessRefreshed', 'liveness_refreshed', 'worker_liveness_current', 'workerLivenessCurrent']);
  const terminalField = workerFields(['worker_terminal_state', 'workerTerminalState', 'terminal_worker_state', 'terminal_state']);
  const launchField = workerFields(['worker_launch_evidence', 'workerLaunchEvidence']);
  const identityField = workerFields(['worker_identity', 'workerIdentity', 'previous_worker_identity', 'previousWorkerIdentity', 'worker']);
  const evidenceField = workerFields(['worker_liveness_evidence', 'workerLivenessEvidence', 'liveness_evidence']);
  const launchOutcomeField = workerFields(['launch_outcome', 'launchOutcome']);
  const invalidAuthority = !inFlightAlias.ok || !packetAlias.ok || !sourceLiveness.ok || !nestedLiveness.ok
    || !nestedRun.ok || !livenessField.ok || !refreshedField.ok || !terminalField.ok
    || !launchField.ok || !identityField.ok || !evidenceField.ok || !launchOutcomeField.ok;
  const packet = packetAlias.ok && packetAlias.present.length === 1 ? packetAlias.value : null;
  const livenessValue = livenessField.value;
  const livenessObject = isRecord(livenessValue) ? livenessValue : {};
  const refreshed = firstValue(refreshedField.value, livenessObject.refreshed, livenessObject.current);
  const terminalValue = firstValue(terminalField.value, livenessObject.terminal_state);
  const rawLiveness = normalizeWorkerLiveness(livenessValue);
  const terminalState = normalizeWorkerTerminalState(terminalValue);
  const runState = typeof run.state === 'string' ? run.state.toUpperCase() : '';
  const completedRun = ['COMPLETED', 'DONE', 'FINISHED', 'EXITED'].includes(runState)
    || source.completed === true || run.completed === true || nested.completed === true;
  const runStateInvalid = Object.prototype.hasOwnProperty.call(run, 'state')
    && !['NONE', 'QUEUED', 'IN_FLIGHT', 'TERMINAL', 'HELD', 'COMPLETED', 'DONE', 'FINISHED', 'EXITED'].includes(runState);
  const terminalEvidence = source.worker_terminal === true
    || source.workerTerminal === true
    || ['TERMINAL', 'COMPLETED', 'DONE', 'FINISHED', 'EXITED'].includes(runState)
    || isRecord(packet)
    || terminalState !== null
    || rawLiveness === 'terminal';
  const launchEvidence = normalizeWorkerLaunchEvidence(launchField.value);
  const identity = normalizeWorkerIdentity(firstValue(identityField.value, nested.worker));
  const evidence = normalizeWorkerLivenessEvidence(firstValue(evidenceField.value, livenessObject.source));
  const launchOutcome = String(launchOutcomeField.value || '').toUpperCase();
  ...
  const explicitLiveness = livenessField.value !== undefined;
  const refreshInvalidated = refreshed === false;
  let liveness = rawLiveness;
  let current = rawLiveness !== null && !refreshInvalidated;
  let currentEvidence = evidence;
  if (invalidAuthority || runStateInvalid) {
    liveness = 'unknown';
    current = false;
    ...
    if (!activeClaim && !identity && String(nested.state || '').toUpperCase() !== 'RUNNING' && !['IN_FLIGHT', 'QUEUED', 'HELD'].includes(runState)) {
      liveness = 'inactive';
      current = true;
    } else {
      liveness = 'unknown';
  ...
  if (explicitLiveness && liveness === 'inactive' && current && currentEvidence !== 'runtime-native') {
  ...
  return {
    ...
    livenessUnverified: invalidAuthority || runStateInvalid || liveness === 'unknown' && (explicitLiveness || activeClaim || runState !== ''),
  };
}
...
function currentStructuralEligibility(input) {
  const source = isRecord(input) ? input : {};
  if (Object.prototype.hasOwnProperty.call(source, 'trusted_structural_eligibility')) return false;
  const aliases = ['structural_admission', 'structuralAdmission'].filter((key) => Object.prototype.hasOwnProperty.call(source, key));
  if (aliases.length !== 1) return false;
  const structural = source[aliases[0]];
  if (!isRecord(structural) || !exactKeys(structural, Object.keys(PROGRAMME_421_CURRENT_STRUCTURAL_ADMISSION))) return false;
  if (kernel.canonicalSerialize(structural) !== kernel.canonicalSerialize(PROGRAMME_421_CURRENT_STRUCTURAL_ADMISSION)) return false;
  const programme = isRecord(source.programme) ? source.programme : {};
  return source.repository === PROGRAMME_421_CURRENT_STRUCTURAL_ADMISSION.repository
    && graphIssue(programme.parent ?? programme.issue ?? source.parent_issue) === PROGRAMME_421_CURRENT_STRUCTURAL_ADMISSION.parent_issue
    && graphIssue(programme.current_child ?? source.current_child) === PROGRAMME_421_CURRENT_STRUCTURAL_ADMISSION.child_issue
    && (programme.lane ?? source.lane) === 'C1';
}
...
function rawGraphAliasCheck(value, aliases) {
  if (!isRecord(value)) return { ok: true, present: [] };
  const present = aliases.filter((key) => Object.prototype.hasOwnProperty.call(value, key));
  return { ok: present.length <= 1, present };
}

function rawProgrammeGraphPreflight(input) {
  if (!isRecord(input)) return false;
  const programmeAliases = ['programme', 'parent'].filter((key) => Object.prototype.hasOwnProperty.call(input, key));
  if (programmeAliases.length > 1) return false;
  const programme = programmeAliases.length === 1 ? input[programmeAliases[0]] : null;
  if (programme !== null && !isRecord(programme)) return false;

  const outcomeSources = [
    ['root', ['outcomes', 'registered_outcomes'], input],
    ['programme', ['outcomes'], programme],
  ].filter(([, aliases, source]) => isRecord(source) && aliases.some((key) => Object.prototype.hasOwnProperty.call(source, key)));
  if (outcomeSources.length > 1) return false;
  for (const [, aliases, source] of outcomeSources) {
    const checked = rawGraphAliasCheck(source, aliases);
    if (!checked.ok) return false;
    const key = checked.present[0];
    if (key !== undefined && !Array.isArray(source[key])) return false;
  }
  const rawOutcomes = outcomeSources.length === 0
    ? []
    : outcomeSources[0][2][outcomeSources[0][1].find((key) => Object.prototype.hasOwnProperty.call(outcomeSources[0][2], key))];
  if (!Array.isArray(rawOutcomes)) return outcomeSources.length === 0;

  const authorityAliases = [
    ['id', 'outcome_id', 'outcomeId'],
    ['native_issue', 'nativeIssue', 'issue'],
    ['delivery_pr', 'deliveryPr', 'delivery_pr_pointer', 'pr'],
    ['current_gate', 'currentGate'],
    ['execution_eligible', 'executionEligible'],
    ['complete_when', 'completeWhen', 'done_when'],
    ['title', 'name'],
    ['dependencies'],
    ['kind'],
    ['materialized'],
    ...
    ['candidate'],
    ['run'],
    ['next_admissible_action', 'nextAction'],
  ];
  for (const raw of rawOutcomes) {
    if (!isRecord(raw)) return false;
    if (authorityAliases.some((aliases) => !rawGraphAliasCheck(raw, aliases).ok)) return false;
  }
  return true;
}
  ...
  for (const raw of outcomes) {
    ...
    if (!expected.execution_eligible) {
      if (deliveryAliases.some((key) => Object.prototype.hasOwnProperty.call(raw, key) && raw[key] !== null && raw[key] !== undefined)
        ...
        || ['CURRENT', 'HELD', 'COMPLETED'].includes(String(raw.lifecycle || '').toUpperCase())
        || raw.candidate !== undefined || raw.run !== undefined
        || raw.next_admissible_action !== undefined || raw.nextAction !== undefined) {
...
function normalizeProgrammeGraphSnapshotInternal(input = {}, options = {}) {
  ...
  if (!rawProgrammeGraphPreflight(input)) return result(false, 'PROGRAMME_GRAPH_STRUCTURAL_CONTRADICTION', { field: 'raw_authority' });
...
function publicGraphArraySafe(value, key, seen, input) {
  ...
  for (const field of keys) {
    if (field !== 'length' && (typeof field !== 'string' || !/^(?:0|[1-9][0-9]*)$/.test(field) || Number(field) >= lengthDescriptor.value)) return false;
  }
  seen.add(value);
  let valid = true;
  for (let index = 0; index < lengthDescriptor.value; index += 1) {
    let descriptor;
    try { descriptor = Object.getOwnPropertyDescriptor(value, String(index)); } catch (_error) { valid = false; break; }
    if (!descriptor || !Object.prototype.hasOwnProperty.call(descriptor, 'value')) {
      valid = false;
      break;
    }
    if (!(input
      ? publicGraphInputSafe(descriptor.value, key, seen)
      : publicGraphFieldsSafe(descriptor.value, key, seen))) {
      valid = false;
      break;
    }
  }
...
function publicGraphFieldsSafe(value, key = '', seen = new Set()) {
  if (typeof value === 'string') return publicGraphTextSafe(value, key === 'body');
  if (value === null || typeof value === 'boolean') return true;
  if (typeof value === 'number') return Number.isFinite(value);
  let array;
  try { array = Array.isArray(value); } catch (_error) { return false; }
  if (array) return publicGraphArraySafe(value, key, seen, false);
  if (!isRecord(value)) return false;
  if (seen.has(value)) return false;
  let prototype;
  let keys;
  ...
  let valid = true;
  for (const field of keys) {
    if (typeof field !== 'string' || PUBLIC_GRAPH_SENSITIVE_KEY.test(field)) { valid = false; break; }
    let descriptor;
    try { descriptor = Object.getOwnPropertyDescriptor(value, field); } catch (_error) { valid = false; break; }
    if (!descriptor || !Object.prototype.hasOwnProperty.call(descriptor, 'value')
      || !publicGraphFieldsSafe(descriptor.value, field, seen)) { valid = false; break; }
  }
  seen.delete(value);
  return valid;
}

function publicGraphInputSafe(value, key = '', seen = new Set()) {
  ...
  let array;
  try { array = Array.isArray(value); } catch (_error) { return false; }
  if (array) return publicGraphArraySafe(value, key, seen, true);
  if (!isRecord(value)) return false;
  if (seen.has(value)) return false;
  let prototype;
  let keys;
  ...
  for (const field of keys) {
    ...
    try { descriptor = Object.getOwnPropertyDescriptor(value, field); } catch (_error) { valid = false; break; }
    if (!descriptor || !Object.prototype.hasOwnProperty.call(descriptor, 'value')
      || !publicGraphInputSafe(descriptor.value, field, seen)) { valid = false; break; }
  }
  seen.delete(value);
  return valid;
}
...
module.exports = Object.freeze({
  ...
  PROGRAMME_421_CURRENT_STRUCTURAL_ADMISSION,
