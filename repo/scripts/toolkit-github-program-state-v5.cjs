...
/*
 * Root-003 human surface.
 *
 * The v5 exports above are retained for the frozen E3 fixtures.  This section
 * deliberately does not add public parser, codec, graph, or authority
 * helpers.  The only new public entry point is the four-operation facade at
 * the end of this section.
 */
const H2_ROOT = 'S2-PRE-E4-HUMAN-SURFACE-TRUST-BOUNDARY-SIMPLIFICATION-003';
const H2_LOCK = 'DL-S2-PRE-E4-HUMAN-SURFACE-TRUST-BOUNDARY-SIMPLIFICATION-003';
...
const H2_PR_PROJECTION_SCHEMA = 'github.program.pr-projection.v2';
...
function h2PrProse(descriptor, authority) {
  const candidate = descriptor.candidate;
  const numberState = authority ? 'BOUND' : 'PRE_NUMBER';
  const number = authority ? authority.pr_number : 'pending provider assignment';
  const currentNextAction = authority
    ? 'Continue only under the bound controller authority.'
    : descriptor.next_action_pre_number;
  const lines = [
    ...
    '| Field | Value |',
    '| --- | --- |',
    '| Repository | ' + h2Cell(descriptor.repository) + ' |',
    '| Parent | #' + h2Identifier(descriptor.parent_issue) + ' |',
    '| Child | #' + h2Identifier(descriptor.child_issue) + ' |',
    '| Epoch | ' + h2Cell(descriptor.epoch_id) + ' |',
    '| Gate | ' + h2Cell(descriptor.gate) + ' |',
    '| Role | ' + h2Cell(descriptor.role) + ' |',
    '| Completes child | ' + h2Cell(String(descriptor.completes_child)) + ' |',
    '| Number state | ' + h2Cell(numberState) + ' |',
    '| PR number | ' + (authority ? '#' + h2Identifier(number) : h2Cell(number)) + ' |',
    '',
    ...
    '',
    '## ' + (authority ? 'PRE_NUMBER descriptor constraints' : 'Design constraints'), ...h2LinesForArray(descriptor.design_constraints),
    '',
    ...
    '| Field | Value |',
    '| --- | --- |',
    '| Repository | ' + h2Lineage(candidate.repository, 'repository') + ' |',
    '| Branch | ' + h2Lineage(candidate.branch, 'branch') + ' |',
    '| Base ref | ' + h2Lineage(candidate.base_ref, 'base_ref') + ' |',
    '| Base SHA | ' + h2Lineage(candidate.base_sha, 'base_sha') + ' |',
    '| Head | ' + h2Lineage(candidate.head, 'head') + ' |',
    '| Tree | ' + h2Lineage(candidate.tree, 'tree') + ' |',
    '| Version | ' + h2Lineage(candidate.version, 'version') + ' |',
    '',
    '## History and recovery', ...h2LinesForArray([...descriptor.repair_history, ...descriptor.before_after, ...descriptor.repair_budget, ...descriptor.hosted_qualification, ...descriptor.recovery_evidence]),
    '',
    '## What happens next', h2Bullet(currentNextAction),
    '',
    '## ELI5', h2Paragraph(descriptor.eli5),
  ];
  return lines;
}
...
function h2BuildPrDoc(descriptor, authority) {
  ...
  let bound = null;
  if (authority !== null) bound = h2BoundAuthority(authority, normalized, 'AUTHORITY');
  const style = h2MarkerStyleForRepository(normalized.repository);
  const projection = {
    schema: H2_PR_PROJECTION_SCHEMA,
    version: H2_VERSION,
    kind: 'pr',
  ...
  const carrier = {
    ...
    pr_number: bound ? bound.pr_number : null,
    number_state: bound ? 'BOUND' : 'PRE_NUMBER',
    descriptor: { schema: H2_DESCRIPTOR_SCHEMA, digest: h2Digest(normalized) },
    candidate: { digest: h2Digest(normalized.candidate) },
    authority_sha256: bound ? bound.authority_sha256 : null,
    projection: { schema: H2_PR_PROJECTION_SCHEMA, digest: h2Digest(projection) },
    public_prose_sha256: null,
  };
  const prose = h2PrProse(normalized, bound);
...
function h2ValidateCarrierShape(carrier, kind) {
  ...
  if (kind === 'parent') {
    ...
    h2Require(h2Exact(carrier.descriptor, ['schema', 'digest']) && carrier.descriptor.schema === H2_DESCRIPTOR_SCHEMA && h2Hash(carrier.descriptor.digest)
      ...
      && h2Exact(carrier.projection, ['schema', 'digest']) && carrier.projection.schema === H2_PR_PROJECTION_SCHEMA && h2Hash(carrier.projection.digest), true, 'CARRIER_INVALID', 'PARSE');
...
function h2ParsePrEnvelope(envelope, descriptor, authority) {
  ...
  h2Require(parts.carrier.repository === normalized.repository
    && parts.carrier.number_state === (bound ? 'BOUND' : 'PRE_NUMBER')
    && parts.carrier.pr_number === (bound ? bound.pr_number : null)
    && parts.carrier.descriptor.digest === h2Digest(normalized)
    && parts.carrier.candidate.digest === h2Digest(normalized.candidate)
    && parts.carrier.authority_sha256 === (bound ? bound.authority_sha256 : null)
    && parts.carrier.projection.digest === h2Digest(expectedDoc.projection), true, 'AUTHORITY_BINDING_MISMATCH', 'AUTHORITY');
  h2Require(expectedDoc.body === envelope.read.body && h2Comparable(expectedDoc.carrier, parts.carrier), true, 'READBACK_MISMATCH', 'READBACK');
  return h2ParsedResult(envelope, 'pr', { descriptor: normalized, bound_authority: bound, projection: expectedDoc.projection, projection_sha256: expectedDoc.projection_sha256, carrier: parts.carrier, pr_number: bound ? bound.pr_number : null, number_state: bound ? 'BOUND' : 'PRE_NUMBER' });
}
...
function h2ReadCompletePublic(input) {
  return h2Call('readComplete', () => {
    ...
    if (parsed.projection) result.projection = h2Clone(parsed.projection);
    if (parsed.descriptor) result.descriptor = h2Clone(parsed.descriptor);
