...
function humanPrProse(descriptor, numberBinding, candidate) {
  ...
  const next = descriptor.next_action || (numberBinding.number_state === 'PRE_NUMBER' ? 'Return the exact PRE_NUMBER body for controller adjudication.' : 'Continue only under the bound controller authority.');
