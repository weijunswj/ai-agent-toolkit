...
function detectGOV005(repo, issues, findings, subjects) {
  ...
  for (const child of children) {
    ...
    if (!parentIssue) {
      emitFinding(findings, 'GOV005', getSubjectForIssue(subjects, child.id), 'parent_not_found', {});
      continue;
    }
    ...
    if (!inBody) {
      emitFinding(findings, 'GOV005', getSubjectForIssue(subjects, child.id), 'child_absent_from_parent_checklist', {});
    }
  }
}
