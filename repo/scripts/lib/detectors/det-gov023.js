...
function detectGOV023(repo, issues, findings, subjects) {
  ...
  for (const issue of issues) {
    ...
    if (!bodyPR && !bodyBranch) {
      emitFinding(findings, 'GOV023', getSubjectForIssue(subjects, issue.id), 'missing_body_implementation_pr', {});
      continue;
    }
    ...
    if (issue.implementation_branch !== undefined && issue.implementation_branch !== null) {
      if (bodyBranch && issue.implementation_branch !== bodyBranch) {
        emitFinding(findings, 'GOV023', getSubjectForIssue(subjects, issue.id), 'branch_disagree', {});
      }
    }
    ...
    if (activePrs.length === 1) {
      ...
      if (bodyPR && bodyPR !== 'Not opened') {
        ...
        if (bodyPRNum && String(soleOpenPR.number) !== bodyPRNum) {
          emitFinding(findings, 'GOV023', getSubjectForIssue(subjects, issue.id), 'pr_disagree', {});
        } else if (!bodyPRNum) {
          emitFinding(findings, 'GOV023', getSubjectForIssue(subjects, issue.id), 'pr_disagree', {});
        }
      }
      if (bodyPR === 'Not opened') {
        emitFinding(findings, 'GOV023', getSubjectForIssue(subjects, issue.id), 'body_not_opened_but_pr_exists', {});
      }
    }

    if (bodyPR && bodyPR !== 'Not opened' && activePrs.length === 0) {
      const bodyPRNum = bodyPR.replace(/[^0-9]/g, '');
      const hasMatchingClosed = implPrs.some(pr => String(pr.number) === bodyPRNum && !activeStates.includes(pr.state));
      if (!hasMatchingClosed) {
        emitFinding(findings, 'GOV023', getSubjectForIssue(subjects, issue.id), 'body_pr_not_in_structured', {});
      }
    }
  }
}
