...
function takeDelegationAnswerFromInput(parsed, input) {
  ...
  if (position >= lines.length) return { choice: 'keep', remainingInput: String(input || '') };
...
function shouldDelegateToManagedSetup(parsed) {
  if (!parsed.execute || parsed.repoRootExplicit) return false;
  const managedRoot = core.defaultManagedSourcePath();
  const managedScript = path.join(managedRoot, 'repo', 'scripts', 'setup-toolkit.cjs');
  const activeRoot = path.resolve(__dirname, '..', '..');
  return fs.existsSync(managedScript) && !samePath(activeRoot, managedRoot);
}
...
function secureSetupPlan(options = {}) {
  ...
  if (delegationStep) {
    ...
    delegationStep.commands = selected === 'limit'
      ? ['Preview, back up, and transactionally set agents.max_threads=1 and agents.max_depth=1 through the official Codex config editor.']
      : [];
...
async function secureApplyHostDelegationControl(args, current, options = {}) {
  if (args.host !== 'codex') {
    return {
      status: 'unsupported',
  ...
  return await delegation.delegationResultForChoice(choice, configPath, options);
}
...
async function main(argv = process.argv.slice(2)) {
  ...
  if (shouldDelegateToManagedSetup(parsed)) return core.main(argv);
  ...
  if (choice === 'limit') {
    ...
    result = await delegation.delegationResultForChoice('limit', current.config_path, {
      codexCommand: parsed.codexCli,
      codexHome: delegation.defaultCodexHome(),
    });
  } else {
    result = await delegation.delegationResultForChoice(choice, current.config_path);
  }
