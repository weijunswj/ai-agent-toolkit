...
const expectedFiles = [
  ...
  'repo/docs/agent-playbooks/docs-governance.md',
  ...
  'repo/docs/agent-playbooks/windows-command-hygiene.md',
  ...
  'skills/ai-coding-agent-rules/repo-local/docs/agent-playbooks/docs-governance.md',
  ...
  'skills/ai-coding-agent-rules/repo-local/docs/agent-playbooks/windows-command-hygiene.md',
  ...
  '_projects/development/ai-coding-agent-rules/_main/repo-local/docs/agent-playbooks/docs-governance.md',
  ...
  '_projects/development/ai-coding-agent-rules/_main/repo-local/docs/agent-playbooks/windows-command-hygiene.md',
...
const sourceWatchPrNotificationRule = 'Stage A source-watch is proposal-only. Scheduled or manual dry-runs may compare active SOURCE-LOCK pins and actionable advisory targets with upstream GitHub commits, then emit deterministic review evidence. They must not create or update a branch, commit, push, create or edit a PR, mutate an issue, copy upstream files, change SOURCE-LOCK/advisory records, execute upstream code, auto-merge, push to main, run live n8n actions, or treat notification as approval. Privileged publication requires a separate activation issue, reviewed PR, bounded canary and controller acceptance.';
...
function validateAutoSyncGeneratedSurfacesWorkflow(entry, text, errors) {
  ...
  if (permissions.length !== expectedPermissions.length || expectedPermissions.some((permission) => !permissions.includes(permission))) {
    fail(errors, `${entry.relPath} must grant only contents: read and pull-requests: read`);
  }
  if (!/^\s{2}pull_request_target:\s*$/m.test(text)) fail(errors, `${entry.relPath} must use pull_request_target`);
  if (!/^\s{2}workflow_dispatch:\s*$/m.test(text)) fail(errors, `${entry.relPath} must provide canonical-main Stage A dry-run dispatch`);
  for (const trigger of ['pull_request:', 'push:', 'schedule:', 'workflow_run:']) {
    if (new RegExp('^\\s{2}' + trigger.replace(':', '\\:') + '\\s*$', 'm').test(text)) fail(errors, `${entry.relPath} contains forbidden Stage A trigger ${trigger}`);
  }
  for (const value of ['ROLLOUT_STAGE: "A"', 'WRITEBACK_ENABLED: "false"', 'GENERAL_ENABLED: "false"']) {
    if (!text.includes(value)) fail(errors, `${entry.relPath} missing inert policy value ${value}`);
  }
  ...
  if ((text.split(checkoutPin).length - 1) !== 2 || (text.split(setupPin).length - 1) !== 1) {
    fail(errors, `${entry.relPath} external actions must match the reviewed full-SHA manifest`);
  }
  if (!/package-manager-cache:\s*false/.test(text)) fail(errors, `${entry.relPath} setup-node must disable package-manager cache`);
  if (/persist-credentials:\s*true/i.test(text) || (text.match(/persist-credentials:\s*false/g) || []).length !== 2) {
    fail(errors, `${entry.relPath} checkouts must not persist credentials`);
  }
  if (packageManagerCommandPattern.test(text)) fail(errors, `${entry.relPath} must not install packages`);
  if (!text.includes('/usr/bin/sha256sum --binary -- repo/scripts/trusted-workflows/capture-node-toolchain.cjs') ||
      !text.includes('/usr/bin/sha256sum --binary -- repo/scripts/trusted-workflows/verify-closure-manifest.cjs')) {
...
function validateSourceWatchPrWorkflow(entry, text, errors) {
  ...
  if (!/^\s{2}schedule:\s*$/m.test(text) || !/^\s{2}workflow_dispatch:\s*$/m.test(text)) {
    fail(errors, `${entry.relPath} must expose schedule and manual Stage A dry-run triggers`);
  }
  for (const value of [
    'ROLLOUT_STAGE: "A"', 'PUBLICATION_MODE: "dry-run"', 'SCHEDULED_WRITE_ENABLED: "false"',
    'MANUAL_CANARY_ENABLED: "false"', 'GENERAL_PUBLICATION_ENABLED: "false"'
  ]) if (!text.includes(value)) fail(errors, `${entry.relPath} missing inert policy value ${value}`);
  const checkoutPin = 'actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1';
  const setupPin = 'actions/setup-node@820762786026740c76f36085b0efc47a31fe5020';
  if ((text.split(checkoutPin).length - 1) !== 1 || (text.split(setupPin).length - 1) !== 1) {
    fail(errors, `${entry.relPath} external actions must match the reviewed full-SHA manifest`);
  }
  if (!/package-manager-cache:\s*false/.test(text) || !/persist-credentials:\s*false/.test(text)) {
    fail(errors, `${entry.relPath} must disable cache and persisted credentials`);
  }
  if (packageManagerCommandPattern.test(text)) fail(errors, `${entry.relPath} must not install packages`);
  if (/github\.token|GH_TOKEN|contents:\s*write|pull-requests:\s*write|git\s+(?:commit|push|switch|reset|rebase)|gh\s+pr\s+(?:create|edit|close)/i.test(text)) {
    fail(errors, `${entry.relPath} Stage A must contain no write-capable publication edge`);
  }
}
