...
function validActivationProof(proof, expected = {}) {
  const structurallyValid = Boolean(proof && proof.schema === 4 && proof.source === 'claude-plugin-list'
    && proof.plugin_version === CONTROL_VERSION
    && ['cache_identity', 'hook_sha256', 'controller_sha256', 'process_launch_sha256', 'agent_hook_sha256', 'n8n_admission_hook_sha256', 'n8n_domain_router_sha256']
...
function verifyCurrentClaudeEnforcement(profile, options = {}) {
  ...
  return Boolean(current?.verified === true && validActivationProof(current.activation_proof)
    && ['schema', 'source', 'plugin_version', 'cache_identity', 'hook_sha256', 'controller_sha256', 'process_launch_sha256', 'agent_hook_sha256', 'n8n_admission_hook_sha256', 'n8n_domain_router_sha256']
