...
function trustedDataValue(value, seen = new Set()) {
  if (value === undefined || value === null || typeof value === 'string' || typeof value === 'boolean' || typeof value === 'number') {
    return { ok: Number.isNaN(value) === false, value };
  }
  ...
  if (Array.isArray(value)) {
    ...
    for (const item of value) {
      const checked = trustedDataValue(item, nextSeen);
...
function trustedAliasValues(value, aliases) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return { ok: false, reason: 'INPUT_NOT_OBJECT' };
  const values = {};
  try {
