...
function readA2Evidence(options = {}) {
  const checked = authorityDataBoundary(options, ['a2_accepted', 'a2Accepted']);
  ...
  return typeof checked.values[aliases[0]] === 'boolean'
    ? { ok: true, accepted: checked.values[aliases[0]] }
    : { ok: false };
}
...
function normalizeHarness(options = {}) {
  const checked = authorityDataBoundary(options, ['harness_identity', 'verified_harness', 'harness', 'harness_verified', 'harness_source']);
  if (!checked.ok) return { name: 'unknown', verified: false, source: 'unavailable', invalid: true };
  const values = checked.values;
  const raw = values.harness_identity ?? values.verified_harness ?? values.harness ?? null;
  if (typeof raw === 'string') {
    return {
      name: ['claude-code', 'codex', 'opencode'].includes(raw) ? raw : 'unknown',
      verified: values.harness_verified === true,
      source: ['hook', 'adapter', 'runtime'].includes(values.harness_source) ? values.harness_source : 'adapter',
    };
  }
  if (isRecord(raw)) {
    const nested = authorityDataBoundary(raw, ['name', 'verified', 'source']);
    if (!nested.ok) return { name: 'unknown', verified: false, source: 'unavailable', invalid: true };
    return {
      name: ['claude-code', 'codex', 'opencode'].includes(nested.values.name) ? nested.values.name : 'unknown',
      verified: nested.values.verified === true,
      source: ['hook', 'adapter', 'runtime'].includes(nested.values.source) ? nested.values.source : 'unavailable',
    };
  }
...
function explicitStackSelection(options) {
  ...
  const checked = authorityDataBoundary(options, keys);
  if (!checked.ok) return { present: true, value: null, invalid: true, reason: checked.reason };
  const present = Object.keys(checked.values);
  if (present.length === 0) return { present: false, value: null };
  if (present.length > 1) return { present: true, value: null, invalid: true, reason: 'STACK_SELECTION_CONFLICT' };
  const raw = checked.values[present[0]];
  if (typeof raw === 'string') return { present: true, value: raw };
  if (isRecord(raw)) {
    const nested = authorityDataBoundary(raw, ['stack_id', 'trusted', 'selection_source']);
    if (nested.ok && typeof nested.values.stack_id === 'string'
      && (nested.values.trusted === undefined || nested.values.trusted === true)
      && (nested.values.selection_source === undefined || nested.values.selection_source === 'explicit_trusted_stack')) {
      return { present: true, value: nested.values.stack_id };
    }
  }
...
function resolveRegistryIdentity(loaded, options = {}) {
  ...
  const suppliedChecked = supplied === null ? { ok: true, values: {} } : authorityDataBoundary(supplied, ['revision', 'digest']);
  if (!suppliedChecked.ok) return result(false, 'REGISTRY_IDENTITY_MISMATCH');
  const trustedSupplied = supplied === null ? null : suppliedChecked.values;
  const revision = options.registry_revision || options.registryRevision || trustedSupplied?.revision || null;
  ...
  if (supplied !== null && (!isRecord(trustedSupplied) || !exactKeys(trustedSupplied, ['revision', 'digest'])
    || trustedSupplied.revision !== identity.identity.revision || trustedSupplied.digest !== identity.identity.digest)) {
...
function resolveRoute(options = {}) {
  ...
  const stage = optionBoundary.values.stage;
  ...
  const semanticSubagent = options.semantic_subagent === true || options.semanticSubagent === true || options.subagent === true || options.route_kind === 'semantic-subagent';
...
function validateRouteBinding(binding, options = {}) {
  ...
  const currentIdentityChecked = currentIdentityInput === undefined ? { ok: true } : authorityDataBoundary(currentIdentityInput, ['revision', 'digest']);
  if (!currentIdentityChecked.ok) return result(false, 'ROUTE_UNAVAILABLE', { reason_code: 'CURRENT_REGISTRY_IDENTITY_INVALID', repair_budget_consumed: false });
