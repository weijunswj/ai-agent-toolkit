...
use crate::canonical::{
    self, Digest, MAX_SAFE_INTEGER, canonical_digest, canonical_serialize, decode_hex, encode_hex,
};
...
pub const PROCESS_IDENTITY_DOMAIN: &str = "toolkit.github-program.process-id.v1";
...
impl HolderKey {
    ...
    pub fn generate() -> Result<Self> {
        ...
        getrandom::fill(&mut bytes).map_err(|_| BrokerError::new(ErrorCode::InternalInvariant))?;
        Ok(Self(bytes))
    }
...
pub fn holder_tag(attestation: &HolderAttestation, key: &HolderKey) -> Result<Digest> {
    ...
    let payload = canonical_serialize(&attestation_value_without(
        attestation,
...
pub fn holder_attestation_digest(attestation: &HolderAttestation) -> Result<Digest> {
    attestation.validate()?;
    canonical_digest(&attestation_value_without(
        attestation,
        &["attestation_digest"],
    )?)
}
...
pub fn verify_holder_attestation(attestation: &HolderAttestation, key: &HolderKey) -> Result<()> {
    ...
    if holder_attestation_digest(attestation)? != attestation.attestation_digest {
...
pub fn principal_digest(platform: &str, principal: &Principal) -> Result<Digest> {
    ...
    let principal = match (platform, principal) {
        ...
        ("linux", Principal::Linux { machine_id, uid }) => {
            ...
            object.insert("machine_id".to_owned(), Value::String(machine_id.clone()));
            object.insert("uid".to_owned(), Value::String(uid.to_string()));
            Value::Object(object)
        }
...
fn validate_machine_id(value: &str) -> Result<()> {
    if canonical::is_lower_hex(value, 32) {
        Ok(())
    } else {
        Err(BrokerError::new(ErrorCode::InvalidField))
    }
