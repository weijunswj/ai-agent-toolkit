...
6. Keep routine setup on the root agent; it must not spawn subagents for instruction, docs, state, choice, or validation inspection. Let the host install, verify, or refresh its own Toolkit native plugin path, then answer the required setup gates. Codex delegation limits of `agents.max_threads = 1` and `agents.max_depth = 1` remain explicit opt-in pending native UAT; Enter and `--yes-recommended` keep config unchanged. Other hosts use portable policy only.
