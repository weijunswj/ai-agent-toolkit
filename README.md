...
6. Keep routine setup on the root agent; it must not spawn subagents for instruction, docs, state, choice, or validation inspection. Codex recommends one helper at most and keeps runtime mechanics in a technical proposal. Root-only remains available, unsafe current capacity is labelled, and user-owned conflicts require one exact `apply` confirmation in the same flow. Other hosts use portable policy only.
