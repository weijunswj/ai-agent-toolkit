...
6. Keep routine setup on the root agent; it must not spawn subagents for instruction, docs, state, choice, or validation inspection. Let the host install, verify, or refresh its own Toolkit native plugin path, then answer the required setup gates. Codex helper capacity is explicit opt-in: Enter and `--yes-recommended` keep config unchanged, while RAM-safe MultiAgentV2 mode allows one helper by setting two total session threads because the root counts toward that total. Other hosts use portable policy only.
