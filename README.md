...
6. Keep routine setup on the root agent; it must not spawn subagents for instruction, docs, state, choice, or validation inspection. Let the host install, verify, or refresh its own Toolkit native plugin path, then answer the required setup gates. Codex can apply the recommended one-direct-specialist, depth-one limits without overwriting unrelated config; other hosts use portable policy only.
