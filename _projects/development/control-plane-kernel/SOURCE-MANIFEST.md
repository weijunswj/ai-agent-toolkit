...
- `control-plane-contract.schema.json` — the privacy-safe public result, remote-identity, and opaque authority-ticket contract.
- `control-plane-policy.json` — the default-off typed-operation policy, resource bounds, hard-deny precedence, and active #342 compatibility rule.
- `fixtures/fixture-manifest.json` — the exact 32-case adversarial contract inventory.
...
- `repo/scripts/toolkit-control-plane/control-plane-kernel.cjs` is the dependency-free deterministic runtime. It observes caller-owned values through independently guarded probes into detached evidence, then validates only detached data.
...
This module owns A1 only: repository/remote identity, typed operation validation, secret/finality boundaries, opaque one-shot authority tickets, bounded replay compaction, detached hostile-input observation, and structural-impact assessment. Repository capability lifecycle, execution-loop orchestration, independent assurance, Web finality, providers, and live systems remain outside scope.
