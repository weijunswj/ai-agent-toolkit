...
| `external-system-router` | Provider APIs or supported CLIs/SDKs, MCP or typed connectors, provider-console browser/computer-use, deployment, DNS, database, storage, workflow, OAuth, external-system setup, provider capability selection, operation receipts, or continuation from a receipt. Use it as the universal risk/envelope/target/router safety gate, not as a generic executor. |
