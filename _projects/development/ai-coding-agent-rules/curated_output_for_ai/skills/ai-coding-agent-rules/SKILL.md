...
The portable `AGENTS.managed.template.md` delegates external-system control-plane work to the sibling `external-system-router` skill. [runtime-dependencies.json](runtime-dependencies.json) is the machine-readable standalone-install contract: a standalone `ai-coding-agent-rules` installation must also install the declared compatible complete `external-system-router` skill folder and every required sibling router/schema file before external-system tasks are attempted. This skill does not silently install that dependency. If the sibling is unavailable, incomplete, or incompatible, fail closed for provider, deployment, DNS, database, storage, OAuth, graphical-console, or other external-system operations and report installation of the declared `external-system-router` dependency as the supported next action.
