...
The portable `AGENTS.managed.template.md` delegates external-system control-plane work to the sibling `external-system-router` skill. A standalone `ai-coding-agent-rules` installation must therefore install the complete `external-system-router` skill folder in a supported skill location before external-system tasks are attempted. This skill does not silently install that dependency. If the sibling is unavailable, fail closed for provider, deployment, DNS, database, storage, OAuth, graphical-console, or other external-system operations and report installation of `external-system-router` as the supported next action.
