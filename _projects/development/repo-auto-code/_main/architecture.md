...
When any adoption or activation grant is incomplete, the controller must first prove that no active or persistent surface exists; a present surface is a topology failure, not benign source-only inactivity. After activation, persistent topology contains exactly one Web Orchestrator and one Executor-root, no unknown surface type, and no extra persistent surface. A Temporary Chat is an exceptional surface only after final exact-head PASS, independent Web verification, and an exact explicit assurance grant.
...
One exceptional Web Temporary Chat may be created only after final exact-head technical PASS, independent Web verification of the complete evidence universe, and an exact explicit assurance grant. It is read-only, fresh for that final head, and returns only CLEAR or CONCERN. It cannot return PASS or AMEND, mutate hosted governance, authorise merge, accept, select work, or acquire controller authority.
...
Without an explicit current-turn structured grant, Fast is disabled and `Agent`, `spawn_agent`, subagents, and equivalent delegation are denied. Fast and delegation are separate permissions: both false means root-only Standard; Fast only means root-only Fast; delegation only keeps root and agents Standard; both true may authorize root Fast, but every delegated child remains Standard with `fastAllowed:false`. Root Fast authority never flows to a delegated child. Silence, prompt omission, generic speed wording, prior-turn permission, and standing permission do not grant either capability. Unsupported or unverifiable enforcement falls back to root-only Standard execution.

Current-execution freshness is evidence-derived, not caller-authored: the resolver requires separately trusted active validity/lifecycle evidence whose observed time exactly matches the authority record. A parseable `now` value alone is insufficient; stale, expired, revoked, future/impossible, or moved run/session/turn/head/tree/workspace evidence rejects admission.
...
This section supersedes earlier generic launch wording. No agent, subagent, helper, reviewer, managed session, parallel lane or replacement worker launches without a structured current-run grant. The grant binds exact count, mode, role, provider, canonical model, reasoning, repository, current-execution validity evidence, workspace/checkout identity, mutation scope, capabilities, run/session/turn identity, expiry, one-use state and the further-delegation boundary. Before consuming an exclusive-worker grant, the requested launch must also bind the exact repository, role, current execution/head/tree, workspace identity, requested count, canonical repo-relative scope, and requested capability subset; any mismatch or ungranted admin/governance capability rejects without consumption. Complexity, capacity, expected speed-up, elapsed time, worker availability, host support, tool availability, silence, generic speed language and prior-turn permission never grant launch authority. Missing or incomplete authority returns DELEGATION_NOT_AUTHORISED. Further delegation requires a separate explicit grant.
