...
Before reporting exceptional assurance eligibility, prove an exact pre-dispatch grant, final exact-head technical PASS and independent Web verification, then admit at most one fresh Web Temporary Chat for that head. Routine assurance is not required. The Temporary Chat is a separate context from the Executor-root, implementation/amendment runs, and G4. It independently evaluates bounded evidence, records both execution identities and provider/model diversity when present, and is grant-bound for same-family routes. It returns only CLEAR or CONCERN and is not G5 or a replacement for G4. A G4 packet or self-attestation is not independent proof.

The Web assurance launch must use a validated `assurance-launch/v1` envelope and the accepted canonical assurance-template revision. The envelope must bind the exact base/head/tree/graph, both execution identities, launch run/session/turn identity, evidence-universe revision, expiry/one-use state, and authoritative raw locators for every mandatory assurance domain. Web must admit a structured `assurance-evidence/v1` receipt with every check, inspected evidence identity, final head recheck, complete context separation, and non-authority attestation. A bare or unsupported CLEAR is `ASSURANCE_CLEAR_UNSUPPORTED` and is operationally CONCERN. Before assurance launch admission, trusted current time, created_at, and expires_at must parse to finite timestamps and satisfy created_at <= trusted_now < expires_at and created_at < expires_at; invalid, missing, future, expired, or misordered values fail closed without consuming authority or creating Temporary Chat.
