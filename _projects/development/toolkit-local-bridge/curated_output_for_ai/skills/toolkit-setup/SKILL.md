...
- Codex helper capacity on Codex only: ask "How many helper agents may Codex use at the same time?" with keep current / RAM-safe one helper / custom advanced count / remove Toolkit-managed capacity / skip. The recommendation, empty input, and `--yes-recommended` select `keep`. Custom counts above one require a separate RAM-risk approval. V2 total session threads include the root, so one helper means `max_concurrent_threads_per_session = 2`.
...
- MultiAgentV2 helper guidance keeps routine work on the root and prohibits helpers from spawning helpers, but this is policy-only because no native hard block has been verified. No documented Codex Security scan-scoped capacity activation exists, so never raise normal global capacity automatically; report lower-capacity ordinary or sequential review, Deep Scan on another machine, or an explicitly approved temporary global increase with backup and restoration as alternatives.
