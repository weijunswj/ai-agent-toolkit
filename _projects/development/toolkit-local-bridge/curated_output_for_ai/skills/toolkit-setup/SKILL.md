...
- Structural truth comes from an actual TOML parser. Text resembling `[agents]`, Toolkit markers, or limit assignments inside basic, literal, or multiline strings is never treated as configuration.
- Toolkit edits only a safe explicit `[agents]` table or appends one when no agents structure exists. Unsupported dotted/inline, duplicate, malformed, symlink, special-file, or ambiguous child-table layouts fail closed without writing.
- The proposed config path and managed block are printed before an explicit limit write. Existing files receive a Toolkit-owned exact-byte backup plus mode/topology metadata. Writes are atomic, resulting TOML is parsed again, and an exact restore command is reported. Keep, skip, configured no-op, and conflicting states create no backup.
