...
- On Codex, ask `How many helper agents may Codex use?` The ordinary recommended outcome is one helper at most: Codex performs the main work and may use one helper for a difficult, clearly separate task. Choices are `One helper at most - recommended`, `No helpers`, `Keep current` with a visible risk label when unsafe, and `Advanced`. When exact PR #237 migration is pending, add `Migrate the existing Toolkit legacy setting`, recommend `Keep current`, and make empty input and `--yes-recommended` keep the legacy block unchanged. Reject ordinary capacity choices in that state rather than silently interpreting them as migration.
