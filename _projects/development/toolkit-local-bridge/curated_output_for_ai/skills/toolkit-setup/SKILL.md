...
- Claude Code setup separately offers `Root agent only`, `Direct Toolkit-managed subagents only`, `Broader native behaviour`, and `Keep current` only when the detected CLI supports their real effects. A strict profile is active only while the enabled installed Toolkit plugin, exact `Agent|Task` `PreToolUse` hook, source/cache identity and bytes, controller version, and current CLI launch controls verify. Proof schema 2 binds the hooks configuration, controller, and executed native-agent hook script. Direct admission rechecks those identities, reuses the original verified setup CLI unless an explicit/current environment override wins, and rejects Toolkit-child re-entry. The direct profile blocks native `Agent` and legacy `Task` launches and admits only external sessions started by `repo/scripts/toolkit-agent-control.cjs`; capability loss invalidates the strict profile to root-only policy. Codex state is never used or changed.
...
- On a capable Claude Code host, ask `How should Claude Code use agents?` and `How should Toolkit manage agent capacity?` as separate canonical rows. Offer or recommend Toolkit-direct automatic/manual admission only when exact CLI controls, a supported validated Linux or Windows resource counter, current installed bytes, and host-reported native hook trust plus active execution all verify. Missing, false, stale, malformed, or unknown proof fails closed to root-only/unapplied and invalidates kept strict state. Setup never creates trust or activation proof. Root-only and broader-native use root-only/not-applicable Toolkit admission capacity, but broader-native remains a distinct topology that permits native Agent/Task outside Toolkit admission.
- The Claude direct boundary requires a launch-spec JSON file declaring a meaningful child responsibility, a distinct productive parent responsibility, root-owned integration, root-owned cross-shard validation, material benefit, and depth 1. Launch with `node repo/scripts/toolkit-agent-control.cjs launch --spec <path>`. The controller returns only `start`, `queue`, or `refuse-root-only`; retry a queued specification with its returned `queue_id`, and only the oldest live entry may start. Every child defaults to medium effort, receives `CLAUDE_CODE_DISABLE_FAST_MODE=1`, and cannot use `Agent` or legacy `Task`. The exact UTF-8 prompt bytes are bounded synchronously before admission and sent through stdin rather than argv; an oversized prompt creates no reservation, queue entry, job artifact, or supervisor. One reviewed executable contract handles bare Claude, shell-free JavaScript and `.exe` paths, and safely escaped `.cmd`/`.bat` paths; official-style symlinks are validated through their regular executable target while preserving the original command, and invalid or missing targets refuse before admission or artifacts. Higher effort requires one named difficult role plus a narrow justification, and only one higher-effort reservation may be active.
