...
[repo/scripts/setup-toolkit.cjs](../../../repo/scripts/setup-toolkit.cjs) is the script-backed English `setup toolkit` and `refresh toolkit` orchestrator. Plan mode is read-only; execute mode verifies the trusted repo state on `main`, runs the selected host-native plugin step, runs lite setup validation, audits bridge targets, and pauses before approval-gated repo-backed auto-update, the required update-report auto-open preference, or OpenCode/Antigravity 2 target writes. Codex uses the default host path and can refresh the Codex native plugin cache only when verification requires it; Claude Code uses `--host claude-code` to verify packaged Claude plugin metadata while relying on Claude Code's own plugin install/trust flow.
