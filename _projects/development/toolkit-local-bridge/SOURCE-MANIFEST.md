...
[repo/scripts/setup-toolkit.cjs](../../../repo/scripts/setup-toolkit.cjs) is the script-backed English `setup toolkit` and `refresh toolkit` orchestrator, with shared workflow in `repo/scripts/setup-toolkit-core.cjs`. One semantic model drives the complete compact bank, terminal and piped answers, flags, plans, JSON, summaries, and execution. It recommends at most one helper as a conservative Codex memory backstop, not launch permission, keeps runtime and TOML mechanics in a technical proposal, supports root-only mode, labels unsafe current capacity, and fails closed on unknown effective runtime. Unexpected piped answers and unsupported MultiAgentV2 child tables stop before setup writes. Structurally complete user-owned controls that already match the selected outcome remain byte-preserved without `apply`, editor, backup, or ownership markers; different effective controls require the exact replacement proposal. Exact PR #237 migration defaults to `keep` and requires the visible explicit `migrate` choice. MultiAgentV2 tracks enablement, capacity, root guidance, and helper guidance ownership independently; current Codex and Claude paths do not expose verified global pre-spawn admission plus child-only non-fast enforcement, so Toolkit omits unsupported topology profiles and never promotes policy-only behavior to hard enforcement. Normal or Security workflows never raise capacity automatically. Structural TOML validation, whole-file proposal deltas, atomic backup/restore, bounded cleanup, and executable shell-safe restore commands preserve the transaction boundary. The wizard also migrates legacy report auto-open state to a central failure-only classifier: actionable reports open, successful activity stays closed, and no-ops remain report-free.
