...
[repo/scripts/setup-codex-toolkit-plugin.cjs](../../../repo/scripts/setup-codex-toolkit-plugin.cjs) is the Codex-only native plugin install verifier and local marketplace wrapper runner. It validates `.agents/plugins/marketplace.json`, `.codex-plugin/plugin.json`, installed Codex plugin state, Toolkit version `2.2.0`, and the installed cache `SessionStart` hook before setup reports success. It uses supported Codex plugin commands instead of editing Codex config by hand and does not install or update Claude Code.
