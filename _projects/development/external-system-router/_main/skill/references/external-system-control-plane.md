...
Production authority is established only when the router is launched directly as `trusted-authority-session` under the exact current Node executable with the permission model enabled. Its exact runtime arguments allow file reads only for that executable, that router file, and the canonical registry file, then enable frozen intrinsics, disable `__proto__`, and disable native add-ons. `NODE_OPTIONS`, `NODE_PATH`, additional runtime arguments, preloaded application modules, missing permission support, or a different entry point fail closed before inventory is read. No fallback returns to the ordinary in-process loader.

The isolated process captures the required built-in operations at bootstrap, verifies the exact executable bytes/version and router bytes/version, and uses a bounded signed JSON-line session. The session binds a fresh session ID and non-repeating nonce; accepts exactly one route request followed by one receipt-or-close request; rejects malformed, duplicated, replayed, oversized, or timed-out input; and keeps the inventory, host-plan, selected-route, and receipt WeakMap chain in the same process. Its output contains only bounded public authority metadata, the selected route or receipt, and privacy-safe stable errors. It never accepts an inventory path.

The registry source must be an exact regular non-link file whose resolved path equals its canonical path. Size and record count are bounded; exact bytes, file identity, path topology, authority-bootstrap version, runtime executable version/digest, router version/source digest, repository, host, installation, schema, generation, and target records are verified. File identity, bytes, generation, runtime, router, and installation are revalidated before route and receipt creation. Visible hashes are evidence, not forgeable authority. Raw registry records, copied JSON, and recomputed hashes are never accepted by route selection.
