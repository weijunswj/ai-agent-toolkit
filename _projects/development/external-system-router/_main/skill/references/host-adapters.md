...
Host plans are process-local derivatives of one zero-option production-loader-authenticated snapshot, not independent or serializable authority. No caller-selected source, synthetic loader, environment switch, or public test helper can mint their inventory parent. Plans bind the snapshot authority ID, source bytes, repository/host/installation and router identities, generation, derivation time, target/resource, installed interface, complete audit digest, audit state, and operation semantics. Final selection accepts only the exact unforgeable plan object retained by the same router module and revalidates its source. A copied plan, recomputed digest, expired plan, another module/runtime/snapshot, or any later inventory change is rejected.
