...
The shared profile pins provider, target alias, account/organisation, resource aliases, environment, sanitized fingerprint, monotonic current inventory generation and exact source digest, credential reference, installed audited interfaces, complete capability-audit digests, canonical operation-semantics digests, per-operation route, router source/installation identity, and schema/version digests. It contains no credential value or raw private origin.

Host plans are process-local derivatives of one zero-option production-loader-authenticated snapshot, not independent or serializable authority. The loader can mint that parent only inside the exact permission-bounded isolated authority session; an ordinary imported router, caller-selected source, synthetic loader, environment switch, mutable core-module export, preload, loader/cache override, or public test helper cannot mint it. Plans bind the snapshot authority ID, source bytes, authority-bootstrap version, exact runtime executable/version digest, repository/host/installation and router identities, generation, derivation time, target/resource, installed interface, complete audit digest, audit state, and operation semantics. Final selection accepts only the exact unforgeable plan object retained by the same router process and revalidates its source. A copied plan, recomputed digest, expired plan, another module/runtime/snapshot, or any later inventory change is rejected.
