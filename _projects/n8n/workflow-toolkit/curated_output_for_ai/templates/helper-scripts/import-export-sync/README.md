...
Normal export preserves both canonical values and canonical absence. Live-only protected leaves or complete mapping domains are removed from the portable result, or the export fails closed when safe deletion is not possible. Only explicit reviewed source-update mode may introduce protected logic. Transactions reject repeated normalized targets before staging, preserve each existing regular-file mode, use the ordinary repository file mode for new files, and keep stages/backups private. Immediately before displacement they revalidate the authorised target's exact type, bytes/hash, size, mode, high-resolution identity, and real parent topology; an originally missing target must still be absent. The displaced backup and absent target path are verified before candidate installation. Concurrent state is never overwritten by rollback: earlier records roll back only while their installed candidate and original backup remain exact transaction-owned identities, otherwise bounded recovery evidence is preserved and `N8N_CANONICAL_TRANSACTION_PARTIAL_RECOVERY` blocks retry pending reconciliation. Import preflight rejects multiple canonical workflows resolving to one target workflow before any live or local identity mutation.
