...
Use [Page 3 - Production Self-Hosting With Cloudflare Tunnel](./Page%203%20-%20Production%20Self-Hosting%20With%20Cloudflare%20Tunnel.md) instead when you want production self-hosting from a local/CGNAT machine through Cloudflare Tunnel.
...
Local Traefik, Caddy, or Nginx can provide local reverse-proxy behavior, but it is not a tunnel by itself. A local reverse proxy can route local hostnames to local services; it does not make your local PC reachable from the public internet by itself. For local external webhook tests, the Compose ngrok path remains the supported dev option. For production self-hosting from a local/CGNAT machine, use [Page 3 - Production Self-Hosting With Cloudflare Tunnel](./Page%203%20-%20Production%20Self-Hosting%20With%20Cloudflare%20Tunnel.md). For VPS production, use [Page 2 - Hostinger Coolify VPS n8n](./Page%202%20-%20Hostinger%20VPS.md) instead of adapting the local launcher.
...
- Use [Page 3 - Production Self-Hosting With Cloudflare Tunnel](./Page%203%20-%20Production%20Self-Hosting%20With%20Cloudflare%20Tunnel.md) for production self-hosting from a local/CGNAT machine.
