...
Instruction files: 1) Create or merge [repo-local/AGENTS.managed.template.md](../../../ai-coding-agent-rules/repo-local/AGENTS.managed.template.md) into the target repo root as `AGENTS.md`. 2) Add [repo-local/CLAUDE.shim.template.md](../../../ai-coding-agent-rules/repo-local/CLAUDE.shim.template.md) as `CLAUDE.md`. 3) Install or load [n8n Agent Rules](../../../n8n-agent-rules/). 4) Optionally merge [CLAUDE.n8n-brief.template.md](../../../n8n-agent-rules/adapters/CLAUDE.n8n-brief.template.md) into the same file. If the target repo already has instruction files, do not overwrite them; produce a merge/diff plan. Run the adapter installer with `--dry-run` first and only use `--write` after explicit current-turn approval.
