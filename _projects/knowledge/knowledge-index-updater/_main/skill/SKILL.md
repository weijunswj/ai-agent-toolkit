...
description: Use when maintaining a personal Notion/GitHub knowledge index: creating or updating the index, merging duplicates, categorising entries, refreshing stable Notion Key and GitHub Key identity fields, scheduling checks, or marking missing/stale items without deleting them.
---
...
- Meaningful writes require explicit current-turn approval for the exact write or exact batch of writes.
- Only pure `Last checked` refreshes for existing rows with no meaningful field changes are approval-free.
- Do not refresh `Last checked` for rows with pending proposed meaningful changes.
- If a batch contains any meaningful write, propose the meaningful writes first and request confirmation before applying anything.
- Read `references/update-confirmation.md` before preparing or applying Notion/GitHub writes; it contains the full meaningful-write list, exact proposal format, batch rules, and refreshed-without-confirmation reporting section.

Do not apply any meaningful write without confirmation. If approval is not available, report the proposed writes and reasons instead of applying them.
...
For copyable Codex automation prompts and static fallback scheduler prompts, read `references/scheduled-updater-prompts.md` only when the user asks to create or edit a recurring updater, external scheduler, or automation prompt. Replace placeholder database names and data source URLs with the user's actual Knowledge Index details before using those prompt templates.
