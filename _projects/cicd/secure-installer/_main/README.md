...
Paste one prompt into your AI coding agent to add a **production-ready, security-first GitHub Actions CI/CD pipeline** to almost any GitHub project.

This repo is for people who want serious CI/CD without manually learning DevOps first.

---

## ELI5: What is CI/CD? 🧃

Imagine your project is a LEGO build.

Every time you add new pieces, you want someone to check:

- Did anything break?
- Are any pieces missing?
- Is the build still stable?
- Is it safe to show other people?
- Can it be delivered without you manually doing the same boring steps again and again?

That is what CI/CD does for code.

**CI** means **Continuous Integration**.

> Every time you change your code, GitHub automatically checks whether the project still works.

CI usually runs secret scanning, dependency checks, code quality checks, tests, and build commands.

**CD** means **Continuous Delivery** or **Continuous Deployment**.

> After checks pass, GitHub can help prepare or deploy the project automatically.
...
GitHub checks it
      ↓
Bad changes get blocked
      ↓
Good changes can be built or deployed
```
...
- You might deploy the wrong version.
- You might repeat the same boring setup steps every project.

With CI/CD:

- Every pull request can be checked automatically.
- Secrets and unsafe files can be blocked.
- Tests and builds can run before deployment.
- Deployment can be repeatable.
- Production can require approval.
- Failures are easier to trace.

The goal of this repo:

> Let your AI coding agent set up the serious stuff for you, while keeping the manual steps tiny and easy to follow.

---

## How does it block bad code? 🛡️

The generated CI/CD pipeline works like a chain of gates.

Code must pass every required gate before deployment is allowed.
...
4. Test suite.
5. Build check.
6. Optional staging deploy.
7. Optional production approval.

If any required gate fails, the workflow fails.

When the workflow fails:

- The pull request should not be merged.
- Deployment should not run.
- Production should stay untouched.

The pipeline does not magically know whether code is perfect. It enforces the checks that are configured.
...
> Bad changes get stopped at the gates. Good changes are allowed to continue.

---

## What this does

This installer helps your AI coding agent:

- Inspect your project automatically.
...
- Block deployment until you approve the deployment plan.

---

## What this does not do

This installer will **not** blindly do risky things.

It will not:

- Paste or expose your real secrets.
- Ask you to paste real API keys into chat.
- Create paid accounts for you.
- Decide billing choices for you.
- Deploy to production without your approval.
- Rewrite git history without asking you first.
- Delete existing workflows without showing you the impact.
- Silently choose or enable a production deployment target without your approval.

The goal is simple:

> Automate setup, not trust.
...
- A GitHub project/repo.
- An AI coding agent with access to your repo, such as Codex, Claude Code, Cursor, Windsurf, or similar.
- Your GitHub account connected inside that coding agent.

You do **not** need to understand GitHub Actions, CI/CD, secrets, runners, deployment environments, or DevOps jargon.

The agent will explain anything you need to do manually.
...
3. Paste it into the AI coding agent.
4. Let the agent inspect your project.
5. Answer the few confirmation questions it asks.
6. Follow any manual steps exactly as shown.
7. Approve commit, push, pull request, merge, and deployment only when you are ready.
...
- If this is confirmed or strongly likely to be an n8n workflow project, use `n8n-workflows/` as the canonical workflow JSON folder and install only the reusable n8n runtime helper scripts from `templates/n8n/` into the target repository at `scripts/`: `export-n8n-workflows-live.ps1`, `import-n8n-workflows-live.ps1`, `n8n-workflow-sync-menu.ps1`, `validate-n8n-workflows.cjs`, `sync-n8n-live-exports.cjs`, `prepare-n8n-live-import.cjs`, `compare-n8n-workflow-credentials.cjs`, `should-import-n8n-workflow.cjs`, `_export-n8n-workflows-live.cmd`, and `_import-n8n-workflows-live.cmd`. Otherwise fetch only those helper scripts from the public AI CI/CD installer repository at https://github.com/weijunswj/ai-cicd-installer/tree/main/templates/n8n/. Do not copy tests, fixtures, reference examples, or docs-only files. Still generate project-specific CI, .gitignore additions, README usage instructions, and CURRENT_CICD_STATUS.md updates dynamically based on the actual target repository structure. Do not run live n8n import/export inside generic CI by default. Never commit .n8n-local/, .tmp/, *.live-export.json, *.live-import.json, credential binding files, or n8n credential exports.
...
Depending on the detected project, the agent may create or update files like:
...
The exact files depend on your repo.

The agent should generate these files based on the actual project structure, package manager, language, framework, deployment target, and security needs.

The agent should explain every created or changed file before committing.
...
```powershell
node scripts/validate-n8n-workflows.cjs n8n-workflows
.\scripts\n8n-workflow-sync-menu.ps1
.\scripts\n8n-workflow-sync-menu.ps1 -UsePrevious
.\scripts\export-n8n-workflows-live.ps1 -Mode RepoTrackedOnly -WorkflowDir n8n-workflows -DryRun
.\scripts\export-n8n-workflows-live.ps1 -Mode AllLive -WorkflowDir n8n-workflows
.\scripts\import-n8n-workflows-live.ps1 -WorkflowDir n8n-workflows -DryRun -ArchivedByNameMode CreateNew
.\scripts\import-n8n-workflows-live.ps1 -WorkflowDir n8n-workflows -ProjectId <PROJECT_ID>
.\scripts\import-n8n-workflows-live.ps1 -WorkflowDir n8n-workflows -UserId <USER_ID>
```

`RepoTrackedOnly` exports only workflows represented by root-level `*.json` files directly inside `n8n-workflows/`. It does not scan recursively and does not pull unrelated live workflows into the repo. `AllLive` is for first-time bootstrap or full live capture; review its changes before committing.

Create `n8n-workflow-policy.json` only when generic validator defaults are too strict or too loose for your repo. It can allow placeholder regexes, email domains, URL hosts, literal safe values, Google-ID warning behavior, and whether warnings should fail validation.
...
Historical version export and deletion of archived workflows are intentionally unsupported by these CLI helper scripts.
...
- What checks currently run.
- Whether deployment is enabled or disabled.
- Which secrets are needed, by name only.
- What manual steps are still pending.
- What is safe to do next.
- What must not be done yet.
- How to rerun or debug the workflow.

This is intentionally a status file, not a one-time setup summary.

---

## Default pipeline standard

A proper generated pipeline should look roughly like this:
...
CI should be installed first.

Deployment should only be enabled after you confirm the target and plan.

---

## Security policy

This installer is security-first.

The AI agent must:

- Scan for secrets before setup.
- Add secret scanning into CI.
- Block deployment if secrets are detected.
- Avoid printing secret values.
- Avoid committing `.env` files.
- Use `.env.example` for safe placeholders.
- Use GitHub Secrets or environment secrets for private values.
- Recommend rotating secrets if they were committed or pushed.
- Ask before rewriting git history.

If a secret is found, the correct behaviour is to stop, clean up, rotate if needed, and recommit safely.

---

## Deployment policy

Deployment is designed to be universal through AI-guided project detection and project-specific generation.

The AI agent must:

- Ask whether deployment is wanted.
- Detect the likely best deploy target.
- Recommend one clear deployment path.
- Ask for approval before enabling deployment.
...
When the AI agent asks you a question and you are unsure, reply:

```txt
I do not know. Choose the safest recommended setup and explain it simply.
```
...
If you are unsure, use this path:
