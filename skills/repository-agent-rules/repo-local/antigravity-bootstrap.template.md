<!--
Curated AI-facing source.
Source: repo/contracts/agent-rules/repo-local
Review rule: Preserve safety constraints from preserved source. Do not weaken credential, .env, .tmp, .n8n-local, live n8n action, approval, attribution, or local-only rules.
-->

<!-- AI-AGENT-TOOLKIT:repo/contracts/agent-rules/repo-local/antigravity-bootstrap.template.md:BEGIN ANTIGRAVITY.BOOTSTRAP v1 -->
# Agent Toolkit Antigravity Bootstrap

Root `AGENTS.md` is the canonical repo instruction file.

**If root `AGENTS.md` is already loaded, do not duplicate or re-import it.**

**If root `AGENTS.md` is not loaded, read root `AGENTS.md` before repo edits.**

Do not add extra `.agents/rules` files.
<!-- AI-AGENT-TOOLKIT:repo/contracts/agent-rules/repo-local/antigravity-bootstrap.template.md:END ANTIGRAVITY.BOOTSTRAP -->
