...
- If Codex or Claude Code opens `session-start.sh` on every new chat, the installed plugin is invoking a bare `.sh` hook on Windows. Run `node repo/scripts/audit-n8n-skills-plugin-hooks.cjs --plugin-root "<plugin-cache-path>" --windows`; if it fails, remove, disable, untrust, or uninstall the official plugin hooks or plugin. Restart the agent, use `npx skills add n8n-io/skills`, and keep the target repo cue that loads `using-n8n-skills` before n8n work. Only reinstall or re-trust the official plugin after hook metadata is Windows-safe and Node JSON fallback exists. The installed plugin cache may need manual reinstall or update after upstream fixes.
