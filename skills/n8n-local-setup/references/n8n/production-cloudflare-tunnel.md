...
This is not the local development path. For local development and webhook testing, keep using [Page 1 - Local Setup](local-setup.md) with localhost-first startup and the ngrok permanent/reserved URL mode.
...
Use this page only for the production Cloudflare Tunnel stack. Keep [Page 1 - Local Setup](local-setup.md) focused on localhost and ngrok dev usage.

Use [Page 2 - Hostinger Coolify VPS n8n](hostinger-vps.md) when the production target is a VPS with Coolify instead of a local/CGNAT machine.
