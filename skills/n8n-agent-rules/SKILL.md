...
Root `AGENTS.md` is the only repo-local target for the n8n adapter. `CLAUDE.md`, `GEMINI.md`, and `.agents/rules/00-agent-toolkit-bootstrap.md` are platform shims owned by `ai-coding-agent-rules`; do not append the n8n adapter to those shim files.
...
- Install or repair repo-local `AGENTS.md` with `ai-coding-agent-rules`.
- Create or update only `AGENTS.md` with the n8n adapter.
- `none`.

The answer `none` is allowed and must be respected.

If Claude Code or Antigravity shim files are missing, stale, or unmanaged, use `ai-coding-agent-rules` to install or repair those shims. Do not use this n8n adapter installer for shim repair.
