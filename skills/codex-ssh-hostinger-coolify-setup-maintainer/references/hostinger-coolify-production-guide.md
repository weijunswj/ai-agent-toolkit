...
For Hostinger VPS plus Coolify deployment setup/maintenance, use `codex-ssh-hostinger-coolify-setup-maintainer`. Use `n8n-environment-setup` for local n8n and for the n8n-specific Hostinger Coolify VPS deployment reference after Coolify exists. If the deployed app is n8n or the task touches live n8n workflows, credentials, import/export, activation, execution, or repo/live sync, also apply `n8n-safety-router`.
