...
The router loads this registry through one bounded authority path derived from the operating system's current user profile rather than caller-controlled environment variables. The production loader accepts zero arguments and has no test flag, source-path override, dependency-injected reader, environment/argv/global switch, identity-expectation helper, or compatibility alias. An ordinarily imported production module always reports authority unavailable and cannot write an authority map.

Production authority is established only when the router is launched directly as `trusted-authority-session` under the exact current Node executable with the permission model enabled. Its exact runtime arguments allow file reads only for that executable, that router file, the canonical registry file, and the bounded inventory-generation anchor, plus file writes only to that anchor; they then enable frozen intrinsics, disable `__proto__`, and disable native add-ons. `NODE_OPTIONS`, `NODE_PATH`, additional runtime arguments, preloaded application modules, missing permission support, or a different entry point fail closed before inventory is read. No fallback returns to the ordinary in-process loader.

The isolated process captures the required built-in operations at bootstrap, verifies the exact executable bytes/version and router bytes/version, and uses a bounded signed JSON-line session. The session binds a fresh session ID and non-repeating nonce; accepts exactly one route request followed by bounded session keepalives and one receipt-or-close request; rejects malformed, duplicated, replayed, oversized, idle, over-limit, or absolute-lease-expired input; and keeps the inventory, host-plan, selected-route, and receipt WeakMap chain in the same process. Signed keepalive acknowledgements retain a 30-second idle cadence within a five-minute read-only or 24-hour mutation absolute lease, so a long-running operation can still produce its authenticated terminal receipt without granting an indefinite session. Its output contains only bounded public authority metadata, the selected route, keepalive acknowledgement, or receipt, and privacy-safe stable errors. It never accepts an inventory path.

The registry source and persistent generation anchor must be exact regular non-link files whose resolved paths equal their canonical paths. The registry-provisioning transaction creates `inventory-generation-anchor.jsonl` once as an empty owner-only regular file beside the registry before the first authority launch; the authority process cannot create or recreate it, and missing or replaced anchor state fails closed. Size and record count are bounded. The append-only generation anchor chains exact registry namespace identity, monotonic sequence, and byte digest, is closed and reread before authority is returned, and survives fresh authority processes; rollback, same-generation byte drift, malformed/truncated chains, topology drift, unsafe concurrent append, or an unverifiable/missing append fails closed. Node's Permission Model disables `fsync`, so this boundary claims cross-process persistence and immediate reread verification, not power-loss durability; the protected broker/TAS journal owns crash-durable generation commits. Exact registry bytes, file identity, path topology, authority-bootstrap version, runtime executable version/digest, router version/source digest, repository, host, installation, schema, generation, and target records are verified. File identity, bytes, generation, runtime, router, and installation are revalidated before route and receipt creation. Visible hashes are evidence, not forgeable authority. Raw registry records, copied JSON, and recomputed hashes are never accepted by route selection.

Trust assumptions are explicit: the OS-launched Node executable, installed router file, canonical inventory file, and their parent access controls must already be trusted; the supervisor must launch the exact documented arguments with `NODE_OPTIONS` and `NODE_PATH` absent. The router does not self-attest a maliciously replaced executable or installation. A platform or supervisor that cannot provide Node's permission model and those exact launch conditions is unsupported and receives `EXTERNAL_INVENTORY_BOOTSTRAP_UNAVAILABLE`; it never mints reduced-assurance authority.

| Platform capability | Authority behavior |
| --- | --- |
| Windows, Linux, or macOS with the exact verified Node executable and support for `--permission`, exact file read grants, `--frozen-intrinsics`, `--disable-proto=throw`, and `--no-addons` | Supported when the trusted supervisor also clears preload/module-search influence and the canonical source passes every identity and topology check. |
| Node without any required flag or permission API | Stable unavailable result before inventory loading. |
| Preload, extra runtime flag, module-search override, replaced source, topology drift, byte drift, generation rollback, runtime/source identity drift, or another installation/runtime | Fail closed; no plan, route, or receipt authority. |
...
- `INVENTORY_AUTHORITIES` has one write site: the zero-option canonical loader inside the isolated authority session, after permission/bootstrap, source topology, bytes, schema, generation, executable, router, repository, host, installation, and authority-path identity pass. The ordinary imported module cannot reach that write with authority enabled.
- `HOST_PLAN_AUTHORITIES` has one write site: host-plan derivation after the exact inventory object is found in `INVENTORY_AUTHORITIES` and its canonical source and runtime parent are revalidated.
- `SELECTED_ROUTE_AUTHORITIES` has one write site: final selection after its inventory or host-plan parent is found in the relevant map, current inventory resolution, exact target/audit/operation agreement, final risk recomputation, full authorisation, interface selection, and one last inventory revalidation.

All three maps are module-private `WeakMap` instances. Serialized, spread, copied, reconstructed, rehashed, stale-generation, cross-module, or cross-runtime objects are absent from the relevant map and fail closed. Repository tests use a non-published harness that compiles an isolated router instance with its own temporary canonical fixture and a process-mode variant that retains the production permission/bootstrap checks. Each instance has separate private maps and cannot mint or inspect authority in either authoritative or generated production modules. The harness and adversarial fixture are outside project outputs, production exports, CLIs, setup helpers, native bridges, skills, and packs.

The production export contract contains only schema/version constants, pure validation/digest/reconciliation helpers, the zero-option loader, authority-consuming plan/route/receipt functions, safe path descriptions, and the read-only validation CLI. It exports no bootstrap runner, runtime capture, authority-map access, source selector, arbitrary reader, identity-expectation helper, authority deserializer, module-loader injection, test constructor, symbol capability, or compatibility alias. A regression test pins the complete export-name set and fails on additions.
