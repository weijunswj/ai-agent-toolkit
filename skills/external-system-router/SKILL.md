...
3. Load the canonical local target registry through the zero-option bounded trusted inventory loader, then resolve one exact provider target. Production has no test flag, source-path override, environment switch, or synthetic authority helper. Reject raw/caller-created records, copied snapshots, links, replaced bytes, identity drift, generation rollback, and recomputed plans. Never guess from recent credentials, tabs, history, environment-variable order, or a generic token.
