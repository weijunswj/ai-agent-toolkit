...
- Claude Code setup separately offers `Root agent only`, `Direct Toolkit-managed subagents only`, `Broader native behaviour`, and `Keep current` only when the detected CLI supports their real effects. The direct profile blocks native `Agent` launches through the plugin `PreToolUse` hook and admits only external sessions started by `repo/scripts/toolkit-agent-control.cjs`. Codex state is never used or changed.
- Automatic Claude capacity uses physical availability, commit/pagefile headroom, pressure, active Toolkit workers, outstanding reservations, estimated cost, topology, and depth under one atomic transaction. Manual maximum remains a backstop. Built-in, team, Security, plugin, user-created, third-party, and direct CLI launches outside this script are not covered.
