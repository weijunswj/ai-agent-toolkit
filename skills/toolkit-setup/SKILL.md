...
- Every setup write carries one deeply immutable invocation authority. Each lock acquisition or reacquisition performs a fresh read, discovery, exact binding comparison, short-lived narrowed phase, bounded write, invalidation, and owned release. `--write` alone grants no mutation authority.
- Consequential Bridge filesystem and process effects are performed only by private action-specific executors. The same component that finally validates the exact action, lock, durable state, source/destination continuity, and operands immediately dispatches the fixed operation and verifies its postcondition. Retries, loop entries, cleanup, restoration, failure persistence, and report operations require independent admission; never add a generic callback executor or reusable permit.
- Sync only enabled targets that the current invocation explicitly authorizes.
- Persist repo, report, retention, and Codex cache preferences only with the Bridge `--preference-only` boundary. That path must not discover or write target state, regenerate adapters, replace target hub payloads, sync destinations, refresh native caches, repair hooks, update repositories, or perform report maintenance.
- Map `skip this time` and `keep current` to zero target mutation. Map enable/synchronise to one bounded selected-target write and disable to that selected target's state only; delivered files remain. Do not follow setup target selection with unrestricted `--sync-enabled`.
- Treat `needs_sync` as observation only. `authorised_would_write` / `would_write` and exact planned writes include only work authorized in the current invocation.
- Manual sync, SessionStart/hook maintenance, repo update, native maintenance, and delegated sync must carry exact action and destination authority and may never widen it from arguments, preferences, durable state, classification, or fresh discovery after locking.
- Repo verification must originate `toolkit.local-bridge.verified-source-receipt.v1`, including its Git-object-backed local source closure and private immutable byte capsule, before downstream activity. Delegation uses the receipt-backed fixed `node -e` bootstrap, manifest-restricted relative loads, and the receipt/commit/tree/entry/manifest child-start handshake. Do not reconstruct child write authority from `--parent-scope-digest`, `--scope-target-sync`, saved target state, or child flags. Delegated children establish their own mutation phase and never independently clean, create, or open update reports.
- Managed Codex cache refresh passes the canonical `--delegated-invocation-authority` capsule to the narrowly delegated setup child. That child independently admits its fixed Codex CLI, SessionStart write, and TOML parser effects. Windows hook repair and staging helpers provide pure plans to Bridge-owned per-effect executors; standalone compatibility writers are not managed authority.
- The receipt and executor controls use a cooperative user-space threat model. Do not claim protection from a hostile kernel, administrator, or runtime-rewriting process.
- Treat report cleanup, creation, and opening as separate permissions. Suppression grants none; cleanup uses only the under-lock candidate inventory and does nothing if inventory fails, creation is exclusive and non-clobbering with bounded attempts, and opening is limited to the exact report freshly created and verified by this invocation.
- Disabled, skipped, kept, never-enabled, or otherwise excluded targets must not be touched.
