...
- On Codex, ask `How many helper agents may Codex use?` This configures a conservative memory backstop, not launch permission. The ordinary capacity recommendation is one helper at most, but the active host profile and per-launch gate must still verify topology, resource admission, medium reasoning, and non-fast execution. List every currently valid outcome directly as separate choices. Supported normal choices are `One helper at most - recommended`, `Root agent only`, `Keep current` with a visible risk label when unsafe, and `Use a custom number`; exact Toolkit-owned removal appears only when it is available. When an exact existing Toolkit setting requires an update before ordinary capacity choices can apply, show only `Keep current - recommended` and `Update the existing Toolkit helper setting`. Do not show unsupported, irrelevant, or generic advanced choices.
