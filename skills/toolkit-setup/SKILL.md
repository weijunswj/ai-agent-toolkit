...
- Repo-update delegation must use the fixed strict child payload over direct child stdin. Do not reconstruct child write authority from `--parent-scope-digest`, `--scope-target-sync`, saved target state, or child flags. Delegated children never independently clean, create, or open update reports.
