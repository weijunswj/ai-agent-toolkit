...
- Every setup write carries an exact invocation action scope. `--write` alone grants no mutation authority.
...
- Manual sync, SessionStart/hook maintenance, repo update, native maintenance, and delegated sync must carry their own exact target/action/destination authority and may never widen it from durable state after locking.
