...
- On Codex, render the `Codex helper agents` question. This configures a conservative memory backstop, not launch permission. Because no strict Toolkit-controlled Codex launch path is verifiable, the ordinary recommendation is `Root agent only`. List every currently valid outcome directly as separate choices and show the recommendation separately from the labels. Supported normal choices are `Root agent only`, `One helper at most`, `Keep current` with a visible risk label when unsafe, and `Use a custom number`; exact Toolkit-owned removal appears only when it is available. When an exact existing Toolkit setting requires an update before ordinary capacity choices can apply, show only `Keep current` and `Update the existing Toolkit helper setting`, while recommending keep-current in the question metadata. Do not show unsupported, irrelevant, or generic advanced choices.
...
- On a capable Claude Code host, ask `How should Claude Code use agents?` and `How should Toolkit manage agent capacity?` as separate canonical rows. Offer or recommend Toolkit-direct automatic/manual admission only when exact CLI controls, a supported validated Linux or Windows resource counter, current installed bytes, and host-reported native hook trust plus active execution all verify. Activation-proof schema 3 binds the non-symlink regular-file hooks, controller, process-launch dependency, and native-agent hook bytes. Schema-2, missing, false, stale, malformed, or unknown proof fails closed to root-only/unapplied and invalidates kept strict state. Setup never creates trust or activation proof. Root-only and broader-native use root-only/not-applicable Toolkit admission capacity, but broader-native remains a distinct topology that permits native Agent/Task outside Toolkit admission.
...
- Codex plugin cache auto-refresh is Codex-only. When enabled, startup hooks may refresh stale Codex Toolkit plugin cache content only from the configured managed `main` repo after repo validation and delegated target sync succeed. On Windows, the same opt-in inventories n8n cache identity independently of the legacy hook-manifest path, uses `codex plugin list --available --json` to select the single installed and enabled `n8n-skills@n8n-io` version/root, and reconciles only a known supported version/layout with the existing Toolkit wrapper. Retained historical caches and unrelated plugins are skipped. Missing, moved, ambiguous, unknown, or malformed current layouts fail closed and require upstream compatibility review.
