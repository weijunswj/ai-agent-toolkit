...
- Codex setup may manage only the Toolkit-marked `agents.max_threads` and `agents.max_depth` block in `CODEX_HOME/config.toml`. Claude Code and OpenCode receive the portable single-agent policy but no invented host-level enforcement.
...
- Codex delegation control on Codex only: current configured/conflicting state and keep current / limit / skip. The recommended limit is `agents.max_threads = 1` and `agents.max_depth = 1`, which permits one direct specialist while preventing recursive delegation. Empty interactive input selects the displayed recommended answer.
...
- Codex delegation limits are Codex-only. Setup may add or refresh only its marked block under `[agents]`; it must preserve unrelated config byte-for-byte, leave compatible user-owned values untouched, refuse conflicting user-owned or unsupported TOML shapes, and report configured, skipped, unsupported, or conflicting status honestly. The one available direct specialist slot remains usable for an explicitly invoked security specialist or Codex Security workflow, while depth `1` prevents that specialist from spawning descendants.
