...
- Codex helper capacity on Codex only: ask "How many helper agents may Codex use at the same time?" with keep current file unchanged / explicitly migrate the exact Toolkit-managed PR #237 legacy setting / RAM-safe one helper / custom advanced count / remove exact Toolkit-managed values / skip. The recommendation, empty input, and `--yes-recommended` select `keep`, which never writes, creates a backup, invokes the editor, or migrates. Migration is explicit and previews target, runtime, before/after semantics, proposed blocks, planned backup metadata, the verified absolute setup script, and exact restore commands before writing. Removal warns that host defaults may restore higher capacity and requires an explicit remove choice. Custom counts above one require separate RAM-risk approval. V2 total session threads include the root, so one helper means `max_concurrent_threads_per_session = 2`. The old `--codex-delegation-control limit` flag remains only as a documented alias for RAM-safe mode.
