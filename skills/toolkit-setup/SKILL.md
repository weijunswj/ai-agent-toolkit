...
- Every setup write carries an immutable execution-authority-v2 ceiling plus a fresh non-widening locked projection. `--write` alone grants no mutation authority.
...
- Manual sync, SessionStart/hook maintenance, repo update, native maintenance, and delegated sync must carry exact effect/resource authority and may never widen it from arguments, preferences, durable state, classification, or fresh discovery after locking.
- Repo-update delegation must use the digest-bound v2 envelope over direct child stdin. Do not reconstruct child write authority from `--parent-scope-digest`, `--scope-target-sync`, saved target state, or child flags. Delegated children never independently clean, create, or open update reports.
- Treat `report.cleanup`, `report.create`, and `report.open` as separate effects. Suppression grants none; cleanup binds the locked candidate set, creation binds one reserved path, and opening binds only the newly created validated report.
