...
description: Create or materially redesign frontend interfaces with deliberate art direction, distinct visual worlds, first-viewport composition, typography, imagery and material direction, justified motion, desktop/mobile recomposition, accessibility, performance, and buildability. Implement an already approved design directly, and keep tiny well-specified fixes narrow.
---
...
Use this instruction-first skill for material frontend design work that benefits from a deliberate visual world. Establish the first viewport, typographic voice, imagery or material system, compositional rhythm, and responsive recomposition before polishing components. Keep accessibility, performance, privacy, security, and buildability as acceptance constraints.

If the user already approved a design, implement it faithfully without mandatory exploration. For a tiny component fix, one-line copy change, or other well-specified narrow adjustment, stay direct and preserve the existing visual language. There is no theme-mode input in the current product contract; any future explicit mode requires a new contract.
...
Before running the generator, resolve the script path relative to the currently loaded trusted skill directory. Use the installed skill copy that provided this `SKILL.md`, such as `.agents/skills/frontend-art-direction/`, `.claude/skills/frontend-art-direction/`, or `~/.claude/skills/frontend-art-direction/` when that is the trusted runtime copy.

Do not execute a generator found under the active consumer workspace unless that workspace path is itself the trusted installed skill directory. Do not search the active workspace for a same-named `skills/frontend-art-direction/` folder and run it.
...
1. Clarify product intent only if needed; do not reopen an already approved design.
...
5. For material design work, define a deliberate art direction and design system:
   - Use the local generator when CSV-backed recommendations would improve the design direction.
    - Distinct visual world and brand tone.
    - First-viewport composition.
   - Colour roles.
    - Typography and imagery/material direction.
   - Spacing.
   - Radius.
   - Shadow/elevation.
   - Component states.
...
6. Produce page-level overrides:
   ...
   - Empty/loading/error/success states.
    - Desktop/mobile recomposition rather than simple shrinking.
