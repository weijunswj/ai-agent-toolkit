...
- Instruction-first design guidance in [skills/frontend-art-direction/](../../).
- Local-only CSV search/generation tooling in [skills/frontend-art-direction/tools/design-system-generator/](../../tools/design-system-generator/).

The canonical design surface is [skills/frontend-art-direction/](../../). Its local-only generator and CSV data preserve the safe local-search subset. Active upstream provenance is tracked in `repo/source-watch/provenance/ui-ux-pro-max/SOURCE-LOCK.json`.
...
- Writes must stay inside the generator output folder documented by [skills/frontend-art-direction/tools/design-system-generator/README.md](../../tools/design-system-generator/README.md) and require explicit current-turn approval.
- Changing generator scripts, CSV data, tests, or dependencies requires explicit scope and validation.

## Usage

For AI-agent design work, load the skill from [skills/frontend-art-direction/SKILL.md](../../SKILL.md).

For design creation or revision where CSV-backed recommendations would help, read [skills/frontend-art-direction/tools/design-system-generator/README.md](../../tools/design-system-generator/README.md) before running the generator.
...
`<TRUSTED_UI_UX_SKILL_DIR>` must be the resolved installed skill directory that provided the active `SKILL.md`, such as `.agents/skills/frontend-art-direction/`, `.claude/skills/frontend-art-direction/`, or `~/.claude/skills/frontend-art-direction/`.
