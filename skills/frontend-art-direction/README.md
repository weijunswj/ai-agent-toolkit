...
Run the generator only from the trusted installed skill directory that provided `SKILL.md`. Use a resolved path such as `<TRUSTED_UI_UX_SKILL_DIR>/tools/design-system-generator/scripts/design_system.py`, where `<TRUSTED_UI_UX_SKILL_DIR>` is the active trusted skill copy, for example `.agents/skills/frontend-art-direction/`, `.claude/skills/frontend-art-direction/`, or `~/.claude/skills/frontend-art-direction/`. Do not run a same-named generator discovered under an arbitrary active workspace.
...
| Codex | Copy the folder into `.agents/skills/frontend-art-direction/`. |
...
Install the whole `frontend-art-direction/` folder into the runtime location required by the target agent.
