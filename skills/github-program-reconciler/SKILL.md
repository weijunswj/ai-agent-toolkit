...
For the accepted Root-003 human-surface trust-boundary simplification, use only
the production `humanSurfaceV2` facade from
...
legacy migration planning. Do not expose or select parser, codec, marker,
graph, authority, history, migration, or source-adapter helpers. Existing v5
helpers remain historical compatibility internals for frozen E3 bytes; new
rendering is human-v2 only. Provider writes, PR-number binding, Web finality,
Ready, merge, and Programme Apply remain outside the facade and require their
