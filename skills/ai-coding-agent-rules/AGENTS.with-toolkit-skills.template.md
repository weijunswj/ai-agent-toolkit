...
-->
# AGENTS.with-toolkit-skills.template.md AI coding agent rules with toolkit skill routing

Use this generated template for Codex or OpenCode when this toolkit's skills folders are installed or copied.
...
Copy or merge the fenced payload into the target repo root as `AGENTS.md` only when the user explicitly wants generic Codex/OpenCode rules with toolkit skill routing installed.
