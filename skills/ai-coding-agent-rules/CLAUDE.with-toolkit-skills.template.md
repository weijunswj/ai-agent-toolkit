...
-->
# CLAUDE.with-toolkit-skills.template.md AI coding agent rules with toolkit skill routing

Use this generated template for Claude Code when this toolkit's skills folders are installed or copied.
...
Copy or merge the fenced payload into the target repo root as `CLAUDE.md` only when the user explicitly wants generic Claude Code rules with toolkit skill routing installed.
