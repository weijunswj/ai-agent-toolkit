...
- If this is confirmed or strongly likely to be an n8n workflow project, use `n8n-workflows/` as the canonical workflow JSON folder for CI validation and local repository hygiene. Keep Secure CI/CD focused on project-specific CI, `.gitignore` additions, README safety notes, validation wiring, and `docs/ci-cd/CURRENT_CICD_STATUS.md` updates based on the actual target repository structure. Route n8n import/export, sanitation, comparison, validation, or import-preparation helper work to `n8n-workflow-transport` after `n8n-safety-router`. Test-only Toolkit workflow fixtures are not a public template product and must not be installed or advertised. Do not run live n8n import/export inside generic CI by default. Never commit `.n8n-local/`, `.tmp/`, `*.live-export.json`, `*.live-import.json`, credential binding files, or n8n credential exports.
