...
Use [Page 2 - Hostinger Coolify VPS n8n](hostinger-vps.md) instead when you want an always-on hosted server.
Use [Page 3 - Production Self-Hosting With Cloudflare Tunnel](production-cloudflare-tunnel.md) instead when you want production self-hosting from a local/CGNAT machine through Cloudflare Tunnel.
...
New-Item -ItemType Directory -Force "$env:USERPROFILE\.n8n-local"
Copy-Item -Path "skills\n8n-environment-setup\templates\.n8n-local\*" -Destination "$env:USERPROFILE\.n8n-local" -Recurse -Force
...
Local Traefik, Caddy, or Nginx can provide local reverse-proxy behavior, but it is not a tunnel by itself. A local reverse proxy can route local hostnames to local services; it does not make your local PC reachable from the public internet by itself. For local external webhook tests, the Compose ngrok path remains the supported dev option. For production self-hosting from a local/CGNAT machine, use [Page 3 - Production Self-Hosting With Cloudflare Tunnel](production-cloudflare-tunnel.md). For VPS production, use [Page 2 - Hostinger Coolify VPS n8n](hostinger-vps.md) instead of adapting the local launcher.
...
| 8 | `Advanced / Recovery: Restore local n8n from backup` | You need to replace the current local database state from a backup `.zip` package. | No. |
...
- Existing scheduled tasks that still call `--run-n8n-cli-backup --scheduled` continue to work and now create recovery zip packages.
...
* Use [n8n Safety Router](../../../n8n-safety-router/) before workflow, helper-script, import/export, credential, execution, repo/live sync, or live-instance work.
...
| --- | --- | --- |
| Codex | [mcp setup - codex.md](../../../../skills/n8n-environment-setup/references/ai-agent-platforms/codex.md) | [codex-mcp-config.md](../../../../skills/n8n-environment-setup/templates/mcp-configs/codex-mcp-config.md) |
| Claude Code | [mcp setup - claude code.md](../../../../skills/n8n-environment-setup/references/ai-agent-platforms/claude-code.md) | [claude-mcp-config.md](../../../../skills/n8n-environment-setup/templates/mcp-configs/claude-mcp-config.md) |
| OpenCode | [mcp setup - opencode.md](../../../../skills/n8n-environment-setup/references/ai-agent-platforms/opencode.md) | [opencode-mcp-config.md](../../../../skills/n8n-environment-setup/templates/mcp-configs/opencode-mcp-config.md) |
| Antigravity | [mcp setup - antigravity.md](../../../../skills/n8n-environment-setup/references/ai-agent-platforms/antigravity.md) | [antigravity-mcp-config.md](../../../../skills/n8n-environment-setup/templates/mcp-configs/antigravity-mcp-config.md) |
...
- Do not mix Cloudflare production hosting controls into `_n8n-local.cmd`.
...
- Use [Page 2 - Hostinger Coolify VPS n8n](hostinger-vps.md) for always-on hosted setup.
- Use [Page 3 - Production Self-Hosting With Cloudflare Tunnel](production-cloudflare-tunnel.md) for production self-hosting from a local/CGNAT machine.
...
| Reference | Use when |
| --- | --- |
| [Page 2 - Hostinger Coolify VPS n8n](hostinger-vps.md) | You need always-on public n8n hosting on Hostinger with Coolify already set up. |
| [Page 3 - Production Self-Hosting With Cloudflare Tunnel](production-cloudflare-tunnel.md) | You need production self-hosting from a local/CGNAT machine through Cloudflare Tunnel. |
| [Local stack templates](../../templates/.n8n-local/) | You need the Docker Compose, environment template, launcher, and menu script. |
| [n8n Safety Router](../../../n8n-safety-router/) | You need the full n8n operating rules before workflow/live n8n work. |
