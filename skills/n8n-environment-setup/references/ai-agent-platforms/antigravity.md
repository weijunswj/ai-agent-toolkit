...
1. **If [Repository Agent Rules](../../../../skills/repository-agent-rules/) is installed, repo-local templates can be checked, bootstrapped, or repaired before repo edits.**

   | Need | Use |
   | --- | --- |
   | Generic Antigravity rules | [Repository Agent Rules](../../../../skills/repository-agent-rules/) |
   | Full n8n operating contract | [n8n Safety Router](../../../../skills/n8n-safety-router/) |
   | Optional n8n pointer | `GEMINI.n8n-brief.template.md` from `skills/n8n-safety-router/adapters/` |
...
4. If Antigravity does not invoke toolkit skills automatically, keep any global `GEMINI.md` nudge tiny:
   ...
   ```text
   Before editing files in an unchecked repository, use `repository-agent-rules` to check, bootstrap, or repair repo-local instructions. For n8n work, load the [official n8n Skills](https://github.com/n8n-io/skills) entry-point meta-skill, currently `using-n8n-skills`, and `n8n-safety-router` before workflow design or live-instance proposals.
   ```
