...
Install the official plugin from upstream inside Claude Code:
...
On Windows, repair and audit the installed plugin cache before approving or trusting hooks. Replace `<plugin-cache-path>` with the actual installed `n8n-skills@n8n-io` cache folder:
...
The repair step parses `hooks/hooks.json`, rewrites generic `.sh` hook commands through `hooks/run-hook.ps1`, rejects bare `bash`, rejects `C:\WINDOWS\system32\bash.exe`, and patches `n8n-skills@n8n-io` hook emitters with Node JSON fallbacks for Windows Git Bash. If repair fails, do not approve or trust the hooks; fix the reported issue or use the upstream "Other platforms" route plus the `AGENTS.md` or `CLAUDE.md` cue below.
...
**If [Repository Agent Rules](../../../../skills/repository-agent-rules/) is installed, repo-local templates can be checked, bootstrapped, or repaired before repo edits.**

| Need | Use |
| --- | --- |
| Generic Claude Code rules | [Repository Agent Rules](../../../../skills/repository-agent-rules/) |
| Full n8n operating contract | [n8n Safety Router](../../../../skills/n8n-safety-router/) |
| Optional n8n pointer | `CLAUDE.n8n-brief.template.md` from `skills/n8n-safety-router/adapters/` |
