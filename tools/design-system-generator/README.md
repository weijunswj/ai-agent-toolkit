...
This tool searches bundled CSV data under [data/](data/) and returns design-system recommendations from local files only. It is separate from the instruction-only frontend design skill so executable tooling does not live under [skills/](../../skills/).
...
- If persistence is added later, writes must be restricted to `tools/design-system-generator/output/` and require explicit user intent.
